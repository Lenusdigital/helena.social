...
//   // Update brush scaling based on the new canvas size
//   brushSize = Math.min(brushSize, newWidth / 10);  // Ensure brush size is not too large
//   drawScene();
// }
...
let lastFx = null, lastFy = null; // Store previous fixed‑FBO coordinates
...
//   // Scale the brush to the fixed resolution based on canvas dimensions
//   const scaleX = fixedFBOWidth / canvas.width;
//   const scaleY = fixedFBOHeight / canvas.height;
//   const fx = x * scaleX;
//   const fy = y * scaleY;
...
async function saveArtwork(name, isOverwrite = false, existingId = null) {
    try {
        ...
        // // Step 5: Convert the composed canvas to a PNG blob
        // offscreenCanvas.toBlob(async (blob) => {
        //     if (!blob) {
        //         console.error("Failed to generate artwork blob.");
        //         showStatusMessage("Error saving artwork.", "error");
        ...
        //     const id = isOverwrite && existingId ? existingId : Date.now(); // Use existing ID if overwriting
        //     const artwork = {
        //         id,
        //         name: name || `Untitled ${id}`,
        //         date: new Date().toISOString(),
        ...
        //         // The 'put' method will update an existing entry with the same 'id' or add a new one if not found
        //         store.put(artwork); // This should now overwrite the existing artwork if the ID matches
        //         showStatusMessage("Artwork saved!", "success");
        //     };
