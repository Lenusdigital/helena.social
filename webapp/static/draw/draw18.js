console.log("Helena Paint - draw17.js")
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("footer").innerHTML += " Loaded"
...
function updateChatUsers() {
    ...
    profileCache.forEach((profile, id) => {
        ...
        avatar.src = profile.profileImage || "{{ url_for('static', filename='draw/images/icons/cat.svg') }}";
...
function addChatMessage(msgObj, type) {
    ...
    if (type === "system") {
        ...
        msgDiv.innerHTML = `${avatar} <span class="chat-text">[${sender}]: ${msgObj.message}</span>`;
    }
...
function sendCurrentArtworkToChat(name = "Untitled") {
    try {
        ...
        offscreenCanvas.toBlob((blob) => {
            if (!blob) {
                console.error("[sendCurrentArtworkToChat] ERROR: Failed to generate artwork blob.");
...
function updateBrushSizeToggleUI() {
  if (minimized) {
    ...
    brushSizeSliderContainer.style.left = "1rem";
