...
let dynamicBrushMode = true;



document.addEventListener("keydown", (event) => {
    if (event.key === "d") { // Press 'D' to toggle
        dynamicBrushMode = !dynamicBrushMode;
        console.log("Dynamic Brush Mode:", dynamicBrushMode ? "ON" : "OFF");
    }
});

// OPTIONAL: Add a button for UI control
// document.getElementById("toggleDynamicBrush").addEventListener("click", () => {
//     dynamicBrushMode = !dynamicBrushMode;
//     document.getElementById("toggleDynamicBrush").innerText = dynamicBrushMode ? "Dynamic Brush: ON" : "Dynamic Brush: OFF";
// });
...
let minBrushSize = 0.5;  // Smallest brush size
let maxBrushSize = 1;   // Largest brush size
let speedFactor = 0.05;  // Adjust sensitivity
...
function drawBrushStrokeToPaintLayer(x, y) {
    ...
    if (dynamicBrushMode) {
        if (lastMouseX === null || lastMouseY === null) {
            // Initialize on first movement instead of skipping speed calculation
            lastMouseX = x;
            lastMouseY = y;
            lastDrawTime = now;
        } else {
            ...
            let timeElapsed = Math.max(1, now - lastDrawTime); // Prevent zero or negative time

            let speed = distance / timeElapsed; // Speed = Distance / Time
            let dynamicSize = maxBrushSize - speed * speedFactor;

            // Ensure brush size stays in valid range
            brushSize = Math.max(minBrushSize, Math.min(dynamicSize, maxBrushSize));

            // 🔥 Now this should always log!
            console.log("Speed:", speed, "Dynamic Size:", dynamicSize, "Brush Size:", brushSize);
        }
    ...
    if (lineMode && lastFx !== null && lastFy !== null) {
        let dxFixed = fx - lastFx;
        let dyFixed = fy - lastFy;
        let dist = Math.sqrt(dxFixed * dxFixed + dyFixed * dyFixed);
        let stepSize = brushSize * fixedFBOWidth * lineStepFactor;
        let steps = Math.max(1, Math.floor(dist / stepSize));

        for (let i = 0; i <= steps; i++) {
            let interpX = lastFx + (dxFixed * i) / steps;
            let interpY = lastFy + (dyFixed * i) / steps;
