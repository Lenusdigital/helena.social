...
function onTransformPointerMove(ev) {
  ...
  const applyGrab = (Li, ref) => {
    Li.x = ref.x + dxCanvas * toDocX;
    Li.y = ref.y + dyCanvas * toDocY;
  };

  const applyScale = (Li, ref) => {
    // Single-key S: move pointer away from pivot → grow, toward → shrink (uniform)
    ...
    let sxNew = Math.min(MAX_SCALE, Math.max(MIN_SCALE, ref.scaleX * f));
    let syNew = Math.min(MAX_SCALE, Math.max(MIN_SCALE, ref.scaleY * f));
    Li.scaleX = sxNew;
    Li.scaleY = syNew;

    // keep pivot stationary in document space
    const px = L.px; const py = L.py; // shared pivot
    const dx0 = ref.x - px;
    const dy0 = ref.y - py;
    Li.x = px + dx0 * (sxNew / ref.scaleX);
    Li.y = py + dy0 * (syNew / ref.scaleY);
  };

  const applyRotate = (Li, ref) => {
    const pc = transformTool.pivotCanvas || { cx: canvas.width * 0.5, cy: canvas.height * 0.5 };
    ...
    // rotate position around pivot to keep pivot under cursor
    const px = L.px; const py = L.py;
    const dx0 = ref.x - px;
    const dy0 = ref.y - py;
  ...
  if (transformTool.multi && transformTool.multi.length > 1) {
    const keepActive = activeLayerIndex;
    if (transformTool.mode === "grab") {
      for (const m of transformTool.multi) applyGrab(layers[m.index], m.ref);
    } else if (transformTool.mode === "scale") {
      for (const m of transformTool.multi) applyScale(layers[m.index], m.ref);
    } else if (transformTool.mode === "rotate") {
      for (const m of transformTool.multi) applyRotate(layers[m.index], m.ref);
    }
    activeLayerIndex = keepActive;
    syncActiveAliases?.();
  } else {
    if (transformTool.mode === "grab") {
      applyGrab(L, transformTool.ref);
    } else if (transformTool.mode === "scale") {
      applyScale(L, transformTool.ref);
    } else if (transformTool.mode === "rotate") {
      applyRotate(L, transformTool.ref);
    }
  }
...
function startTransform(mode, withShift = false) {
  ...
  // pivot from pointer (fallback center)
  const pcx = (typeof lastX !== "undefined" && lastX != null) ? lastX : canvas.width * 0.5;
  const pcy = (typeof lastY !== "undefined" && lastY != null) ? lastY : canvas.height * 0.5;
  const pf = canvasPxToFBO ? canvasPxToFBO(pcx, pcy) : { fx: fixedFBOWidth * 0.5, fy: fixedFBOHeight * 0.5 };
  ...
  // IMPORTANT: scale ignores Shift entirely (unified S). Rotate may still use withShift if desired.
  ...
  // group transform if multiple layers are selected
  let multi = null;
  if (selectedLayerIndices && selectedLayerIndices.size > 1) {
    multi = Array.from(selectedLayerIndices).map((idx) => {
      const Li = layers[idx];
      if (!Li) return null;
      ...
      // align shared pivot for consistent gesture
      Li.px = L.px; Li.py = L.py;
      return {
        index: idx,
        ref: { x: Li.x, y: Li.y, scaleX: Li.scaleX, scaleY: Li.scaleY, rotation: Li.rotation }
      };
    }).filter(Boolean);
  }
  transformTool.multi = (multi && multi.length > 1) ? multi : null;

  if (typeof isDrawing !== "undefined" && isDrawing) { isDrawing = false; stopRender("draw"); }
...
function endTransform(bake = true) {
  ...
  if (bake) {
    if (transformTool.multi && transformTool.multi.length > 1) {
      const befores = [];
      const afters  = [];
      const keepIdx = activeLayerIndex;

      for (const m of transformTool.multi) {
        befores.push({ index: m.index, snap: snapshotLayer(m.index) });
      }

      for (const m of transformTool.multi) {
        activeLayerIndex = m.index;
        try { applyActiveLayerTransform?.(); } catch {}
      }

      for (const m of transformTool.multi) {
        afters.push({ index: m.index, snap: snapshotLayer(m.index) });
      }

      activeLayerIndex = keepIdx;
      syncActiveAliases?.();

      History.push({ type: "transform_bake_multi", befores, afters });
      didBake = true;
    } else {
...
// REPLACE ENTIRE BLOCK: History core (adds transform_bake_multi + merge_layers support)
const History = {
  stack: [],
  redo: [],
  limit: UNDO_LIMIT,
    ...
    if (action.type === "merge_layers") {
      try {
        ...
        if (Array.isArray(action.removed)) {
          for (const r of action.removed) if (r?.data?.snapshot) disposeSnapshot(r.data.snapshot);
        }
      } catch (e) { console.warn("dispose merge_layers", e); }
    }

    if (action.type === "transform_bake_multi") {
      try {
        if (Array.isArray(action.befores)) for (const b of action.befores) disposeSnapshot(b.snap);
        if (Array.isArray(action.afters))  for (const a of action.afters)  disposeSnapshot(a.snap);
      } catch (e) { console.warn("dispose transform_bake_multi", e); }
    }
  },
  ...
  _apply(action) {
    switch (action.type) {
      ...
      case "transform_bake_multi":
        if (Array.isArray(action.afters)) {
          for (const a of action.afters) restoreLayerSnapshot(a.snap);
        }
        break;
  ...
  _applyInverse(action) {
    switch (action.type) {
      ...
      case "transform_bake_multi":
        if (Array.isArray(action.befores)) {
          for (const b of action.befores) restoreLayerSnapshot(b.snap);
        }
        break;
...
// REPLACE ENTIRE BLOCK: Transform & layer hotkeys
document.addEventListener("keydown", (e) => {
  ...
  // Ignore only system combos; Shift is allowed
  if (e.ctrlKey || e.metaKey || e.altKey) return;
  ...
  // Nudge while transforming (desktop)
  if (typeof isMobile === "function" && !isMobile() && window.transformTool?.mode !== "idle" && isArrow) {
    ...
    if (mode === "grab") {
      ...
      // Uniform nudge; Shift just increases step size
  ...
  // Reorder with Shift+Arrow (idle, desktop)
  if (typeof isMobile === "function"
      && !isMobile()
      && window.transformTool?.mode === "idle"
      && e.shiftKey
      && (k === "ArrowUp" || k === "ArrowDown")) {

    e.preventDefault(); e.stopImmediatePropagation();
    if (!Array.isArray(window.layers) || !layers.length) return;

    const cur = Number.isInteger(window.activeLayerIndex) ? activeLayerIndex : 0;
    const delta = (k === "ArrowUp") ? +1 : -1;
    const to = Math.max(0, Math.min(layers.length - 1, cur + delta));
    if (to === cur) return;

    const moved = layers.splice(cur, 1)[0];
    layers.splice(to, 0, moved);
    activeLayerIndex = to;

    try { window.recordLayerMove?.(cur, to); } catch {}
    try { window.rebuildLayersUI?.(); } catch {}
    try { window.syncActiveAliases?.(); } catch {}
  ...
  // Select next/prev with Arrow (idle, desktop)
  if (typeof isMobile === "function"
      && !isMobile()
      && window.transformTool?.mode === "idle"
      && !e.shiftKey
  ...
  if (lower === "h") {
    e.preventDefault(); e.stopImmediatePropagation();
    const idx = window.activeLayerIndex;
    const L = Array.isArray(window.layers) ? layers?.[idx] : null;
    if (!L) return;
    const prev = !!L.visible;
    L.visible = !prev;
    try { window.recordLayerVisibilityChange?.(idx, prev, L.visible); } catch {}
    try { window.rebuildLayersUI?.(); } catch {}
    window.needsRedraw = true;
    requestDrawIfIdle?.();
    return;
  ...
  if (lower === "g") {
    e.preventDefault(); e.stopImmediatePropagation();
    window.startTransform?.("grab");
    return;
  }

  // S: scale — Shift+S behaves identically (unified); scaling down/up by cursor motion
  if (lower === "s") {
    e.preventDefault(); e.stopImmediatePropagation();
    window.startTransform?.("scale", false); // ignore Shift for scale
    return;
  }

  // R: rotate (Shift can invert direction if your code uses it)
  if (lower === "r") {
    e.preventDefault(); e.stopImmediatePropagation();
    window.startTransform?.("rotate", !!e.shiftKey);
    return;
  }

  // Esc: cancel transform
  if (k === "Escape" && window.transformTool?.mode !== "idle") {
    const L = typeof getActiveLayer === "function" ? getActiveLayer() : null;
    ...
    window.endTransform?.(false);
    window.needsRedraw = true;
    e.preventDefault(); e.stopImmediatePropagation();
    return;
  }

  // Enter/Return: confirm transform
  if ((k === "Enter" || k === "Return") && window.transformTool?.mode !== "idle") {
    window.endTransform?.(true);
    window.needsRedraw = true;
    e.preventDefault(); e.stopImmediatePropagation();
    return;
  }
