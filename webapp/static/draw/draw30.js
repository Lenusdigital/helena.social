...
/* 
  Guard: block drawing while a two-finger gesture is active.
  (Prevents stray strokes after a pinch.)
*/
...
function endTransform(bake = true) {
  ...
  if (!wasActive) return;         // <— early out, do nothing
...
/* 
  Pointer → canvas/FBO coordinates (canonical)
  - Use this *single* definition set.
  - All event handlers should call pointerToBoth().
*/
...
let lastDrawTime = 0;      // ~60fps throttle for moves
...
canvas.addEventListener("mousedown", (ev) => {
  ...
  if (currentTool === "fill") {
    ...
    needsRedraw = true;    // single refresh for the result
    drawScene();
    return;
  }

  isDrawing = true;
  startRender("draw");     // RAF only while drawing
  drawBrushStrokeToPaintLayer(p.cx, p.cy);
});
...
canvas.addEventListener("mousemove", (ev) => {
  ...
  // update brush angle from last canvas position
  if (lastX !== null && lastY !== null) {
  ...
  // NOTE: no redraw on idle hover — zero cost when not drawing
});

canvas.addEventListener("mouseup", () => {
  isDrawing = false;
  stopRender("draw");      // loop stops naturally
