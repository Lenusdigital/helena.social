...
// Composes layers exactly like your on-screen WebGL draw.
// When useViewTransform = true, applies pan/zoom/viewRotation so the result matches the screen.
// Returns an HTMLCanvasElement with top-left origin pixels (ready for 2D use/cropping).
function composeToCanvas(useViewTransform = false) {
  const Wdoc = fixedFBOWidth | 0;
  const Hdoc = fixedFBOHeight | 0;

  // Output size
  const outW = useViewTransform ? (canvas.width | 0) : Wdoc;
  const outH = useViewTransform ? (canvas.height | 0) : Hdoc;

  // --- 1) FBO target (like exportDrawingWithModal) ---
  const outTex = gl.createTexture();
  gl.bindTexture(gl.TEXTURE_2D, outTex);
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, outW, outH, 0, gl.RGBA, gl.UNSIGNED_BYTE, null);
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
  ...
  // IMPORTANT: -1.0 to keep geometry upright in doc coords
  if (uFlipY)      gl.uniform1f(uFlipY, -1.0);
  if (uResolution) gl.uniform2f(uResolution, outW, outH);

  // Shared VBO for quad vertices (pos.xy, uv.xy)
  const vbo = gl.createBuffer();
  gl.bindBuffer(gl.ARRAY_BUFFER, vbo);
  const posLoc = gl.getAttribLocation(quadProgram, "a_position");
  const uvLoc  = gl.getAttribLocation(quadProgram, "a_texCoord");
  ...
  // --- 3) Doc→Output transform (matches the screen when useViewTransform = true) ---
  const cx = outW * 0.5;
  const cy = outH * 0.5;
  const zs = zoomScale || 1;
  const rot = viewRotation || 0;
  const c = Math.cos(rot), s = Math.sin(rot);

  function docToOut(x, y) {
    if (!useViewTransform) return { x, y };
    // replicate 2D path: translate(panX,panY) + center, rotate, scale, then place doc center
    // start in doc space (origin top-left), so shift doc center to (0,0)
    x -= Wdoc * 0.5; y -= Hdoc * 0.5;
    // scale+rotate
    const rx = (x * zs) * c - (y * zs) * s;
    const ry = (x * zs) * s + (y * zs) * c;
    // place at output center + pan
    return { x: panX + cx + rx, y: panY + cy + ry };
  }
  ...
  if (currentImage) {
    ...
    const b0 = docToOut(0,    0);
    const b1 = docToOut(Wdoc, 0);
    const b2 = docToOut(0,    Hdoc);
    const b3 = docToOut(Wdoc, Hdoc);
    drawQuad(bgTex, b0, b1, b2, b3, 1.0);
    gl.deleteTexture(bgTex);
  }

  // --- 5) Layers bottom→top (v62 transform math) ---
  for (let i = 0; i < layers.length; i++) {
    const L = layers[i];
    if (!L || L.visible === false || (L.opacity ?? 1) <= 0 || !L.texture) continue;

    const Lx = Number.isFinite(L.ox) ? L.ox : 0;
    const Ly = Number.isFinite(L.oy) ? L.oy : 0;
    const Lw = Math.max(1, (Number(L.texW) || Wdoc) | 0);
    const Lh = Math.max(1, (Number(L.texH) || Hdoc) | 0);
    ...
    function tf(x, y) {
      let rx = x - pivX, ry = y - pivY;
      rx *= sx; ry *= sy;
      const wx = pivX + (rx * rc - ry * rs) + dx;
      const wy = pivY + (rx * rs + ry * rc) + dy;
      return useViewTransform ? docToOut(wx, wy) : { x: wx, y: wy };
    }
  ...
  const outCtx = outCanvas.getContext("2d");
  outCtx.putImageData(new ImageData(flipped, outW, outH), 0, 0);
...
// ==== Ported non-transform features from v70 (v71) ====
