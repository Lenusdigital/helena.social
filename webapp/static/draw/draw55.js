...
//   // Compose into a 2D canvas in document space (no resampling/gamma surprises)
//   const out = document.createElement("canvas");
//   out.width = W; out.height = H;
...
//   for (let i = 0; i < layers.length; i++) {
//     const L = layers[i];
//     // IMPORTANT: treat undefined as visible=true (cloned layers sometimes miss the flag)
//     if (!L || L.visible === false) continue;
//     if (L.opacity != null && L.opacity <= 0) continue;

//     // Safe numbers / defaults
...
//     const rw = Math.max(1, (Number(L.texW) || W) | 0);
//     const rh = Math.max(1, (Number(L.texH) || H) | 0);

//     // Read pixels from this layer’s FBO
//     const pixels = new Uint8Array(rw * rh * 4);
//     gl.bindFramebuffer(gl.FRAMEBUFFER, L.fbo);
//     gl.readPixels(0, 0, rw, rh, gl.RGBA, gl.UNSIGNED_BYTE, pixels);
//     gl.bindFramebuffer(gl.FRAMEBUFFER, null);

//     // CPU row-flip to make the image upright (no drawImage flip = avoids resampling/gamma)
//     const flipped = new Uint8ClampedArray(rw * rh * 4);
//     const rowBytes = rw * 4;
//     for (let y = 0; y < rh; y++) {
//       const src = y * rowBytes;
//       const dst = (rh - 1 - y) * rowBytes;
//       flipped.set(pixels.subarray(src, src + rowBytes), dst);
//     }

//     // Pack into an offscreen canvas without smoothing
//     const layerCanvas = document.createElement("canvas");
//     layerCanvas.width = rw; layerCanvas.height = rh;
//     const lctx = layerCanvas.getContext("2d");
//     lctx.imageSmoothingEnabled = false;
//     lctx.putImageData(new ImageData(flipped, rw, rh), 0, 0);

//     // Pivoted transform in document space, then draw the valid sub-rect at (ox, oy)
//     ctx.save();
//     ctx.imageSmoothingEnabled = false;
//     ctx.globalAlpha = Math.max(0, Math.min(1, Number(L.opacity) || 1));

//     ctx.translate(pivX + lx, pivY + ly);
//     if (rot) ctx.rotate(rot);
//     if (sx !== 1 || sy !== 1) ctx.scale(sx, sy);
//     ctx.translate(-pivX, -pivY);

//     ctx.drawImage(layerCanvas, 0, 0, rw, rh, ox, oy, rw, rh);
//     ctx.restore();
//   }
