...
// Applies pointer deltas to the active layer while transforming.
// Uses a layer-local pivot (L.px/L.py) seeded at gesture start.
// Logs per-frame deltas and resulting transforms when enabled.
function onTransformPointerMove(ev) {
  ...
  const c = screenToCanvasPx(ev.clientX, ev.clientY, false);
  const f = canvasPxToFBO(c.x, c.y);
  const p = { cx: c.x, cy: c.y, fx: f.fx, fy: f.fy };

  if (!transformTool.start) {
    transformTool.start = { cx: p.cx, cy: p.cy };

    // seed layer-local pivot at gesture start to match the red ring (current pointer)
    const pf0 = canvasPxToFBO(transformTool.pivotCanvas ? transformTool.pivotCanvas.cx : p.cx,
                              transformTool.pivotCanvas ? transformTool.pivotCanvas.cy : p.cy);
    L.px = pf0.fx - L.x;
    L.py = pf0.fy - L.y;
    transformTool.pivotCanvas = { cx: transformTool.pivotCanvas ? transformTool.pivotCanvas.cx : p.cx,
                                  cy: transformTool.pivotCanvas ? transformTool.pivotCanvas.cy : p.cy };

    if (typeof PIVOT_DEBUG !== "undefined" && PIVOT_DEBUG) {
      console.log("[pivot] seed gesture", {
        mode: transformTool.mode,
        start: transformTool.start,
        docPivot: { fx: pf0.fx, fy: pf0.fy },
        layerPos: { x: L.x, y: L.y },
        layerLocalPivot: { px: L.px, py: L.py },
  ...
  if (transformTool.mode === "grab") {
    ...
    // keep layer-local pivot constant (no change to L.px/L.py)
    if (typeof PIVOT_DEBUG !== "undefined" && PIVOT_DEBUG) {
      console.log("[pivot] move", { dx, dy, toDocX, toDocY, from, to: { x: L.x, y: L.y }, layerLocalPivot: { px: L.px, py: L.py } });
    }
    ...
    let fct = Math.exp(delta * SENSITIVITY);
    if (transformTool.shiftInvert) fct = 1 / Math.max(fct, 1e-6);

    const clampScale = (v) => Math.min(MAX_SCALE, Math.max(MIN_SCALE, v));
    const from = { scaleX: L.scaleX, scaleY: L.scaleY };
    L.scaleX = clampScale(transformTool.ref.scaleX * fct);
    L.scaleY = clampScale(transformTool.ref.scaleY * fct);
    // layer-local pivot remains L.px/L.py

    if (typeof PIVOT_DEBUG !== "undefined" && PIVOT_DEBUG) {
      console.log("[pivot] scale", {
        start: transformTool.start, p, pivotCanvas: pc,
        distances: { d0raw, d0, d1, delta },
        factor: fct, from, to: { scaleX: L.scaleX, scaleY: L.scaleY },
        layerLocalPivot: { px: L.px, py: L.py }
      });
    }
    ...
    const SNAP = Math.PI / 90;
    delta = Math.round(delta / SNAP) * SNAP;

    const from = L.rotation;
    ...
    if (typeof PIVOT_DEBUG !== "undefined" && PIVOT_DEBUG) {
      console.log("[pivot] rotate", {
        start: transformTool.start, p, pivotCanvas: pc,
        angles: { a0, a1, delta }, from, to: L.rotation,
        layerLocalPivot: { px: L.px, py: L.py }
      });
    }
  }
...
// Initializes a layer transform around the current pivot.
// Reads pivot from lastPointer in canvas px (fallback: canvas center),
// converts it to FBO/doc space, then to LAYER-LOCAL pivot (subtracts layer position).
// Stores the same canvas-space pivot for UI, captures a reference snapshot,
// starts the transform loop, shows status, logs if enabled, and requests redraw.
function startTransform(mode, withShift = false) {
  ...
  // layer-local pivot = document(FBO) pivot - layer position
  L.px = pf.fx - L.x;
  L.py = pf.fy - L.y;
