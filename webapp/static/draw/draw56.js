...
function mergeSelectedLayers() {
  ...
  const ids = __getSelectedIndices().filter(i => i >= 0 && i < layers.length);
  if (ids.length < 2) {
    showStatusMessage?.("Select at least two layers to merge.", "info");
    return;
  }
  ...
  // 1) Create destination doc-sized texture/FBO
  ...
  if (gl.checkFramebufferStatus(gl.FRAMEBUFFER) !== gl.FRAMEBUFFER_COMPLETE) {
    ...
    showStatusMessage?.("Merge failed: FBO incomplete.", "error");
    return;
  }

  // 2) Clear and set up pipeline
  ...
  if (uFlipY)      gl.uniform1f(uFlipY, 1.0);          // render to FBO space (no screen flip)
  if (uResolution) gl.uniform2f(uResolution, Wdoc, Hdoc);

  // one shared VBO for all draws
  ...
  // 3) Draw each selected layer in **document order** (bottom→top)
  ids.forEach((idx) => {
    const L = layers[idx];
    if (!L || !L.visible) return;

    const verts = __layerQuadInFboSpace(L, Wdoc, Hdoc);
    gl.bufferData(gl.ARRAY_BUFFER, verts, gl.STREAM_DRAW);
  ...
  // 4) Create a new merged layer (document-sized for compatibility)
  const insertAt = Math.min(...ids);
  const nameJoin = ids.map(i => layers[i]?.name || `#${i}`).join("+");
  const merged = {
    id: Date.now() + Math.random(),
    name: `Merged(${nameJoin})`,
    fbo: outFbo,
    texture: outTex,
    visible: true,
    opacity: 1,

    // reset transform (pixels already baked into outTex)
    x: 0, y: 0, scaleX: 1, scaleY: 1, rotation: 0,
    px: Wdoc * 0.5, py: Hdoc * 0.5,
  ...
  // 5) Remove source layers and insert merged at the lowest index
  const removed = [];
  // remove higher indices first so positions stay valid
  ids.slice().sort((a,b)=>b-a).forEach(i => {
    const rem = layers.splice(i, 1)[0];
    if (rem) removed.push(rem);
  });
  layers.splice(insertAt, 0, merged);
  activeLayerIndex = insertAt;

  // 6) Free GL objects of removed layers
  removed.forEach(L => {
    try { gl.deleteTexture(L.texture); } catch {}
    try { gl.deleteFramebuffer(L.fbo); } catch {}
  });

  // 7) Refresh UI + history
  try { rebuildLayersUI?.(); } catch {}
  try { syncActiveAliases?.(); } catch {}
  try { showStatusMessage?.("Layers merged", "success"); } catch {}
  needsRedraw = true;
  requestDrawIfIdle?.();
}
...
// COMPLETE REPLACEMENT — keeps brush under the pointer even when the active layer
// has translation/scale/rotation/pivot; no extra helpers, self-contained, with
// optional debug logs (toggle via: window.HP_DEBUG_PAINT = true)


// COMPLETE REPLACEMENT — drawBrushStrokeToPaintLayer
function drawBrushStrokeToPaintLayer(x, y) {
  // 0) gates & bounds (unchanged from your version)
  if (!canPaint()) return;
  ...
  // screen/canvas px -> document/FBO px
  const fx = x * (fixedFBOWidth  / canvas.width);
  const fy = y * (fixedFBOHeight / canvas.height);

  // ---- invert active-layer transform to get layer-pre coords ----
  // inline inverse transform so we don't rely on any helpers
  ...
  const rt = Number.isFinite(L.rotation) ? L.rotation : 0;

  function docToLocal(dx, dy) {
    // remove translation
    let x1 = dx - tx, y1 = dy - ty;
    // to pivot frame
    x1 -= px; y1 -= py;
    // inverse rotate
    ...
    const ry = x1 * s + y1 * c;
    // inverse scale
    const lx = (sx !== 0 ? rx / sx : rx) + px;
    const ly = (sy !== 0 ? ry / sy : ry) + py;
    return { x: lx, y: ly };
  }

  // current pointer in layer-local space
  const curLocal = docToLocal(fx, fy);

  // persistent spacing/angle state (global carry like your code)
  if (window.__stampCarry === undefined) window.__stampCarry = 0;
  if (window.__angleEMA   === undefined) window.__angleEMA   = null;

  // keep per-layer last point in local space, so switching layers works
  if (!L.__lastLocal) L.__lastLocal = null;

  // size modulation (kept as-is)
  let sizeMultiplier = 1.0;
  if (typeof dynamicModeEnabled !== "undefined" && dynamicModeEnabled) {
    sizeMultiplier = (typeof intuitiveMode !== "undefined" && intuitiveMode)
      ? (0.8 + Math.random() * 0.8) // 0.8–1.6
      : (1.5 - Math.random());      // 0.5–1.5
  }

  // First stamp bootstrap: just stamp once under the cursor (in local)
  if (!L.__lastLocal) {
    let angle0 = currentAngle;
    if (typeof reverseModeEnabled !== "undefined" && reverseModeEnabled) angle0 += Math.PI;

    if (window.HP_DEBUG_PAINT) {
      console.log("[PAINT]init",
        { fx, fy, local: curLocal, brushSize, sizeMultiplier, angle0 });
    }

    drawSingleBrushStamp(curLocal.x, curLocal.y, brushSize * sizeMultiplier, angle0);
    L.__lastLocal = { x: curLocal.x, y: curLocal.y };
    lastFx = fx; lastFy = fy; // keep your original globals in doc space
    lastX = x; lastY = y;

    needsRedraw = true;
    strokeCount++;
    if (typeof FLATTEN_THRESHOLD !== "undefined" && strokeCount >= FLATTEN_THRESHOLD) {
      if (window.HP_DEBUG_PAINT) console.log("[PAINT]flatten @ init");
      flattenStrokes?.();
    }
    return;
  }

  // Vector in LOCAL (layer-pre) space for spacing/orientation of stamps
  const lx0 = L.__lastLocal.x, ly0 = L.__lastLocal.y;
  const dxL = curLocal.x - lx0;
  const dyL = curLocal.y - ly0;
  const segDist = Math.hypot(dxL, dyL);

  // derive angle (you used EMA; keep it if present)
  let actualAngle = Math.atan2(dyL, dxL);
  if (window.__angleEMA == null) window.__angleEMA = actualAngle;
  else {
    const ALPHA = 0.35;
    let a0 = window.__angleEMA, a1 = actualAngle;
    // unwrap to avoid π jumps
    while (a1 - a0 >  Math.PI) a1 -= 2*Math.PI;
    while (a1 - a0 < -Math.PI) a1 += 2*Math.PI;
    window.__angleEMA = a0 + (a1 - a0) * ALPHA;
    actualAngle = window.__angleEMA;
  }
  if (typeof reverseModeEnabled !== "undefined" && reverseModeEnabled) {
    actualAngle += Math.PI;
  }

  // brush size dynamics identical to your pipeline
  ...
  // uniform, carry-aware spacing IN LAYER-LOCAL SPACE
  let step = baseBrushW * (typeof lineStepFactor !== "undefined" ? lineStepFactor : 0.3)
           * speedFactor * sizeMultiplier;
  step = Math.max(0.5, step);

  let s = Math.max(0, step - window.__stampCarry);

  if (!lineMode) {
    // single stamp at end point (in local space)
    drawSingleBrushStamp(curLocal.x, curLocal.y, dynamicBrushSize, actualAngle);
    window.__stampCarry = (step - (segDist % step)) % step;
    if (window.HP_DEBUG_PAINT) {
      console.log("[PAINT]single", { segDist, step, carry: window.__stampCarry });
    }
  } else {
    // iterate evenly along the local-space segment
    if (window.HP_DEBUG_PAINT) {
      console.log("[PAINT]segment", {
        from: { x: lx0, y: ly0 }, to: { x: curLocal.x, y: curLocal.y },
        segDist, step, carryIn: window.__stampCarry
      });
    }
    while (s <= segDist) {
      const t = s / segDist;
    ...
    if (window.HP_DEBUG_PAINT) {
      console.log("[PAINT]done", { carryOut: window.__stampCarry });
    }
  }

  // advance cursors (store BOTH)
  L.__lastLocal = { x: curLocal.x, y: curLocal.y };
  lastFx = fx; lastFy = fy; // keep your original globals unchanged
  lastX = x;  lastY = y;

  // flatten & redraw (unchanged)
  strokeCount++;
  if (typeof FLATTEN_THRESHOLD !== "undefined" && strokeCount >= FLATTEN_THRESHOLD) {
    if (window.HP_DEBUG_PAINT) console.log("[PAINT]flatten @ strokeCount", strokeCount);
    flattenStrokes?.();
  }
  needsRedraw = true;
}
