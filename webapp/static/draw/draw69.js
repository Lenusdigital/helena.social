...
canvasWrapper.addEventListener("mousemove", (e) => {
  const c = screenToCanvasPx(e.clientX, e.clientY, /*clamp=*/true); // keep clamped for HUD
  lastPointer.cx = c.x;
  lastPointer.cy = c.y;
...
// === onTransformPointerMove (grab uses DOC deltas; scale/rotate keep pivotDoc fixed; localPivot constant) ===
function onTransformPointerMove(ev) {
  ...
  if (!transformTool.start) transformTool.start = { cx: p.cx, cy: p.cy, fx: p.fx, fy: p.fy };

  const ref      = transformTool.ref;
  const pivotDoc = transformTool.pivotDoc || { fx: ref.x, fy: ref.y };
  const local    = transformTool.localPivot || { px: 0, py: 0 };

  if (transformTool.mode === "grab") {
    // DOC-space delta to avoid jumps; move pivotDoc with the layer; keep localPivot unchanged.
    const ddx = p.fx - transformTool.start.fx;
    const ddy = p.fy - transformTool.start.fy;

    L.x = ref.x + ddx;
    L.y = ref.y + ddy;

    transformTool.pivotDoc = { fx: pivotDoc.fx + ddx, fy: pivotDoc.fy + ddy };
  ...
  if (transformTool.mode === "scale") {
    ...
    const sxC = p.cx - pc.cx;
    const syC = p.cy - pc.cy;

    const d0raw = Math.hypot(s0x, s0y);
    const d1    = Math.hypot(sxC,  syC);
    ...
    const newSX = clampScale(ref.scaleX * f);
    const newSY = clampScale(ref.scaleY * f);

    // Keep pivotDoc fixed: T' = pivotDoc - R(ref.rot) · (S'(localPivot))
    const c = Math.cos(ref.rotation), s = Math.sin(ref.rotation);
    const rx = local.px * newSX, ry = local.py * newSY;
    const rrx = rx *  c - ry * s;
    const rry = rx *  s + ry * c;

    L.scaleX = newSX;
    L.scaleY = newSY;
    L.rotation = ref.rotation;
    L.x = pivotDoc.fx - rrx;
    L.y = pivotDoc.fy - rry;
  ...
  if (transformTool.mode === "rotate") {
    ...
    const SNAP = Math.PI / 90; // ~2°
    dA = Math.round(dA / SNAP) * SNAP;

    const newRot = ref.rotation + dA;

    // Keep pivotDoc fixed: T' = pivotDoc - R(newRot) · (S(ref.scale) · localPivot)
    const c = Math.cos(newRot), s = Math.sin(newRot);
    const rx = local.px * ref.scaleX, ry = local.py * ref.scaleY;
    const rrx = rx *  c - ry * s;
    const rry = rx *  s + ry * c;
...
// === startTransform (stable world pivot + true layer-local pivot; does NOT touch L.px/L.py) ===
// Uses pointer (canvas px) → doc/FBO pivot, then computes local pivot via inverse pose.
// Stores: pivotCanvas (UI), pivotDoc (world/doc), localPivot (layer space), full ref pose.
// Leaves L.px/L.py untouched to avoid mixing spaces; logs kept by outer flags.
function startTransform(mode, withShift = false) {
  const L = getActiveLayer();
  if (!L) { console.warn("[pivot] startTransform: no active layer"); return; }
  ...
  // pointer pivot (canvas px; fallback center)
  ...
  // canvas → doc/FBO pivot
  const pf = canvasPxToFBO(pcx, pcy); // { fx, fy }

  // inverse map to layer-local (DO NOT write L.px/L.py here)
  // local = S^-1 · R(-rot) · (world - T)
  const vx = pf.fx - L.x;
  const vy = pf.fy - L.y;
  const c = Math.cos(L.rotation), s = Math.sin(L.rotation);
  const ix =  vx *  c + vy * s;   // R(-rot)
  const iy = -vx *  s + vy * c;
  const localPivot = {
    px: ix / (L.scaleX || 1),
    py: iy / (L.scaleY || 1)
  ...
  transformTool.pivotCanvas = { cx: pcx, cy: pcy };     // UI
  transformTool.pivotDoc    = { fx: pf.fx, fy: pf.fy }; // world/doc (must remain fixed for scale/rotate)
  transformTool.localPivot  = { ...localPivot };        // layer-local (must remain constant during gesture)
  transformTool.ref = {
    x: L.x, y: L.y,
  ...
  showStatusMessage(
    ...
    mode === "rotate" ? "Rotate layer" : String(mode)
  );
...
// Initializes a transform gesture using a stable world pivot and a layer-local pivot
// computed via inverse of the CURRENT layer pose. Does NOT mutate L.px/L.py.
// Stores: pivotCanvas (UI), pivotDoc (world/FBO), localPivot (layer space), and full ref pose.
// Keeps existing status/logging and requests a redraw.
function startTransform(mode, withShift = false) {
  const L = getActiveLayer();
  if (!L) { console.warn("[pivot] startTransform: no active layer"); return; }
  ...
  // pointer → canvas px (fallback: center)
  const pcx = (lastPointer && Number.isFinite(lastPointer.cx)) ? lastPointer.cx : canvas.width  * 0.5;
  const pcy = (lastPointer && Number.isFinite(lastPointer.cy)) ? lastPointer.cy : canvas.height * 0.5;

  // canvas → document/FBO px
  const pf = canvasPxToFBO(pcx, pcy); // { fx, fy }

  // Compute local pivot via inverse pose WITHOUT touching L.px/L.py
  // world = T + R(rot) * (S * local)  =>  local = S^{-1} * R(-rot) * (world - T)
  const vx = pf.fx - L.x;
  const vy = pf.fy - L.y;
  const c = Math.cos(L.rotation), s = Math.sin(L.rotation);
  const rx =  vx *  c + vy * s;    // R(-rot)
  const ry = -vx *  s + vy * c;
  const localPivot = {
    px: rx / (L.scaleX || 1),
    py: ry / (L.scaleY || 1)
  ...
  transformTool.mode        = mode;
  transformTool.shiftInvert = !!withShift;
  transformTool.pivotCanvas = { cx: pcx, cy: pcy };     // UI-space pivot (for angle/scale gesture math)
  transformTool.pivotDoc    = { fx: pf.fx, fy: pf.fy }; // world-space pivot to remain fixed
  transformTool.localPivot  = { ...localPivot };        // layer-local pivot for re-solving translation
  transformTool.ref = {
    x: L.x, y: L.y,
    scaleX: L.scaleX, scaleY: L.scaleY,
    rotation: L.rotation
  };
  transformTool.start = null; // filled on first move

  if (isDrawing) { isDrawing = false; stopRender("draw"); }
  startRender("transform");

  if (typeof PIVOT_DEBUG !== "undefined" && PIVOT_DEBUG) {
    console.log("[pivot] startTransform", {
      mode, withShift,
      lastPointer: lastPointer ? { cx: lastPointer.cx, cy: lastPointer.cy } : null,
      pivotCanvas: transformTool.pivotCanvas,
      pivotDoc: transformTool.pivotDoc,
      localPivot: transformTool.localPivot,
      ref: { ...transformTool.ref }
    });
  }
