...
function endTransform(bake = true) {
  ...
  // —— prevent pixel loss when the transformed quad goes off the fixed FBO ——
  try {
    ...
    // If guard fails, proceed without blocking bake
...
// ===============================================
// Merge selected layers into a single new layer
// ===============================================
// Drop-in fix: Merge (Selected/All) without vertical flip, no crop, stable undo/redo.

function mergeSelectedLayers() {
  if (!gl || !layers || !selectedLayerIndices || selectedLayerIndices.size < 2) return;

  const W = fixedFBOWidth | 0;
  const H = fixedFBOHeight | 0;

  // ----- helpers -----
  function drawLayerIntoDocFBO(layer, targetW, targetH) {
    // Build transformed quad DIRECTLY in document pixels (no screen scale)
    const Lx = Number.isFinite(layer.ox)   ? layer.ox   : 0;
    const Ly = Number.isFinite(layer.oy)   ? layer.oy   : 0;
    const Lw = Math.max(1, Number.isFinite(layer.texW) ? layer.texW : targetW);
    const Lh = Math.max(1, Number.isFinite(layer.texH) ? layer.texH : targetH);

    const pivX = Number.isFinite(layer.px) ? layer.px : targetW * 0.5;
    const pivY = Number.isFinite(layer.py) ? layer.py : targetH * 0.5;

    const c = Math.cos(layer.rotation || 0);
    const s = Math.sin(layer.rotation || 0);
    const scx = Number.isFinite(layer.scaleX) ? layer.scaleX : 1;
    const scy = Number.isFinite(layer.scaleY) ? layer.scaleY : 1;
    ...
    // IMPORTANT: rendering into an FBO in document space → no Y flip
    const uFlipY = gl.getUniformLocation(quadProgram, "u_flipY");
    if (uFlipY) gl.uniform1f(uFlipY, 1.0); // (screen draws use -1.0)

    // Resolution must match the CURRENT framebuffer (the merge target)
  ...
  function createDocSizedTarget(w, h) {
  ...
  // ----- record originals for undo -----
  const sel = Array.from(selectedLayerIndices).sort((a, b) => a - b);
  const removed = sel.map((idx) => ({
    index: idx,
    data: {
      name: layers[idx].name,
      visible: layers[idx].visible,
      opacity: layers[idx].opacity,
      snapshot: snapshotLayer(idx)
    }
  }));

  // ----- create merge FBO (full doc → no crop) -----
  const target = createDocSizedTarget(W, H);
  ...
  // ----- compose bottom → top into doc FBO (no flip) -----
  for (const idx of sel) {
    const L = layers[idx];
    if (!L || !L.visible || (L.opacity || 0) <= 0 || !gl.isTexture(L.texture)) continue;
    drawLayerIntoDocFBO(L, W, H);
  }

  gl.bindFramebuffer(gl.FRAMEBUFFER, null);

  // ----- replace selection by merged layer at topmost index -----
  const insertAt = sel[sel.length - 1];
  const merged = {
    id: Date.now() + Math.random(),
    ...
    visible: true,
    opacity: 1,
    // merged is a full-doc layer with neutral transform → no future flips
  ...
  // delete originals (top→bottom to keep indices valid)
  for (let k = sel.length - 1; k >= 0; k--) {
    const i = sel[k];
    const L = layers[i];
    try { if (L?.texture && gl.isTexture(L.texture)) gl.deleteTexture(L.texture); } catch {}
    try { if (L?.fbo) gl.deleteFramebuffer(L.fbo); } catch {}
    layers.splice(i, 1);
  }
  layers.splice(Math.min(insertAt, layers.length), 0, merged);
  activeLayerIndex = Math.min(insertAt, layers.length - 1);
  ...
  const added = {
    ...
    data: {
      ...
      snapshot: snapshotLayer(activeLayerIndex)
    }
  };
...
// Drop-in fix: keeps layer rect/scale stable after stroke-flattening
// Drop-in replacement: live "flatten while drawing" WITHOUT CROPPING (even after transforms/moves)
...
function flattenStrokes() {
  ...
  const DOC_W = fixedFBOWidth | 0;
  const DOC_H = fixedFBOHeight | 0;

  // Source rect in document coords (existing layer placement)
  const srcOx = Number.isFinite(L.ox)   ? L.ox   : 0;
  const srcOy = Number.isFinite(L.oy)   ? L.oy   : 0;
  const srcW  = Math.max(1, (Number(L.texW) || DOC_W) | 0);
  const srcH  = Math.max(1, (Number(L.texH) || DOC_H) | 0);
  ...
  try {
    // 1) Create FULL-DOCUMENT target so bounds never shrink ⇒ no future crop
    const dstTex = gl.createTexture();
    gl.bindTexture(gl.TEXTURE_2D, dstTex);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, DOC_W, DOC_H, 0, gl.RGBA, gl.UNSIGNED_BYTE, null);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
    ...
    // 2) Copy the old layer into the SAME document position inside full-doc target
    if (gl.blitFramebuffer) {
      // WebGL2: pixel-perfect FBO→FBO blit (no filtering, no flip)
      gl.bindFramebuffer(gl.READ_FRAMEBUFFER, L.fbo);
      gl.bindFramebuffer(gl.DRAW_FRAMEBUFFER, dstFbo);

      // Source: (0..srcW, 0..srcH) in the old layer FBO
      // Dest:   (srcOx..srcOx+srcW, srcOy..srcOy+srcH) in full-doc FBO
      gl.blitFramebuffer(
        0, 0, srcW, srcH,
        srcOx, srcOy, srcOx + srcW, srcOy + srcH,
        gl.COLOR_BUFFER_BIT,
        gl.NEAREST
      );
      ...
      // WebGL1: shader copy into document coordinates
      gl.useProgram(quadProgram);
      ...
      // Rendering into FBO in document space → no vertical flip
      if (uFlipY)      gl.uniform1f(uFlipY, 1.0);
      if (uResolution) gl.uniform2f(uResolution, DOC_W, DOC_H);
      if (uOpacity)    gl.uniform1f(uOpacity, 1.0);

      gl.bindFramebuffer(gl.FRAMEBUFFER, dstFbo);
      gl.viewport(0, 0, DOC_W, DOC_H);

      // Positions are the destination quad in document pixels where the layer lives now.
      // UVs span the whole source texture (0..1).
      const x0 = srcOx,      y0 = srcOy;
      const x1 = srcOx+srcW, y1 = srcOy+srcH;
    ...
    // 3) Swap in full-doc layer; reset placement to (0,0) to prevent future crops
    const oldTex = L.texture;
    const oldFbo = L.fbo;
    ...
    // IMPORTANT: full bounds — drawing after moves will NEVER clip at previous edges
    L.ox   = 0;
    L.oy   = 0;
    L.texW = DOC_W;
    L.texH = DOC_H;

    // Keep transforms untouched (x/y/scale/rotation/px/py) so nothing "jumps"
    // Clean old GPU objects
    try { if (oldTex && gl.isTexture(oldTex)) gl.deleteTexture(oldTex); } catch {}
    try { if (oldFbo) gl.deleteFramebuffer(oldFbo); } catch {}

    // reset counters/hooks
    strokeCount = 0;
    if (typeof syncActiveAliases === "function") syncActiveAliases();
  } finally {
    // --- Restore GL state ---
...
function applyActiveLayerTransform() {
  ...
  // document-space rect this layer currently occupies
  const Lx = Number.isFinite(L.ox)   ? L.ox   : 0;
  const Ly = Number.isFinite(L.oy)   ? L.oy   : 0;
  const Lw = Number.isFinite(L.texW) ? L.texW : fixedFBOWidth;
  const Lh = Number.isFinite(L.texH) ? L.texH : fixedFBOHeight;

  // pivot in document px
  const pivX = Number.isFinite(L.px) ? L.px : fixedFBOWidth  * 0.5;
  const pivY = Number.isFinite(L.py) ? L.py : fixedFBOHeight * 0.5;

  const c = Math.cos(L.rotation || 0);
  const s = Math.sin(L.rotation || 0);

  function tf(x, y) {
    // x/y are in document space
    let dx = x - pivX;
    let dy = y - pivY;
    dx *= (Number.isFinite(L.scaleX) ? L.scaleX : 1);
    dy *= (Number.isFinite(L.scaleY) ? L.scaleY : 1);
  ...
  // Transform the layer-rect corners (doc space → doc space)
  const p0 = tf(Lx,     Ly);
  const p1 = tf(Lx+Lw,  Ly);
  const p2 = tf(Lx,     Ly+Lh);
  const p3 = tf(Lx+Lw,  Ly+Lh);

  // Compute bounding box that fully contains transformed quad
  ...
  // Create output texture/FBO sized to the bbox
  ...
  if (gl.checkFramebufferStatus(gl.FRAMEBUFFER) !== gl.FRAMEBUFFER_COMPLETE) {
    console.warn("applyActiveLayerTransform: FBO incomplete (dynamic)");
    gl.bindFramebuffer(gl.FRAMEBUFFER, null);
  ...
  // Render the original layer.texture into outFBO with a transform that
  // maps the ORIGINAL rect to the new bounding box in *doc* space.
  ...
  // Build a full-quad covering the out texture [0..outW]x[0..outH]
  // and compute the positions (in that out-FBO’s pixel space) by applying
  // the same transform as above, then shifting by (-minX, -minY)
  function toOut(pt) {
    const t = tf(pt.x, pt.y); // doc px
    return { x: t.x - minX, y: t.y - minY }; // shift into out-FBO space
  }
  const q0 = toOut({x:Lx,     y:Ly    });
  const q1 = toOut({x:Lx+Lw,  y:Ly    });
  const q2 = toOut({x:Lx,     y:Ly+Lh });
  const q3 = toOut({x:Lx+Lw,  y:Ly+Lh });

  // Positions in out-FBO px, UVs in [0..1] from the source texture
  const verts = new Float32Array([
    q0.x, q0.y, 0, 0,
    q1.x, q1.y, 1, 0,
    q2.x, q2.y, 0, 1,
  ...
  const uFlipY = gl.getUniformLocation(quadProgram, "u_flipY");
  if (uFlipY) gl.uniform1f(uFlipY, 1.0); // drawing into an FBO in tex space (no screen flip)
  const uRes = gl.getUniformLocation(quadProgram, "u_resolution");
  ...
  // Replace layer GPU objects
  try { gl.deleteTexture(L.texture); } catch {}
  try { gl.deleteFramebuffer(L.fbo); } catch {}
  L.texture = outTex;
  L.fbo     = outFBO;

  // Reset transform to identity and update doc-space rect to the new bbox
  L.x = 0; L.y = 0; L.scaleX = 1; L.scaleY = 1; L.rotation = 0;

  // Keep the pivot at the same *document* position it visually was:
  // convert old pivot through tf and then into the new local coords.
  const pivDoc = tf(pivX, pivY);
  L.px = pivDoc.x - minX; // store pivot in the layer’s doc rect space? No → store doc pivot:
  L.py = pivDoc.y - minY; // However, v46 expects px/py in document space. So:
  L.px = pivDoc.x;        // keep px/py in document space (like before)
  L.py = pivDoc.y;

  // Update the layer’s rect (in document space)
  L.ox = minX;
  L.oy = minY;
  L.texW = outW;
  L.texH = outH;

  // Done; drawing now uses (ox,oy,texW,texH) and identity transform
}
