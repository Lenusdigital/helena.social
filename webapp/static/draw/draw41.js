...
/** Read an FBO into a Canvas (preserves alpha). */
function fboToCanvas(fbo, W, H) {
...
/** Export the active layer as a PNG file download. */
function exportActiveLayerAsPNG() {
  ...
  const dataURL = canvas.toDataURL("image/png"); // lossless, keeps alpha

  const name = sanitizeFilename(L.name || "Layer");
  const filename = `${name}.png`;

  // Trigger download (fallback opens in new tab if 'download' is ignored)
  ...
  // iOS/Safari fallback: open in a new tab
