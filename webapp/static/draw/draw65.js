...
/* History (drop-in full replacement)
   Unified undo/redo for:
   - Pixel edits: "stroke", "fill", "clear_layer", "transform_bake" (use CPU snapshots from layer FBO).
   - Parametric edits: "layer_transform", "layer_visibility", "layer_opacity", "layer_rename".
   - Structural edits: "layer_move", "add_layer", "remove_layer", "merge_layers", "clear_project".
   Integrates with:
   - snapshotLayer(index)              → returns { index,w,h,pixels,?transform }
   - restoreLayerSnapshot(snapshot)    → writes snapshot back to layer texture (no draws)
   - applyLayerTransformState(index,s) → applies param state only (no pixels)
*/
const History = {
  /* stacks */
  stack: [],
  ...
  /* push(action): add to undo, trim, clear redo */
  push(action) {
    this.stack.push(action);
    if (this.stack.length > this.limit) this._dispose(this.stack.shift());
    this.redo.length = 0;
    console.log("[History] push:", action.type, action);
  },

  /* undo(): apply inverse, move to redo, force redraw */
  undo() {
    ...
    console.log("[History] undo:", action.type, action);
  },

  /* redoDo(): re-apply, move back to undo, force redraw */
  redoDo() {
    ...
    console.log("[History] redo:", action.type, action);
  },

  /* _dispose(action): release large CPU buffers on eviction */
  _dispose(action) {
    const nul = (s) => { if (s && s.pixels) s.pixels = null; };
    try {
      nul(action?.before);
      nul(action?.after);
      nul(action?.removedLayer?.snapshot);
      nul(action?.addedLayer?.snapshot);
      if (action?.type === "merge_layers") {
        nul(action?.added?.data?.snapshot);
        if (Array.isArray(action?.removed)) for (const r of action.removed) nul(r?.data?.snapshot);
      }
    } catch (e) {
      console.warn("[History] dispose warning:", e);
    }
  },

  /* _apply(action): redo path */
  _apply(action) {
    switch (action.type) {
      /* pixel actions */
      case "stroke":
      ...
      case "layer_visibility": {
        const L = layers[action.index]; if (!L) break;
        L.visible = action.next;
        break;
      }
      case "layer_opacity": {
        const L = layers[action.index]; if (!L) break;
        L.opacity = action.next;
        break;
      }
      case "layer_rename": {
        const L = layers[action.index]; if (!L) break;
  ...
  /* _applyInverse(action): undo path */
  _applyInverse(action) {
    switch (action.type) {
      ...
      case "layer_visibility": {
        const L = layers[action.index]; if (!L) break;
        L.visible = action.prev;
        break;
      }
      case "layer_opacity": {
        const L = layers[action.index]; if (!L) break;
        L.opacity = action.prev;
        break;
      }
      case "layer_rename": {
        const L = layers[action.index]; if (!L) break;
  ...
  /* _moveLayer(from,to): reorder list + UI */
  _moveLayer(from, to) {
    if (from === to) return;
    ...
    syncActiveAliases?.();
    rebuildLayersUI?.();
    console.log("[History] _moveLayer:", { from, to });
  },

  /* _removeLayerAt(index): delete GL resources, remove from list */
  _removeLayerAt(index) {
    ...
    syncActiveAliases?.();
    rebuildLayersUI?.();
    console.log("[History] _removeLayerAt:", { index, activeLayerIndex });
  },

  /* _insertLayer(index,data): create GL resources, insert, optionally apply snapshot */
  _insertLayer(index, data) {
    ...
    syncActiveAliases?.();
    rebuildLayersUI?.();
    console.log("[History] _insertLayer:", { index, name: layer.name });
  },

  /* _captureProject(): CPU-only capture of all layers + optional background */
  _captureProject() {
    return {
      w: fixedFBOWidth,
      h: fixedFBOHeight,
  ...
  /* _restoreProject(all): rebuild layers from CPU snapshots */
  _restoreProject(all) {
    if (!all) return;
    try {
      // dispose current
      ...
      needsRedraw = true;
      requestDrawIfIdle?.();
      console.log("[History] _restoreProject: done", { count: layers.length });
    } catch (e) {
