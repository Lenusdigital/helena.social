...
/* toggleUIVisibilityMode()
   Toggles a “clean canvas” view: hides all UI except the drawing canvases and a small eye button in the top-left.
   Implementation:
   - Adds a <style> rule using :has() to hide any element that is neither a <canvas> nor an ancestor of a <canvas>,
     and also keeps the toggle button and its ancestors visible.
   - Creates the eye toggle button once; clicking it toggles the mode on/off.
   - No emojis; inline SVG icon; no extra global listeners. */
function toggleUIVisibilityMode() {
  // one-time setup
  if (!toggleUIVisibilityMode.__init) {
    // style: hide everything except canvases and the toggle button (and their ancestors)
    const css = `
      body.__ui_off :not(canvas):not(#__uiToggleBtn):not(:has(canvas)):not(:has(#__uiToggleBtn)) {
        visibility: hidden !important;
      }
      /* ensure canvases remain interactive while UI is hidden */
      body.__ui_off canvas { pointer-events: auto !important; }
      /* keep the toggle button always clickable */
      #__uiToggleBtn { position: fixed; left: 8px; top: 8px; z-index: 999999; width: 34px; height: 34px;
                       border: none; border-radius: 6px; background: rgba(0,0,0,0.6); color: #fff;
                       display: inline-flex; align-items: center; justify-content: center; padding: 0; }
      #__uiToggleBtn:hover { background: rgba(0,0,0,0.75); }
      #__uiToggleBtn svg { width: 20px; height: 20px; }
    `;
    const style = document.createElement("style");
    style.id = "__uiToggleStyle";
    style.textContent = css;
    document.head.appendChild(style);

    // button: inline SVG eye
    const btn = document.createElement("button");
    btn.id = "__uiToggleBtn";
    btn.setAttribute("aria-label", "Toggle UI");
    btn.innerHTML = `
      <svg viewBox="0 0 24 24" aria-hidden="true">
        <path d="M12 5c5.5 0 9.5 3.5 11 7-1.5 3.5-5.5 7-11 7S2.5 15.5 1 12C2.5 8.5 6.5 5 12 5zm0 2C7.9 7 4.6 9.7 3.2 12 4.6 14.3 7.9 17 12 17s7.4-2.7 8.8-5c-1.4-2.3-4.7-5-8.8-5zm0 2.5A2.5 2.5 0 1 1 9.5 12 2.5 2.5 0 0 1 12 9.5zm0-2a4.5 4.5 0 1 1-4.5 4.5A4.5 4.5 0 0 1 12 7.5z" fill="currentColor"/>
      </svg>`;
    btn.addEventListener("click", () => toggleUIVisibilityMode());

    document.body.appendChild(btn);
    toggleUIVisibilityMode.__btn = btn;
    toggleUIVisibilityMode.__init = true;
  }

  // toggle body class
  const off = document.body.classList.toggle("__ui_off");

  // update button appearance (filled vs outlined eye)
  if (toggleUIVisibilityMode.__btn) {
    toggleUIVisibilityMode.__btn.style.background = off ? "rgba(0,0,0,0.85)" : "rgba(0,0,0,0.6)";
    toggleUIVisibilityMode.__btn.title = off ? "Show UI" : "Hide UI";
  }
}
