...
function drawScene() {
  ...
  // 4) Touch point visualization (always on top; non-invasive)
  if (typeof touchViz === "object" &&
      touchViz?.visible &&
      Array.isArray(touchViz.pts) &&
      touchViz.pts.length &&
      typeof drawRingOverlay === "function") {

    // if overlayProgram exists, ensure uniforms are ready for overlay drawing
    if (overlayProgram) {
      gl.useProgram(overlayProgram);
