...
// DEBUG: set to false to silence logs later
// Logs pivot/transform diagnostics when true. Toggle off in production.
const PIVOT_DEBUG = true;
const log = (...args) => { if (PIVOT_DEBUG) console.log("[pivot]", ...args); };
...
// Converts document/FBO pixels → canvas pixels (ignores CSS pan/zoom—WebGL draws in canvas space)
function docToCanvasPx(dx, dy) {
  return {
    x: dx * (canvas.width  / fixedFBOWidth),
    y: dy * (canvas.height / fixedFBOHeight)
  };
}

// Emits a single, consistent snapshot of all pivot-related spaces.
function tracePivot(label) {
  const L = getActiveLayer(); if (!L) { console.warn("[pivot]", label, "no active layer"); return; }
  const pc   = transformTool.pivotCanvas;              // canvas px used by UI ring
  const pDoc = { x: L.px, y: L.py };                   // doc/FBO px used by transform math
  const pCan = docToCanvasPx(pDoc.x, pDoc.y);          // pDoc mapped to canvas px (expected to match 'pc')
  const last = { ...lastPointer };                     // latest pointer (canvas px)
  console.log("[pivot]", label, {
    mode: transformTool.mode,
    lastPointer: last,
    pivotCanvas_UI: pc,
    pivotDoc_math: pDoc,
    pivotDoc_asCanvas: pCan
  });
}
...
// Converts screen (client) coordinates to canvas pixel coordinates.
// Applies inverse pan/rotation/zoom. If clamp=false, returns values outside the document bounds.
// Logs inputs/outputs when PIVOT_DEBUG is enabled.
function screenToCanvasPx(clientX, clientY, clamp = true) {
  const wrap = canvasWrapper.getBoundingClientRect();
  let px = clientX - wrap.left;
  let py = clientY - wrap.top;

  const traceIn = { clientX, clientY, panX, panY, zoomScale, viewRotation, clamp };

  // remove viewport pan
  px -= panX; 
  py -= panY;

  // undo viewport rotation around the transform origin
  ...
  if (!clamp) {
    if (PIVOT_DEBUG) log("screenToCanvasPx(unclamped)", traceIn, "→", { x: cx, y: cy });
    return { x: cx, y: cy };
  }
  ...
  if (PIVOT_DEBUG) log("screenToCanvasPx(clamped)", traceIn, "→", out);
  return out;
}
...
// Maps a pointer-like event to canvas pixels and FBO/document space.
// While a transform is active, coordinates are not clamped to document bounds.
// Logs the mode, clamping behavior, and results when PIVOT_DEBUG is enabled.
function pointerToBoth(eLike) {
  const isTransforming = (transformTool && transformTool.mode && transformTool.mode !== "idle");
  const clamp = !isTransforming; // unclamp while transforming
  const c = screenToCanvasPx(eLike.clientX, eLike.clientY, clamp);
  const f = canvasPxToFBO(c.x, c.y);
  const out = { cx: c.x, cy: c.y, fx: f.fx, fy: f.fy };
  if (PIVOT_DEBUG) log("pointerToBoth", { mode: transformTool?.mode, clamp }, "→", out);
  return out;
}
...
canvasWrapper.addEventListener("mousemove", (e) => {
  const c = screenToCanvasPx(e.clientX, e.clientY, true);
  lastPointer.cx = c.x;
  lastPointer.cy = c.y;
...
// Seeds the exact transform pivot on mouse down using UNCLAMPED canvas coordinates.
// This ensures the first transform frame anchors at the true click position.
// Logs the chosen pivot when PIVOT_DEBUG is enabled.
canvasWrapper.addEventListener("mousedown", (e) => {
  const c = screenToCanvasPx(e.clientX, e.clientY, /*clamp=*/false);
  lastPointer.cx = c.x;
  lastPointer.cy = c.y;
  if (PIVOT_DEBUG) log("mousedown → seed pivot", { clientX: e.clientX, clientY: e.clientY, canvas: c });
}, { passive: true });


// Seeds the exact transform pivot on single-finger touch start (UNCLAMPED).
// Ignores multi-touch to avoid interfering with pinch/rotate gestures.
// Logs the chosen pivot when PIVOT_DEBUG is enabled.
canvasWrapper.addEventListener("touchstart", (e) => {
  if (!e.touches || e.touches.length !== 1) return;
  const t = e.touches[0];
  const c = screenToCanvasPx(t.clientX, t.clientY, /*clamp=*/false);
  lastPointer.cx = c.x;
  lastPointer.cy = c.y;
  if (PIVOT_DEBUG) log("touchstart → seed pivot", { clientX: t.clientX, clientY: t.clientY, canvas: c });
}, { passive: true });
...
// Maps a world/doc point (fx,fy) into the active layer's local space using the layer's
// current translation (x,y), rotation (rotation), and scale (scaleX,scaleY).
function worldToLayer(L, fx, fy) {
  const dx = fx - L.x;
  const dy = fy - L.y;
  const c = Math.cos(L.rotation);
  const s = Math.sin(L.rotation);
  // inverse rotate then inverse scale
  const lx = ( c * dx + s * dy) / (L.scaleX || 1);
  const ly = (-s * dx + c * dy) / (L.scaleY || 1);
  return { lx, ly };
}

// Maps a layer-local point (lx,ly) to world/doc space using the layer's current
// translation (x,y), rotation (rotation), and scale (scaleX,scaleY).
function layerLocalToWorld(L, lx, ly) {
  const sx = lx * (L.scaleX || 1);
  const sy = ly * (L.scaleY || 1);
  const c = Math.cos(L.rotation);
  const s = Math.sin(L.rotation);
  const rx = c * sx - s * sy;
  const ry = s * sx + c * sy;
  return { fx: L.x + rx, fy: L.y + ry };
}

// Initializes a layer transform around a document-space pivot taken from the pointer (for scale/rotate) or the existing pivot (for grab).
function startTransform(mode, withShift = false) {
  ...
  L.px = Number.isFinite(L.px) ? L.px : (fixedFBOWidth  * 0.5);
  L.py = Number.isFinite(L.py) ? L.py : (fixedFBOHeight * 0.5);

  // pointer → canvas px → document/FBO px
  const pcx = (lastPointer && Number.isFinite(lastPointer.cx)) ? lastPointer.cx : canvas.width  * 0.5;
  const pcy = (lastPointer && Number.isFinite(lastPointer.cy)) ? lastPointer.cy : canvas.height * 0.5;
  const pf  = canvasPxToFBO(pcx, pcy); // { fx, fy }

  // choose pivot (document space)
  let pivFx = L.px, pivFy = L.py; // keep current for grab
  if (mode === "scale" || mode === "rotate") {
    const W = fixedFBOWidth  | 0;
    const H = fixedFBOHeight | 0;
    pivFx = Math.max(0, Math.min(W, pf.fx));
    pivFy = Math.max(0, Math.min(H, pf.fy));

    // move the document-space pivot to the pointer without a visual jump:
    // keep the world position of the old local pivot fixed by compensating x/y.
    const c = Math.cos(L.rotation), s = Math.sin(L.rotation);
    const dx0 = (L.px - pivFx) * L.scaleX;
    const dy0 = (L.py - pivFy) * L.scaleY;
    const rx0 =  dx0 * c - dy0 * s;
    const ry0 =  dx0 * s + dy0 * c;
    L.x += rx0;
    L.y += ry0;
    L.px = pivFx;
    L.py = pivFy;
  ...
  transformTool.pivotWorld  = { fx: pivFx, fy: pivFy };   // document-space pivot for math
  transformTool.pivotCanvas = { cx: pcx, cy: pcy };       // canvas-space pivot for UI
  ...
  transformTool.ref = {
    ...
    rotation: L.rotation,
    px: L.px, py: L.py // document-space
  };
  transformTool.start = null;

  if (isDrawing) { isDrawing = false; stopRender("draw"); }
...
// Handles grab/scale/rotate while keeping the document-space pivot fixed at transformTool.pivotWorld.
function onTransformPointerMove(ev) {
  const L = getActiveLayer();
  if (!L || !transformTool || transformTool.mode === "idle") return;

  const ref = transformTool.ref;
  if (!ref) return;

  const toDoc = (x, y) => canvasPxToFBO(x, y); // {fx, fy}
  const cx = ev.clientX, cy = ev.clientY;
  if (!transformTool.start) transformTool.start = { cx, cy };

  const P = transformTool.pivotWorld || { fx: ref.px, fy: ref.py };
  const mode = transformTool.mode;

  // world→local using the REFERENCE pose (prevents drift)
  function worldToLocalRef(fx, fy) {
    const dx = fx - ref.x, dy = fy - ref.y;
    const c = Math.cos(ref.rotation), s = Math.sin(ref.rotation);
    const ix =  dx *  c + dy * s;
    const iy = -dx *  s + dy * c;
    return { lx: ix / (ref.scaleX || 1), ly: iy / (ref.scaleY || 1) };
  }
  // local→world using CURRENT pose
  function localToWorld(lx, ly) {
    const sx = lx * (L.scaleX || 1), sy = ly * (L.scaleY || 1);
    const c = Math.cos(L.rotation), s = Math.sin(L.rotation);
    return { fx: L.x + (sx * c - sy * s), fy: L.y + (sx * s + sy * c) };
  }

  if (mode === "grab") {
    const s = transformTool.start;
    const p0 = toDoc(s.cx, s.cy);
    const p1 = toDoc(cx, cy);
    L.x = ref.x + (p1.fx - p0.fx);
    L.y = ref.y + (p1.fy - p0.fy);
    ...
    const v0x = p0.fx - P.fx, v0y = p0.fy - P.fy;
    const v1x = p1.fx - P.fx, v1y = p1.fy - P.fy;
    const d0 = Math.hypot(v0x, v0y) || 1;
    const d1 = Math.hypot(v1x, v1y) || 1;

    let k = d1 / d0;
    if (transformTool.shiftInvert) k = 1 / k;

    const MIN_S = 0.05, MAX_S = 64;
    const newSX = Math.max(MIN_S, Math.min(MAX_S, ref.scaleX * k));
    const newSY = Math.max(MIN_S, Math.min(MAX_S, ref.scaleY * k));

    const lp = worldToLocalRef(P.fx, P.fy);
    L.scaleX = newSX; L.scaleY = newSY; L.rotation = ref.rotation;

    const wp = localToWorld(lp.lx, lp.ly);
    L.x += (P.fx - wp.fx);
    L.y += (P.fy - wp.fy);
    ...
    const a0 = Math.atan2(p0.fy - P.fy, p0.fx - P.fx);
    const a1 = Math.atan2(p1.fy - P.fy, p1.fx - P.fx);
    let dA = a1 - a0;
    if (dA >  Math.PI) dA -= 2 * Math.PI;
    if (dA < -Math.PI) dA += 2 * Math.PI;
    if (transformTool.shiftInvert) dA = -dA;

    const lp = worldToLocalRef(P.fx, P.fy);
    L.rotation = ref.rotation + dA;
    L.scaleX = ref.scaleX; L.scaleY = ref.scaleY;
...
function startTransform(mode, withShift = false) {
  ...
  // choose pivot
  let pivFx = L.px, pivFy = L.py; // default: keep current for grab
  if (mode === "scale" || mode === "rotate") {
    const W = fixedFBOWidth  | 0;
    const H = fixedFBOHeight | 0;
    pivFx = Math.max(0, Math.min(W, pf.fx));
    pivFy = Math.max(0, Math.min(H, pf.fy));
    // move the document-space pivot to pointer without visible jump:
    // keep the world position of the old local pivot fixed by compensating x/y.
    const c = Math.cos(L.rotation), s = Math.sin(L.rotation);
    const dx0 = (L.px - pivFx) * L.scaleX;
    const dy0 = (L.py - pivFy) * L.scaleY;
  ...
  transformTool.pivotWorld  = { fx: pivFx, fy: pivFy };
  transformTool.pivotCanvas = { cx: pcx, cy: pcy };
  transformTool.mode = mode;
  transformTool.shiftInvert = !!withShift;
  transformTool.ref = {
    x: L.x, y: L.y,
...
// ===== Transform Debug Overlay (multi-color, with axes and numeric logs) =====
// Draws: world pivot (red), local-pivot mapped to world (orange), layer center (green),
// layer origin (cyan), X/Y axes at pivot, and connecting lines. Logs numeric values.

let TRANSFORM_DEBUG = true;

function docToCanvasScale() {
  return { sx: canvas.width / fixedFBOWidth, sy: canvas.height / fixedFBOHeight };
}

function docToCanvasPt(pt) {
  const { sx, sy } = docToCanvasScale();
  return { x: pt.x * sx, y: pt.y * sy };
}

function layerLocalToDoc(L, px, py) {
  const sx = Number.isFinite(L.scaleX) ? L.scaleX : 1;
  const sy = Number.isFinite(L.scaleY) ? L.scaleY : 1;
  const r  = Number.isFinite(L.rotation) ? L.rotation : 0;
  const c  = Math.cos(r), s = Math.sin(r);
  const rx = px * sx, ry = py * sy;
  return { x: L.x + (rx * c - ry * s), y: L.y + (rx * s + ry * c) };
}

function docToLayerLocal(L, fx, fy) {
  const vx = fx - L.x, vy = fy - L.y;
  const r  = Number.isFinite(L.rotation) ? L.rotation : 0;
  const c  = Math.cos(r), s = Math.sin(r);
  const ix =  vx *  c + vy * s;
  const iy = -vx *  s + vy * c;
  const sx = Number.isFinite(L.scaleX) ? L.scaleX : 1;
  const sy = Number.isFinite(L.scaleY) ? L.scaleY : 1;
  return { px: ix / sx, py: iy / sy };
}

function drawCrosshair(cx, cy, sizePx, thicknessPx, rgba) {
  drawDashedLine(cx - sizePx, cy, cx + sizePx, cy, thicknessPx, 1e6, 0, rgba);
  drawDashedLine(cx, cy - sizePx, cx, cy + sizePx, thicknessPx, 1e6, 0, rgba);
}

function drawAxisFromPivot(L, pivotDoc, lenPx) {
  const sx = Number.isFinite(L.scaleX) ? L.scaleX : 1;
  const sy = Number.isFinite(L.scaleY) ? L.scaleY : 1;
  const r  = Number.isFinite(L.rotation) ? L.rotation : 0;
  const c  = Math.cos(r), s = Math.sin(r);

  // unit local axes → world directions
  const xDir = { x: c * sx, y: s * sx };
  const yDir = { x: -s * sy, y:  c * sy };

  // normalize to screen pixels approximately
  const { sx: kx, sy: ky } = docToCanvasScale();
  const toCanvas = (pt) => docToCanvasPt({ x: pt.x, y: pt.y });

  const endX = { x: pivotDoc.x + xDir.x * (lenPx / Math.max(kx, 1e-6)),
                 y: pivotDoc.y + xDir.y * (lenPx / Math.max(ky, 1e-6)) };
  const endY = { x: pivotDoc.x + yDir.x * (lenPx / Math.max(kx, 1e-6)),
                 y: pivotDoc.y + yDir.y * (lenPx / Math.max(ky, 1e-6)) };

  const pC = toCanvas(pivotDoc);
  const xC = toCanvas(endX);
  const yC = toCanvas(endY);

  drawDashedLine(pC.x, pC.y, xC.x, xC.y, 2.0, 10.0, 0.0, [1, 0, 1, 0.9]); // X axis (magenta)
  drawDashedLine(pC.x, pC.y, yC.x, yC.y, 2.0, 10.0, 0.0, [0, 0.4, 1, 0.9]); // Y axis (blue)
}
...
// === drawTransformDebugForActiveLayer (uses ONLY transformTool.localPivot; never L.px/L.py) ===
// Shows: world pivot (red), localPivot mapped to world (orange), layer origin (cyan), layer center (green).
// Draws connectors and axes at pivot for visual verification.
function drawTransformDebugForActiveLayer() {
  if (!window.TRANSFORM_DEBUG) return;
  const L = getActiveLayer && getActiveLayer();
  if (!L || !L.visible) return;
  if (!overlayProgram || typeof drawRingOverlay !== "function" || typeof drawDashedLine !== "function") return;
  ...
  // helpers
  const toCanvas = ({x,y}) => ({ x: x * (canvas.width/fixedFBOWidth), y: y * (canvas.height/fixedFBOHeight) });
  const layerLocalToDoc = (px, py) => {
    const sx = Number.isFinite(L.scaleX) ? L.scaleX : 1;
    const sy = Number.isFinite(L.scaleY) ? L.scaleY : 1;
    const r  = Number.isFinite(L.rotation) ? L.rotation : 0;
    const c  = Math.cos(r), s = Math.sin(r);
  ...
  // use the active gesture pivots; if missing, derive local from pivotDoc via inverse
  const pivotDoc = (transformTool && transformTool.pivotDoc)
    ? { x: transformTool.pivotDoc.fx, y: transformTool.pivotDoc.fy }
    : { x: L.x, y: L.y };

  let localPivot = (transformTool && transformTool.localPivot)
    ? { px: transformTool.localPivot.px, py: transformTool.localPivot.py }
    : (function deriveLocalFromPivotDoc(){
        const vx = pivotDoc.x - L.x, vy = pivotDoc.y - L.y;
        const c = Math.cos(L.rotation), s = Math.sin(L.rotation);
        const ix =  vx *  c + vy * s;
        const iy = -vx *  s + vy * c;
        return { px: ix / (L.scaleX||1), py: iy / (L.scaleY||1) };
      })();

  const localPivotWorld = layerLocalToDoc(localPivot.px, localPivot.py);
  const originWorld     = { x: L.x, y: L.y };

  const centerLocal = {
    x: (Number.isFinite(L.ox) ? L.ox : 0) + (Number.isFinite(L.texW) ? L.texW : fixedFBOWidth)  * 0.5,
    y: (Number.isFinite(L.oy) ? L.oy : 0) + (Number.isFinite(L.texH) ? L.texH : fixedFBOHeight) * 0.5
  };
  const centerWorld = layerLocalToDoc(centerLocal.x, centerLocal.y);

  const P_pivot  = toCanvas(pivotDoc);
  const P_local  = toCanvas(localPivotWorld);
  const P_center = toCanvas(centerWorld);
  const P_orig   = toCanvas(originWorld);

  drawRingOverlay(P_pivot.x,  P_pivot.y,  12, 2.0, [1, 0, 0, 0.95]);   // world pivot (red)
  drawRingOverlay(P_local.x,  P_local.y,   8, 2.0, [1, 0.5, 0, 0.95]); // local→world (orange)
  drawDashedLine(P_pivot.x, P_pivot.y, P_local.x,  P_local.y,  2.0, 8.0, 6.0, [1, 1, 0, 0.9]); // yellow
  drawDashedLine(P_pivot.x, P_pivot.y, P_center.x, P_center.y, 2.0, 8.0, 6.0, [1, 0, 0, 0.8]); // red
  drawDashedLine(P_orig.x,  P_orig.y,  P_center.x, P_center.y, 1.5, 6.0, 4.0, [0, 1, 1, 0.75]); // cyan

  // axes at pivot
  (function drawAxes(lenPx=80){
    const sx = Number.isFinite(L.scaleX) ? L.scaleX : 1;
    const sy = Number.isFinite(L.scaleY) ? L.scaleY : 1;
    const r  = Number.isFinite(L.rotation) ? L.rotation : 0;
    const c  = Math.cos(r), s = Math.sin(r);
    const xDir = { x: c * sx, y: s * sx };
    const yDir = { x: -s * sy, y: c * sy };
    const kx = canvas.width / fixedFBOWidth;
    const ky = canvas.height / fixedFBOHeight;
    const endX = toCanvas({ x: pivotDoc.x + xDir.x * (lenPx/Math.max(kx,1e-6)),
                            y: pivotDoc.y + xDir.y * (lenPx/Math.max(ky,1e-6))});
    const endY = toCanvas({ x: pivotDoc.x + yDir.x * (lenPx/Math.max(kx,1e-6)),
                            y: pivotDoc.y + yDir.y * (lenPx/Math.max(ky,1e-6))});
    drawDashedLine(P_pivot.x, P_pivot.y, endX.x, endX.y, 2.0, 10.0, 0.0, [1, 0, 1, 0.9]); // magenta X
    drawDashedLine(P_pivot.x, P_pivot.y, endY.x, endY.y, 2.0, 10.0, 0.0, [0, 0.4, 1, 0.9]); // blue Y
  })();

  if (typeof PIVOT_DEBUG !== "undefined" && PIVOT_DEBUG) {
    console.log("[pivot-debug]", {
      pose: { x:L.x, y:L.y, sx:L.scaleX, sy:L.scaleY, rot:L.rotation },
      pivotDoc, localPivot, localPivotWorld, originWorld, centerWorld
    });
  }
}
...
const BackgroundPicker = (() => {
  let curated = [
    "/static/draw/images/backgrounds/WEBP_1920/46.webp",
    "/static/draw/images/backgrounds/WEBP_1920/45.webp",
    "/static/draw/images/backgrounds/WEBP_1920/44.webp",
    "/static/draw/images/backgrounds/WEBP_1920/43.webp",
    "/static/draw/images/backgrounds/WEBP_1920/42.webp",
    "/static/draw/images/backgrounds/WEBP_1920/41.webp",
    "/static/draw/images/backgrounds/WEBP_1920/40.webp"
  ];
  ...
  function imageToCanvasCover(img,w,h){
    const c=createCanvas(w,h), ctx=c.getContext("2d");
    const iw=img.naturalWidth||img.width, ih=img.naturalHeight||img.height;
    const arI=iw/ih, arC=w/h;
    let dw,dh,dx,dy;
    if(arI>arC){ dh=h; dw=Math.ceil(h*arI); dx=Math.floor((w-dw)/2); dy=0; }
    else{ dw=w; dh=Math.ceil(w/arI); dx=0; dy=Math.floor((h-dh)/2); }
    ctx.drawImage(img,dx,dy,dw,dh);
    return c;
  }
  function solidToCanvas(color,w,h){ const c=createCanvas(w,h),ctx=c.getContext("2d"); ctx.fillStyle=color; ctx.fillRect(0,0,w,h); return c; }

  function markSelection(btn){
    els.curatedGrid.querySelectorAll(".bgp-thumb.selected").forEach(n=>n.classList.remove("selected"));
    if(btn) btn.classList.add("selected");
  }

  function buildCurated(){
    els.curatedGrid.innerHTML="";
    curated.forEach((url)=>{
      const b=el("button","bgp-thumb"); b.type="button";
      const im=el("img"); im.loading="lazy"; im.src=url; im.alt="background";
      b.appendChild(im);
      b.addEventListener("click", async ()=>{
        const w = fixedFBOWidth || canvas.width;
        const h = fixedFBOHeight || canvas.height;
        const img = await loadImage(url);
        selectedCanvas = imageToCanvasCover(img,w,h);
        markSelection(b);
      });
      els.curatedGrid.appendChild(b);
    });
  }

  function switchTab(name){
    currentTab = name;
    document.querySelectorAll(".bgp-tab").forEach(t=>t.classList.toggle("active",t.dataset.tab===name));
    q("bgPickerModal").querySelectorAll(".bgp-pane").forEach(p=>{
      const pane = p.classList.contains("bgp-pane-curated") ? "curated" :
                   p.classList.contains("bgp-pane-upload")  ? "upload"  : "solid";
      p.classList.toggle("active", pane===name);
    });
  }

  function syncColorHex(){
    els.colorHex.value = els.color.value.toUpperCase();
    // live preview
    const w = fixedFBOWidth || canvas.width;
    const h = fixedFBOHeight || canvas.height;
    const c = solidToCanvas(els.color.value, w, h);
    const p = q("bgpSolidPreview").getContext("2d");
    p.clearRect(0,0,q("bgpSolidPreview").width,q("bgpSolidPreview").height);
    p.drawImage(imageToCanvasCover(c, q("bgpSolidPreview").width, q("bgpSolidPreview").height),0,0);
    selectedCanvas = c;
  }
  ...
  function open(){
    ...
    // seed previews
    syncColorHex();
    q("bgPickerModal").style.display = "flex";
  }

  function close(){
    q("bgPickerModal").style.display = "none";
  }

  function applySelected(){
    if(!selectedCanvas) { close(); return; }
    const img = new Image();
    img.onload = () => {
      currentImage = img;
    ...
    img.src = selectedCanvas.toDataURL("image/png");
  }
  ...
  function onUploadFile(ev){
    ...
    reader.onload = async () => {
      ...
      // draw preview
      const pv = q("bgpUploadPreview");
      const pctx = pv.getContext("2d");
      pctx.clearRect(0,0,pv.width,pv.height);
      pctx.drawImage(imageToCanvasCover(img,pv.width,pv.height),0,0);
    };
    reader.readAsDataURL(file);
  }

  function bind(){
    els.gridBtn   = q("bgpCuratedGrid");
    els.modal     = q("bgPickerModal");
    els.curatedGrid = q("bgpCuratedGrid");
    els.file      = q("bgpFile");
    els.fileName  = q("bgpFileName");
    els.color     = q("bgpColor");
    els.colorHex  = q("bgpColorHex");
    els.apply     = q("bgpApply");
    els.cancel    = q("bgpCancel");
    els.closeX    = q("bgpClose");

    document.querySelectorAll(".bgp-tab").forEach(btn=>{
      btn.addEventListener("click", ()=>switchTab(btn.dataset.tab));
    });

    els.file.addEventListener("change", onUploadFile);
    els.color.addEventListener("input", syncColorHex);
    els.colorHex.addEventListener("input", ()=>{
      let v = els.colorHex.value.trim();
      if(!/^#([0-9a-f]{3}|[0-9a-f]{6}|[0-9a-f]{8})$/i.test(v)) return;
      els.color.value = v;
      syncColorHex();
    });
    ...
    document.getElementById("bgPickerBtn")?.addEventListener("click", open);
  ...
  function install(opts){
    if (opts?.curated && Array.isArray(opts.curated)) curated = opts.curated;
    bind();
  }
...
// draw68.js — call once after DOM is ready (script is already at the end of body)
BackgroundPicker.install({
  curated: [
    "/static/draw/images/backgrounds/WEBP_1920/sl_072622_51930_13.jpg",
    "/static/draw/images/backgrounds/WEBP_1920/02.webp",
    "/static/draw/images/backgrounds/WEBP_1920/44.webp",
    "/static/draw/images/backgrounds/WEBP_1920/43.webp",
    "/static/draw/images/backgrounds/WEBP_1920/42.webp",
    "/static/draw/images/backgrounds/WEBP_1920/41.webp",
...
// Finalizes transform and optionally bakes the result.
function endTransform(bake = true) {
  ...
  let didBake = false, before = null, after = null;
  ...
  if (bake) {
    try {
      before = snapshotLayer?.(activeLayerIndex);
      applyActiveLayerTransform?.();
      after  = snapshotLayer?.(activeLayerIndex);
  ...
  try { transformTool.start = null; transformTool.ref = null; transformTool.shiftInvert = false; transformTool.pivotCanvas = null; } catch {}
  if (transformTool) transformTool.mode = "idle";
  try { isTwoFingerGesture = false; pinchStart = null; } catch {}
...
// Detects whether baking into the fixed document FBO would crop the layer quad (document-space math).
function activeLayerWouldClip() {
  ...
  const tx = Number.isFinite(L.x) ? L.x : 0;
  const ty = Number.isFinite(L.y) ? L.y : 0;
  const px = Number.isFinite(L.px) ? L.px : W * 0.5; // document-space pivot
  const py = Number.isFinite(L.py) ? L.py : H * 0.5;

  const c = Math.cos(rot), s = Math.sin(rot);
  function tf(x, y) {
    const dx = (x - px) * sx, dy = (y - py) * sy;
    return { x: px + dx * c - dy * s + tx, y: py + dx * s + dy * c + ty };
  }
...
// ESC: toggle UI visibility both ways (hide ↔ unhide)
document.addEventListener("keydown", (e) => {
