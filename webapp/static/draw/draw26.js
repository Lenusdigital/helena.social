console.log("Helena Paint - draw26.js")
...
function flattenStrokes() {
  ...
  // 1) create a new texture+FBO to hold the merged result
  ...
  gl.framebufferTexture2D(gl.DRAW_FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, mergedTexture, 0);
  ...
  // 5) keep the legacy aliases in sync so stamping keeps working
  syncActiveAliases();
...
async function loadArtworkObject(art) {
  ...
  if (art.project && art.project.layers && art.project.layers.length) {
    ...
    function drawImageIntoLayerFBO(image, fbo) {
      ...
      gl.useProgram(quadProgram);
      gl.uniform1f(gl.getUniformLocation(quadProgram, "u_flipY"), 1.0);
      gl.uniform2f(gl.getUniformLocation(quadProgram, "u_resolution"), w, h);
      const uLayerOpacity = gl.getUniformLocation(quadProgram, "u_layerOpacity");
      if (uLayerOpacity) gl.uniform1f(uLayerOpacity, 1.0);

      const verts = new Float32Array([0,0,0,0,  w,0,1,0,  0,h,0,1,   0,h,0,1,  w,0,1,0,  w,h,1,1]);
      ...
      gl.enableVertexAttribArray(texLoc); gl.vertexAttribPointer(texLoc, 2, gl.FLOAT, false, 16, 8);
