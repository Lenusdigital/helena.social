...
function mergeSelectedLayers() {
  if (!gl || !layers || !layers.length || !selectedLayerIndices || selectedLayerIndices.size < 2) return;

  const W = fixedFBOWidth  | 0;
  const H = fixedFBOHeight | 0;

  // Collect and sort by visual order (bottom→top)
  const ids = Array.from(selectedLayerIndices).sort((a,b)=>a-b);
  console.log("[mergeSelectedLayers] merging", ids);
  ...
  if (gl.checkFramebufferStatus(gl.FRAMEBUFFER) !== gl.FRAMEBUFFER_COMPLETE) {
    console.warn("[mergeSelectedLayers] FBO incomplete");
    gl.bindFramebuffer(gl.FRAMEBUFFER, null);
  ...
  // Draw only selected layers; others remain untouched
  for (const idx of ids) {
    const L = layers[idx];
    if (!L || !L.texture || L.visible === false) continue;

    const opacity = Math.max(0, Math.min(1, Number(L.opacity) || 1));
    if (opacity === 0) continue;

    const ox = Number.isFinite(L.ox) ? L.ox : 0;
    const oy = Number.isFinite(L.oy) ? L.oy : 0;
    const tw = Math.max(1, (Number(L.texW) || W) | 0);
    const th = Math.max(1, (Number(L.texH) || H) | 0);
    ...
    gl.bufferData(gl.ARRAY_BUFFER, verts, gl.STREAM_DRAW);
    if (aPos >= 0) gl.vertexAttribPointer(aPos, 2, gl.FLOAT, false, 16, 0);
    if (aUV  >= 0) gl.vertexAttribPointer(aUV,  2, gl.FLOAT, false, 16, 8);
  ...
  // Build the merged layer and replace the selection with the result
  const merged = {
    ...
    texture: dstTex,
    fbo: dstFbo,
    texW: W, texH: H, ox: 0, oy: 0,
    x: 0, y: 0, scaleX: 1, scaleY: 1, rotation: 0,
    px: W * 0.5, py: H * 0.5
  };

  // Replace: remove selected indices, insert merged at the highest index spot
  const keep = layers.filter((_, i) => !ids.includes(i));
  const insertAt = Math.max(...ids);
  keep.splice(insertAt, 0, merged);
  // Free GPU of removed layers
  for (const i of ids) {
    const L = layers[i];
    if (!L) continue;
    if (L.texture) gl.deleteTexture(L.texture);
  ...
  console.log("[mergeSelectedLayers] done (NEAREST, 1:1 draw)", { insertAt });
  needsRedraw = true;
  requestDrawIfIdle?.();
}
...
function flattenStrokes() {
  ...
  // Current layer rect in document px (source sub-rect)
  const srcOx = Number.isFinite(L.ox)   ? L.ox   : 0;
  const srcOy = Number.isFinite(L.oy)   ? L.oy   : 0;
  const srcW  = Math.max(1, (Number(L.texW) || DOC_W) | 0);
  const srcH  = Math.max(1, (Number(L.texH) || DOC_H) | 0);

  // Clip to document bounds (so we don't read/write outside)
  let dstOx = Math.max(0, Math.floor(srcOx));
  let dstOy = Math.max(0, Math.floor(srcOy));
  let copyW = Math.max(0, Math.min(srcW, DOC_W - dstOx));
  let copyH = Math.max(0, Math.min(srcH, DOC_H - dstOy));

  // Adjust source offset if clipped on left/top
  let srcX0 = 0, srcY0 = 0;
  if (srcOx < 0) { srcX0 = -srcOx | 0; copyW = Math.max(0, copyW - srcX0); }
  if (srcOy < 0) { srcY0 = -srcOy | 0; copyH = Math.max(0, copyH - srcY0); }
  if (copyW <= 0 || copyH <= 0) return; // nothing to copy
  ...
  try {
    // 1) Create destination texture EXACTLY the size of the layer rect we keep.
    const dstTex = gl.createTexture();
    gl.bindTexture(gl.TEXTURE_2D, dstTex);
    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, copyW, copyH, 0, gl.RGBA, gl.UNSIGNED_BYTE, null);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, gl.CLAMP_TO_EDGE);
    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
    ...
    // 2) Clear target and copy JUST the visible sub-rect (no rescale ⇒ no jump)
    gl.viewport(0, 0, copyW, copyH);
    gl.disable(gl.DEPTH_TEST);
    gl.disable(gl.CULL_FACE);
    gl.disable(gl.SCISSOR_TEST);
    gl.disable(gl.BLEND);
    ...
    if (gl.blitFramebuffer) {
      // WebGL2 path: FBO→FBO blit (pixel-perfect, no filtering)
      gl.bindFramebuffer(gl.READ_FRAMEBUFFER, L.fbo);
      gl.bindFramebuffer(gl.DRAW_FRAMEBUFFER, dstFbo);
      gl.blitFramebuffer(
        srcX0,           srcY0,           srcX0 + copyW, srcY0 + copyH, // from old layer FBO
        0,               0,               copyW,         copyH,         // into (0,0)-(copyW,copyH)
        gl.COLOR_BUFFER_BIT,
        gl.NEAREST
      );
      gl.bindFramebuffer(gl.READ_FRAMEBUFFER, null);
      ...
      // WebGL1 fallback: shader copy with proper UVs for the sub-rect
      gl.useProgram(quadProgram);
      ...
      if (uFlipY)      gl.uniform1f(uFlipY, 1.0);                 // FBO-space copy (no screen flip)
      if (uResolution) gl.uniform2f(uResolution, copyW, copyH);
      if (uOpacity)    gl.uniform1f(uOpacity, 1.0);

      gl.bindFramebuffer(gl.FRAMEBUFFER, dstFbo);
      gl.viewport(0, 0, copyW, copyH);

      // positions in dstFbo space, UVs into sub-rect of the source texture
      const v = new Float32Array([
        0,      0,       srcX0 / srcW,           srcY0 / srcH,
        copyW,  0,      (srcX0 + copyW) / srcW,  srcY0 / srcH,
        0,      copyH,   srcX0 / srcW,          (srcY0 + copyH) / srcH,

        0,      copyH,   srcX0 / srcW,          (srcY0 + copyH) / srcH,
        copyW,  0,      (srcX0 + copyW) / srcW,  srcY0 / srcH,
        copyW,  copyH,  (srcX0 + copyW) / srcW, (srcY0 + copyH) / srcH
      ]);
    ...
    // 3) Swap in compact texture/FBO; keep rect aligned to document (no visual jump)
    const oldTex = L.texture;
    const oldFbo = L.fbo;
    ...
    // Keep the rect in document coords consistent with what we copied
    L.ox   = dstOx;
    L.oy   = dstOy;
    L.texW = copyW;
    L.texH = copyH;
