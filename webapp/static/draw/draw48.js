...
// === REPLACE ENTIRE FUNCTION: mergeSelectedLayers (adds full Undo/Redo support) ===
function mergeSelectedLayers() {
  if (!layers || layers.length < 2) return;

  // Determine which layers to merge: use selection if ≥2, otherwise do nothing
  const indices = (selectedLayerIndices && selectedLayerIndices.size >= 2)
    ? Array.from(selectedLayerIndices).sort((a, b) => a - b)
    : [];
  if (indices.length < 2) return;

  const w = fixedFBOWidth, h = fixedFBOHeight;

  // Capture originals BEFORE merge (metadata + pixel snapshot)
  const removed = indices.map((i) => {
    const L = layers[i];
    return {
      index: i,
      data: {
  ...
  // Create the destination (merged) layer at the smallest index
  ...
  layers.splice(targetIndex, 0, mergedLayer);

  // Composite: draw each original snapshot into destination with standard alpha blending
  ...
  const texLoc = gl.getUniformLocation(quadProgram, "u_texture");
  ...
  // Draw originals from bottom to top (ascending index reflects your UI order)
  for (const i of indices) {
    const info = removed.find(r => r.index === i).data;
    const srcTex = info.snapshot.texture;
    const opacity = Number.isFinite(info.opacity) ? info.opacity : 1;
  ...
  // Remove original layers (adjust indices > target due to the inserted mergedLayer)
  const toRemove = indices.map(i => (i >= targetIndex ? i + 1 : i)).sort((a, b) => b - a);
  ...
  // Record AFTER snapshot of the merged result
  const added = {
    index: targetIndex,
    data: {
      ...
      snapshot: snapshotLayer(targetIndex)
    }
  };
  ...
  History.push({
    type: "merge_layers",
    removed,         // originals (with snapshots)
    added            // merged result (with snapshot)
  });

  // Reset selection to the merged layer
  selectedLayerIndices.clear();
  selectedLayerIndices.add(targetIndex);
...
function mergeAllLayers() {
  try {
    if (!Array.isArray(layers) || layers.length < 2) {
      console.warn("[mergeAllLayers] not enough layers to merge");
      return;
    }
    if (!(selectedLayerIndices instanceof Set)) {
      console.error("[mergeAllLayers] selectedLayerIndices Set missing");
      return;
    }

    // Build full index list (include every layer)
    const allIdx = layers.map((_, i) => i);
    console.debug("[mergeAllLayers] will merge indices", allIdx);

    // Sync the UI selection set to reflect intent (purely informational)
    selectedLayerIndices.clear();
    allIdx.forEach(i => selectedLayerIndices.add(i));
    try { rebuildLayersUI?.(); } catch {}

    // Prefer existing merge routine if available to keep rendering path consistent
    if (typeof mergeSelectedLayers === "function") {
      console.debug("[mergeAllLayers] delegating to mergeSelectedLayers()");
      mergeSelectedLayers();
      return;
    }

    // Fallback path — REQUIRE existing helpers from your codebase:
    //   - snapshotLayer(index)
    //   - createLayerFBO(w,h)
    //   - blitFBOtoFBO(srcFbo, dstFbo, w,h)
    //   - History.push({ type:"merge_layers", removed:[{index,data}], added:{index,data} })
    //   - drawCompositeStackToFBO(indices, targetFBO)  <-- if you have one
    // If none exist, stop here to avoid breaking rendering.
    if (typeof snapshotLayer !== "function" ||
        typeof createLayerFBO !== "function" ||
        typeof blitFBOtoFBO !== "function") {
      console.error("[mergeAllLayers] missing required merge primitives; aborting");
      showStatusMessage?.("Merge All not available on this build", "warning");
      return;
    }

    // Capture originals (for history undo)
    const removed = allIdx.map((i) => {
      const L = layers[i];
      return {
        index: i,
        data: {
    ...
    // Create merged destination layer data
    const mergeAt = layers.length; // append to top; adjust if you want a specific index
    const mergedName = "Merged All";
    const { texture: tex, fbo } = createLayerFBO(fixedFBOWidth, fixedFBOHeight);
    const mergedLayer = {
      id: Date.now() + Math.random(),
      name: mergedName,
      fbo,
    ...
    // Composite all layers into mergedLayer.fbo using your app's renderer
    // Expectation: you have a function to composite the current stack to a given FBO.
    if (typeof compositeLayersIntoFBO === "function") {
      console.debug("[mergeAllLayers] composite via compositeLayersIntoFBO");
      compositeLayersIntoFBO(allIdx, mergedLayer.fbo);
    } else if (typeof renderStackToFBO === "function") {
      console.debug("[mergeAllLayers] composite via renderStackToFBO");
      renderStackToFBO(allIdx, mergedLayer.fbo);
    } else {
      // As a last resort, fall back to the already-rendered scene:
      // bind the main scene FBO (if you have one), otherwise abort safely.
      console.error("[mergeAllLayers] no compositor available; aborting to avoid corruption");
      gl.deleteTexture(mergedLayer.texture);
      gl.deleteFramebuffer(mergedLayer.fbo);
      showStatusMessage?.("Merge All requires compositor; not available in this build", "warning");
      return;
    }

    // Insert merged layer & remove originals (DESC to keep indices valid)
    layers.splice(mergeAt, 0, mergedLayer);
    const toRemove = allIdx.slice().sort((a, b) => b - a);
    ...
    activeLayerIndex = Math.max(0, Math.min(mergeAt - toRemove.filter(i => i < mergeAt).length, layers.length - 1));
    syncActiveAliases?.();
    rebuildLayersUI?.();

    // History entry: merge_layers (matches your History implementation)
    const addedSnap = snapshotLayer(activeLayerIndex);
    History?.push?.({
      type: "merge_layers",
      removed,
      added: { index: activeLayerIndex, data: { name: mergedName, visible: true, opacity: 1, snapshot: addedSnap } }
    });

    console.debug("[mergeAllLayers] done", { activeLayerIndex, totalLayers: layers.length });
    needsRedraw = true;
    requestDrawIfIdle?.();
    showStatusMessage?.("Merged all layers", "success");
  } catch (err) {
    console.error("[mergeAllLayers] error", err);
    showStatusMessage?.("Merge All failed", "warning");
  }
}
...
function rebuildLayersUI() {
  const list = document.getElementById("layersList");
  if (!list) { console.error("[layersUI] #layersList missing"); return; }
  if (!Array.isArray(layers)) { console.error("[layersUI] layers not array"); return; }
  if (typeof activeLayerIndex !== "number") { console.error("[layersUI] activeLayerIndex invalid"); return; }
  if (!(selectedLayerIndices instanceof Set)) { console.error("[layersUI] selectedLayerIndices Set missing"); return; }

  // Keep at least active layer selected
  if (!layers.length) {
    selectedLayerIndices.clear();
  } else if (selectedLayerIndices.size === 0) {
    selectedLayerIndices.add(activeLayerIndex);
  }

  console.debug("[layersUI] rebuild start", {
    count: layers.length,
    activeLayerIndex,
    selected: Array.from(selectedLayerIndices)
  });

  list.innerHTML = "";

  // Header actions: Merge Selected + Merge All
  const header = document.createElement("div");
  header.className = "layers-header";

  const mergeSelBtn = document.createElement("button");
  mergeSelBtn.type = "button";
  mergeSelBtn.className = "bs";
  mergeSelBtn.textContent = selectedLayerIndices.size > 1 ? `Merge Selected (${selectedLayerIndices.size})` : "Merge Selected";
  mergeSelBtn.disabled = selectedLayerIndices.size < 2;
  mergeSelBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    console.debug("[layersUI] Merge Selected click", Array.from(selectedLayerIndices));
    try { mergeSelectedLayers?.(); } catch (err) { console.error("[layersUI] mergeSelectedLayers error", err); }
  });

  const mergeAllBtn = document.createElement("button");
  mergeAllBtn.type = "button";
  mergeAllBtn.className = "bs";
  mergeAllBtn.textContent = "Merge All";
  mergeAllBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    console.debug("[layersUI] Merge All click");
    try { mergeAllLayers?.(); } catch (err) { console.error("[layersUI] mergeAllLayers error", err); }
  });
  ...
  function selectSingle(idx) {
    ...
    console.debug("[layersUI] selectSingle", { idx, selected: Array.from(selectedLayerIndices) });
  ...
  function toggleSelection(idx, additive) {
    if (!additive) {
      ...
      selectedLayerIndices.add(idx);
      activeLayerIndex = idx;
      console.debug("[layersUI] toggleSelection replace", { idx, selected: Array.from(selectedLayerIndices) });
      return;
    }
    ...
    console.debug("[layersUI] toggleSelection additive", { idx, selected: Array.from(selectedLayerIndices) });
  }

  function rangeSelect(toIdx) {
    const anchor = Number.isInteger(activeLayerIndex) ? activeLayerIndex : toIdx;
    ...
    console.debug("[layersUI] rangeSelect", { anchor, toIdx, selected: Array.from(selectedLayerIndices) });
  }
  ...
  (layers || []).forEach((L, idx) => {
    ...
    row.className = "layer-item" + (isActive ? " active" : "") + (isSelected ? " is-selected" : "");
    row.dataset.index = String(idx);

    // checkbox
    const selWrap = document.createElement("div");
    selWrap.className = "layer-select";
    const cb = document.createElement("input");
    cb.type = "checkbox";
    cb.checked = isSelected;
    cb.title = "Select for group ops";
    cb.addEventListener("click", (e) => {
      e.stopPropagation();
      toggleSelection(idx, true);
      row.classList.toggle("is-selected", selectedLayerIndices.has(idx));
      refreshHeader();
      console.debug("[layersUI] checkbox click", { idx, checked: cb.checked, selected: Array.from(selectedLayerIndices) });
    });
    ...
    function refreshEye() {
      eye.src = L.visible ? EYE_OPEN : EYE_CLOSE;
      eye.alt = L.visible ? "Visible" : "Hidden";
    ...
    visBtn.addEventListener("click", (e) => {
      ...
      L.visible = !prev;
      refreshEye();
      console.debug("[layersUI] visibility", { idx, prev, next: L.visible });
      needsRedraw = true;
    });
    ...
    nameInput.addEventListener("focus", () => { activeLayerIndex = idx; });
    const commitName = () => {
      if (nameInput.value !== L.name) {
        const prev = L.name;
        L.name = nameInput.value;
        console.debug("[layersUI] rename", { idx, prev, next: L.name });
      }
    };
    ...
    dupBtn.type = "button"; dupBtn.className = "bs icon-btn";
    dupBtn.innerHTML = `<img src="/static/draw/images/icons/clone.svg" width="18" height="18" style="pointer-events:none" alt="">`;
    dupBtn.title = "Duplicate";
    dupBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      try { duplicateLayer?.(idx); console.debug("[layersUI] duplicate", { idx }); } catch (err) { console.error("[layersUI] duplicate error", err); }
    });

    const upBtn = document.createElement("button");
    upBtn.type = "button"; upBtn.className = "bs icon-btn";
    upBtn.innerHTML = `<img src="/static/draw/images/icons/up-shevron.svg" width="18" height="18" style="pointer-events:none" alt="">`;
    upBtn.title = "Move up";
    upBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      try { moveLayerUp?.(idx); console.debug("[layersUI] move up", { idx }); } catch (err) { console.error("[layersUI] move up error", err); }
    });

    const downBtn = document.createElement("button");
    downBtn.type = "button"; downBtn.className = "bs icon-btn";
    downBtn.innerHTML = `<img src="/static/draw/images/icons/down-shevron.svg" width="18" height="18" style="pointer-events:none" alt="">`;
    downBtn.title = "Move down";
    downBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      try { moveLayerDown?.(idx); console.debug("[layersUI] move down", { idx }); } catch (err) { console.error("[layersUI] move down error", err); }
    });
    ...
    range.type = "range"; range.min = "0"; range.max = "1"; range.step = "0.01"; range.value = String(L.opacity);
    ["click","mousedown","touchstart","keydown"].forEach(ev =>
      range.addEventListener(ev, (e) => e.stopPropagation())
    );
    range.addEventListener("input", () => {
      const prev = L.opacity;
      const v = parseFloat(range.value);
      if (!Number.isNaN(v)) {
        L.opacity = Math.max(0, Math.min(1, v));
        console.debug("[layersUI] opacity", { idx, prev, next: L.opacity });
        needsRedraw = true;
      }
    });
    opacityRow.appendChild(label);
    ...
    row.addEventListener("click", (ev) => {
      ...
      if (ranged) {
        ...
        selectSingle(idx);
        return; // selectSingle refreshes tree
      }
      ...
      refreshHeader();
      console.debug("[layersUI] row click", { idx, activeLayerIndex, selected: Array.from(selectedLayerIndices) });
    });
    ...
    // newest right under header
    list.insertBefore(row, list.children[1] || null);
  });

  refreshHeader();
  console.debug("[layersUI] rebuild done", {
    activeLayerIndex,
    selected: Array.from(selectedLayerIndices)
  });
}
...
/* ===== REPLACE ENTIRE KEYBOARD HANDLER (verbose logging; S/G/R/T/H, arrows) ===== */
document.removeEventListener?.("__hp_transform_keys__", () => {});
document.addEventListener("__hp_transform_keys__", () => {}); // noop tag to allow remove above
document.addEventListener("keydown", (e) => {
  try {
    ...
    if (!L) { console.warn("[keys] no active layer"); return; }

    // Count selected (checkboxes / Set)
    let selectedCount = 1;
    if (typeof selectedLayerIndices !== "undefined" && selectedLayerIndices && selectedLayerIndices.size) {
      selectedCount = selectedLayerIndices.size;
    } else {
      // DOM fallback checkboxes
      try {
        const listEl = document.getElementById("layersList");
        let cnt = 0;
        if (listEl) {
          listEl.querySelectorAll(".layer-item .layer-select input[type='checkbox']").forEach(cb => { if (cb.checked) cnt++; });
        }
        selectedCount = Math.max(1, cnt);
      } catch {}
    }
    ...
    if (lower === "h") {
      e.preventDefault();
      e.stopImmediatePropagation();
      const prev = !!L.visible; L.visible = !prev;
      console.debug("[keys] H toggle visibility", { index: activeLayerIndex, prev, next: L.visible });
      try { rebuildLayersUI?.(); } catch {}
      needsRedraw = true;
      return;
    }
    ...
    if (lower === "g") {
      e.preventDefault();
      e.stopImmediatePropagation();
      console.debug("[keys] G grab pressed", { activeLayerIndex, selectedCount });
      startTransform?.("grab", false);
      return;
    }
    if (lower === "s") {
      e.preventDefault();
      e.stopImmediatePropagation();
      console.debug("[keys] S scale pressed", { activeLayerIndex, selectedCount });
      startTransform?.("scale", false); // unified S (no Shift mode)
      return;
    }
    if (lower === "r") {
      e.preventDefault();
      e.stopImmediatePropagation();
      console.debug("[keys] R rotate pressed", { activeLayerIndex, selectedCount, shift: e.shiftKey });
    ...
    if (k === "Escape" && transformTool?.mode !== "idle") {
      console.debug("[keys] Escape cancel transform");
      endTransform?.(false);
      needsRedraw = true;
      e.preventDefault();
      e.stopImmediatePropagation();
    ...
    if ((k === "Enter" || k === "Return") && transformTool?.mode !== "idle") {
      console.debug("[keys] Enter confirm transform");
      endTransform?.(true);
      needsRedraw = true;
      e.preventDefault();
      e.stopImmediatePropagation();
    ...
    if (lower === "t") {
      ...
      console.debug("[keys] T reset transform", { index: activeLayerIndex });
      needsRedraw = true;
      requestDrawIfIdle?.();
      return;
    }
...
/* ===== REPLACE ENTIRE FUNCTION: startTransform (verbose; multi-selection detection) ===== */
function startTransform(mode, withShift = false) {
  try {
    if (!canvas || !layers) {
      console.error("[startTransform] Missing canvas or layers");
      return;
    }
    const L = (typeof getActiveLayer === "function") ? getActiveLayer() : layers?.[activeLayerIndex];
    if (!L) { console.warn("[startTransform] No active layer at index:", activeLayerIndex); return; }
    ...
    // Pivot from last pointer (fallback to canvas center)
    const pcx = (typeof lastX === "number" && lastX >= 0) ? lastX : (canvas.width  * 0.5);
    const pcy = (typeof lastY === "number" && lastY >= 0) ? lastY : (canvas.height * 0.5);
    const pf  = (typeof canvasPxToFBO === "function")
      ? canvasPxToFBO(pcx, pcy)
      : { fx: (fixedFBOWidth || 0) * 0.5, fy: (fixedFBOHeight || 0) * 0.5 };

    // Persist pivot on layer in document/FBO space
    L.px = Math.max(0, Math.min(fixedFBOWidth,  pf.fx));
    L.py = Math.max(0, Math.min(fixedFBOHeight, pf.fy));

    // Canvas-space pivot for gesture math
    const toCanvasX = canvas.width  / Math.max(1, fixedFBOWidth);
    const toCanvasY = canvas.height / Math.max(1, fixedFBOHeight);
    const pivotCanvas = { cx: L.px * toCanvasX, cy: L.py * toCanvasY };

    // Gather indices: selectedLayerIndices Set → fallback DOM checkboxes → ensure active included
    let indices = [];
    if (typeof selectedLayerIndices !== "undefined" && selectedLayerIndices && selectedLayerIndices.size) {
      indices = Array.from(selectedLayerIndices);
    } else {
      // DOM fallback
      try {
        const listEl = document.getElementById("layersList");
        if (listEl) {
          listEl.querySelectorAll(".layer-item").forEach((row) => {
            ...
            if (cb && cb.checked && idxAttr != null) indices.push(Number(idxAttr));
          });
        }
      } catch (e) {
        console.warn("[startTransform] DOM selection probe failed:", e);
      }
    }
    if (!indices.includes(activeLayerIndex)) indices.push(activeLayerIndex);
    indices = indices
      .filter(i => Number.isInteger(i) && i >= 0 && i < layers.length)
      .filter((v, i, a) => a.indexOf(v) === i);

    transformTool.mode = mode;                       // "grab" | "scale" | "rotate"
    transformTool.shiftInvert = (mode === "rotate") ? !!withShift : false;
    transformTool.pivotCanvas = pivotCanvas;
    ...
    transformTool.multi = null;
    if (indices.length >= 2) {
      transformTool.multi = indices.map((idx) => {
        const Li = layers[idx]; if (!Li) return null;
        Li.x = Number.isFinite(Li.x) ? Li.x : 0;
        Li.y = Number.isFinite(Li.y) ? Li.y : 0;
        Li.scaleX = Number.isFinite(Li.scaleX) ? Li.scaleX : 1;
        Li.scaleY = Number.isFinite(Li.scaleY) ? Li.scaleY : 1;
        Li.rotation = Number.isFinite(Li.rotation) ? Li.rotation : 0;
        Li.px = L.px; Li.py = L.py; // share active layer pivot
        return { index: idx, ref: { x: Li.x, y: Li.y, scaleX: Li.scaleX, scaleY: Li.scaleY, rotation: Li.rotation } };
      }).filter(Boolean);
    }

    const group = (transformTool.multi && transformTool.multi.length > 1);
    console.debug("[startTransform]", {
      mode,
      activeLayerIndex,
      selectedIndices: indices,
      groupCount: group ? transformTool.multi.length : 1,
      pivotCanvas
    });
    ...
    needsRedraw = true;
    requestDrawIfIdle?.();
    showStatusMessage?.(group ? `Transform (${mode}) — ${transformTool.multi.length} layers` : `Transform (${mode})`, "info");
  } catch (err) {
    console.error("[startTransform] error:", err);
    showStatusMessage?.("Transform start failed", "warning");
  }
}
...
function onTransformPointerMove(ev) {
  try {
    if (!transformTool || transformTool.mode === "idle") return;

    const p = (typeof pointerToBoth === "function") ? pointerToBoth(ev) : null;
    if (!p || !Number.isFinite(p.cx) || !Number.isFinite(p.cy)) { console.warn("[transform] invalid pointer", p); return; }

    if (!transformTool.start) {
      transformTool.start = { cx: p.cx, cy: p.cy };
      console.debug("[transform] gesture begin", { mode: transformTool.mode, start: transformTool.start });
    }

    const dxCanvas = p.cx - transformTool.start.cx;
    const dyCanvas = p.cy - transformTool.start.cy;

    const toDocX = Math.max(1, fixedFBOWidth)  / Math.max(1, canvas?.width  || 1);
    const toDocY = Math.max(1, fixedFBOHeight) / Math.max(1, canvas?.height || 1);
    const pc = transformTool.pivotCanvas || { cx: canvas.width * 0.5, cy: canvas.height * 0.5 };

    function applyGrab(Li, ref) {
      Li.x = ref.x + dxCanvas * toDocX;
      Li.y = ref.y + dyCanvas * toDocY;
    }

    function applyScale(Li, ref) {
      // unified S — move away from pivot to scale up, towards to scale down
      const SENS = 0.004, DEAD = 3, MIN_S = 0.05, MAX_S = 20;
      const s0x = transformTool.start.cx - pc.cx;
      const s0y = transformTool.start.cy - pc.cy;
      const sx  = p.cx - pc.cx;
      const sy  = p.cy - pc.cy;
      const d0 = Math.hypot(s0x, s0y);
      const d1 = Math.hypot(sx,  sy);
      let delta = d1 - d0;
      if (Math.abs(delta) <= DEAD) delta = 0; else delta -= Math.sign(delta) * DEAD;
      let f = Math.exp(delta * SENS);
      if (!Number.isFinite(f) || f <= 0) f = 1;
      const clamp = (v) => Math.min(MAX_S, Math.max(MIN_S, v));
      Li.scaleX = clamp(ref.scaleX * f);
      Li.scaleY = clamp(ref.scaleY * f);
    }

    function applyRotate(Li, ref) {
      ...
      Li.rotation = ref.rotation + delta;
    }

    const multi = (transformTool.multi && transformTool.multi.length > 1) ? transformTool.multi : null;

    if (multi) {
      if (transformTool.mode === "grab")   multi.forEach(m => applyGrab  (layers[m.index], m.ref));
      if (transformTool.mode === "scale")  multi.forEach(m => applyScale (layers[m.index], m.ref));
      if (transformTool.mode === "rotate") multi.forEach(m => applyRotate(layers[m.index], m.ref));
    } else {
      const L = (typeof getActiveLayer === "function") ? getActiveLayer() : layers?.[activeLayerIndex];
      if (!L) return;
      if (transformTool.mode === "grab")   applyGrab  (L, transformTool.ref);
      if (transformTool.mode === "scale")  applyScale (L, transformTool.ref);
      if (transformTool.mode === "rotate") applyRotate(L, transformTool.ref);
    }

    needsRedraw = true;
    requestDrawIfIdle?.();
  } catch (err) {
    console.error("[transform] move error", err);
  }
}
...
function endTransform(bake = true) {
  try {
    ...
    console.debug("[transform] end", { bake, mode: transformTool.mode, groupCount: transformTool.multi?.length || 1 });
    ...
    if (bake) {
      ...
      if (multi) {
        const keep = activeLayerIndex;
        multi.forEach((m) => {
          try {
            if (!layers[m.index]) return;
            const before = snapshotLayer(m.index);
            activeLayerIndex = m.index;
            applyActiveLayerTransform?.();
            const after = snapshotLayer(m.index);
            if (before && after) {
              History?.push?.({ type: "transform_bake", before, after });
              console.debug("[transform] baked", m.index);
            }
          } catch (e) {
            console.warn("[transform] bake failed for layer", m.index, e);
          }
        });
        activeLayerIndex = keep;
        syncActiveAliases?.();
      } else {
        try {
          ...
          if (before && after) {
            History?.push?.({ type: "transform_bake", before, after });
            console.debug("[transform] baked single", activeLayerIndex);
          }
        } catch (e) {
          console.warn("[transform] single bake failed", e);
        }
      }
    }
    ...
    showStatusMessage?.(bake ? "Transform applied" : "Transform cancelled", bake ? "success" : "info");
    needsRedraw = true;
    requestDrawIfIdle?.();
  } catch (err) {
    console.error("[transform] end error", err);
    showStatusMessage?.("Transform end failed", "warning");
    try {
      transformTool.start = null;
      transformTool.ref = null;
      transformTool.shiftInvert = false;
...
const History = {
  ...
  _dispose(action) {
    ...
    // merge_layers: dispose all embedded snapshots
    if (action.type === "merge_layers") {
      try {
        if (action.added && action.added.data && action.added.data.snapshot) {
          disposeSnapshot(action.added.data.snapshot);
        }
        if (Array.isArray(action.removed)) {
          for (const r of action.removed) {
            if (r && r.data && r.data.snapshot) disposeSnapshot(r.data.snapshot);
          }
        }
  ...
  _apply(action) {
    switch (action.type) {
      ...
      case "merge_layers": {
        // action.removed: [{index, data:{name,visible,opacity,snapshot}}...]
        // action.added:   {index, data:{name,visible,opacity,snapshot}}
        if (!Array.isArray(action.removed) || !action.added) break;
        // Remove originals in DESC order
  ...
  _applyInverse(action) {
    switch (action.type) {
      ...
      case "merge_layers": {
        // Remove merged result
        if (action.added) this._removeLayerAt(action.added.index);
        // Reinsert originals in ASC order at their original indices
        if (Array.isArray(action.removed)) {
...
document.addEventListener("keydown", (e) => {
  ...
  // Merge: Ctrl/Cmd+E (common in editors)
