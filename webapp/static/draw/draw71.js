console.log("Helena Paint - draw56.js")
...
// === REPLACE: snapshotLayer(layerIndex) ===
// Exact GPU snapshot of the layer (no transforms, no flip, no deletes).
function snapshotLayer(layerIndex) {
  const W = fixedFBOWidth | 0;
  const H = fixedFBOHeight | 0;
  const L = layers && layers[layerIndex];
  if (!gl || !L || !L.fbo || !W || !H) return null;

  // Allocate doc-sized target
  const tex = gl.createTexture();
  gl.bindTexture(gl.TEXTURE_2D, tex);
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, W, H, 0, gl.RGBA, gl.UNSIGNED_BYTE, null);
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
  ...
  // Copy L.fbo → fbo 1:1 (no transform)
  if (gl instanceof WebGL2RenderingContext && gl.blitFramebuffer) {
    ...
    // WebGL1 fallback: draw a full-rect quad sampling L.texture
    const prevFBO = gl.getParameter(gl.FRAMEBUFFER_BINDING);
    const prevProg = gl.getParameter(gl.CURRENT_PROGRAM);
    const prevViewport = gl.getParameter(gl.VIEWPORT);
  ...
  // Return GPU handles (what restore/History needs)
...
// === REPLACE THIS: endTransform(bake = true) ===
// Uses only existing programs. No helpers. No jumps. No cropping.
function endTransform(bake = true) {
  ...
  if (bake) {
    try {
      ...
      applyActiveLayerTransform();                 // << drop-in below
