...
/* ===== REPLACE ENTIRE FUNCTION: onTransformPointerMove (v48 math; pivot fixed) ===== */
function onTransformPointerMove(ev) {
  try {
    ...
    function applyScale(Li, ref) {
      // v48: distance delta from pivot → exponential scale; deadzone; clamp
    ...
    function applyRotate(Li, ref) {
      // v48: pure angular delta around pivot; optional invert on Shift (from startTransform flag)
    ...
    if (multi) {
      multi.forEach(({ index, ref }) => {
        const Li = layers[index];
        if (!Li) return;
        if (transformTool.mode === "grab")   applyGrab(Li, ref);
        if (transformTool.mode === "scale")  applyScale(Li, ref);
        if (transformTool.mode === "rotate") applyRotate(Li, ref);
      });
    } else {
      const L = getActiveLayer(); if (!L || !transformTool.ref) return;
...
/* ===== REPLACE ENTIRE FUNCTION: startTransform (v48-accurate pivot @ pointer, multi-select) ===== */
function startTransform(mode, withShift = false) {
  try {
    ...
    // Pivot from latest cursor position (v48 behavior).
    // If lastX/lastY are null (e.g., keypress before a paint move), fall back to lastPointer, else canvas center.
    let pcx = (typeof lastX === "number") ? lastX :
              (window.lastPointer && Number.isFinite(window.lastPointer.cx)) ? window.lastPointer.cx :
              (canvas.width  * 0.5);
    let pcy = (typeof lastY === "number") ? lastY :
              (window.lastPointer && Number.isFinite(window.lastPointer.cy)) ? window.lastPointer.cy :
              (canvas.height * 0.5);
    ...
    // Persist pivot on the layer in document/FBO space
    L.px = Math.max(0, Math.min(fixedFBOWidth,  pf.fx));
    L.py = Math.max(0, Math.min(fixedFBOHeight, pf.fy));

    // Canvas-space pivot for gesture math (no snapping; v48 math expects this)
    ...
    // Collect selected indices (v48 multi-select path), ensure active included
    let indices = [];
    ...
    if (indices.length >= 2) {
      transformTool.multi = indices.map((idx) => {
        ...
        // share active layer pivot (v48 behavior)
    ...
    requestDrawIfIdle?.();
    showStatusMessage?.(
      (transformTool.multi && transformTool.multi.length > 1)
        ? `Transform (${mode}) — ${transformTool.multi.length} layers`
        : `Transform (${mode})`,
      "info"
    );
  } catch {}
...
/* FINAL Transform: one body, bakes + records history (with off-canvas guard) */
function endTransform(bake = true) {
