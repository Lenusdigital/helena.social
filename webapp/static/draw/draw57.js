...
function mergeAllLayers() {
  try {
    if (!gl) { console.error("[mergeAllLayers] WebGL not available"); return; }
    if (!Array.isArray(layers) || layers.length < 2) {
      console.warn("[mergeAllLayers] not enough layers to merge", { count: layers?.length ?? 0 });
      return;
    }
    ...
    // Destination (merged) layer inserted at bottom (index 0)
    const { texture: dstTex, fbo: dstFbo } = createLayerFBO(w, h);
    const mergedLayer = {
      id: Date.now() + Math.random(),
      name: "Merged All",
    ...
    layers.splice(0, 0, mergedLayer);

    // ---- Compose all originals into destination (bottom→top) ----
    gl.bindFramebuffer(gl.FRAMEBUFFER, dstFbo);
    gl.viewport(0, 0, w, h);
    gl.disable(gl.DEPTH_TEST);
    gl.enable(gl.BLEND);
    ...
    if (uFlipY) gl.uniform1f(uFlipY, 1.0); // offscreen FBO in doc space (no Y flip)
    const uRes   = gl.getUniformLocation(quadProgram, "u_resolution");
    if (uRes) gl.uniform2f(uRes, w, h);
    const uOp    = gl.getUniformLocation(quadProgram, "u_layerOpacity");
    ...
    // Draw originals bottom→top (ascending index reflects UI stacking)
    for (let i = 1; i < removed.length + 1; i++) {
      const src = removed[i - 1].data;
      if (!src?.snapshot?.texture) continue;
      const opacity = Number.isFinite(src.opacity) ? src.opacity : 1;
      if (uOp) gl.uniform1f(uOp, opacity);
      gl.activeTexture(gl.TEXTURE0);
      gl.bindTexture(gl.TEXTURE_2D, src.snapshot.texture);
      gl.drawArrays(gl.TRIANGLES, 0, 6);
    }
    ...
    // Remove original layers (they are now shifted by +1 due to insertion at 0)
    ...
    // AFTER snapshot for merged result
    const added = {
      index: 0,
      data: {
        name: mergedLayer.name,
    ...
    // Selection/UI
    if (!(selectedLayerIndices instanceof Set)) window.selectedLayerIndices = new Set();
    selectedLayerIndices.clear();
    ...
    console.log("[mergeAllLayers] done", { activeLayerIndex, totalLayers: layers.length });
...
function rebuildLayersUI() {
  const list = document.getElementById("layersList");
  if (!list) { console.error("[layersUI] #layersList not found"); return; }
  if (!Array.isArray(layers)) { console.error("[layersUI] layers not ready"); return; }
  if (typeof activeLayerIndex !== "number") { console.error("[layersUI] activeLayerIndex not ready"); return; }
  ...
  console.log("[layersUI] rebuild start", {
  ...
  // Header with Merge Selected + Merge All
  ...
  mergeSelBtn.textContent = selectedLayerIndices.size > 1
  ...
  mergeSelBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    console.log("[layersUI] Merge Selected click", Array.from(selectedLayerIndices));
  ...
  mergeAllBtn.addEventListener("click", (e) => {
    e.stopPropagation();
    console.log("[layersUI] Merge All click");
  ...
  function selectSingle(idx, layer) {
    ...
    console.log("[layersUI] selectSingle", { idx, name: layer?.name, selected: Array.from(selectedLayerIndices) });
    try { syncActiveAliases?.(); } catch {}
    try { rebuildLayersUI?.(); } catch {}
    try { showStatusMessage?.(`Selected: ${layer.name}`, "info"); } catch {}
    needsRedraw = true;
  }

  function setSelectionFromCheckbox(idx, checked) {
    if (checked) {
      selectedLayerIndices.add(idx);
      activeLayerIndex = idx;
    } else {
      if (selectedLayerIndices.size <= 1 && selectedLayerIndices.has(idx)) {
        selectedLayerIndices.add(idx);
        return false;
      }
      selectedLayerIndices.delete(idx);
      if (!selectedLayerIndices.has(activeLayerIndex)) {
        const first = Array.from(selectedLayerIndices)[0];
        if (Number.isInteger(first)) activeLayerIndex = first;
      }
    }
    return true;
  }
  ...
  function rangeSelect(toIdx) {
    ...
    console.log("[layersUI] rangeSelect", { anchor, toIdx, selected: Array.from(selectedLayerIndices) });
  }
  ...
  (layers || []).forEach((L, idx) => {
    ...
    cb.addEventListener("change", (e) => {
      e.stopPropagation();
      const ok = setSelectionFromCheckbox(idx, cb.checked);
      if (!ok) cb.checked = true;
      row.classList.toggle("is-selected", selectedLayerIndices.has(idx));
      refreshHeader();
      console.log("[layersUI] checkbox change", {
        idx,
        checked: cb.checked,
        activeLayerIndex,
        selected: Array.from(selectedLayerIndices)
    ...
    cb.addEventListener("input", (e) => {
      e.stopPropagation();
      console.log("[layersUI] checkbox input", { idx, value: cb.checked });
    });
    cb.addEventListener("click", (e) => {
      e.stopPropagation();
      console.log("[layersUI] checkbox click", { idx });
    });
    ...
    visBtn.addEventListener("click", (e) => {
      ...
      L.visible = !prev;
      refreshEye();
      try { recordLayerVisibilityChange?.(idx, prev, L.visible); } catch {}
      needsRedraw = true;
      console.log("[layersUI] visibility toggle", { idx, prev, next: L.visible });
    });
    ...
    const commitName = () => {
      ...
      if (v !== lastName) {
        ...
        lastName = v;
        L.name = v;
        try { recordLayerRename?.(idx, prev, v); } catch {}
        refreshEye();
        console.log("[layersUI] rename", { idx, prev, next: v });
      }
    };
    ...
    dupBtn.addEventListener("click", (ev) => {
      ev.stopPropagation();
      duplicateLayer?.(idx);
      console.log("[layersUI] duplicate", { idx });
    });
    ...
    upBtn.addEventListener("click", (ev) => {
      ...
      if (to !== idx) { moveLayerUp?.(idx); try { recordLayerMove?.(idx, to); } catch {} }
      console.log("[layersUI] move up", { from: idx, to });
    });
    ...
    downBtn.addEventListener("click", (ev) => {
      ...
      if (to !== idx) { moveLayerDown?.(idx); try { recordLayerMove?.(idx, to); } catch {} }
      console.log("[layersUI] move down", { from: idx, to });
    });
    ...
    let lastOpacity = L.opacity;
    range.addEventListener("input", () => {
      const v = parseFloat(range.value);
      if (!Number.isNaN(v) && v !== L.opacity) {
        L.opacity = Math.max(0, Math.min(1, v));
        needsRedraw = true;
        console.log("[layersUI] opacity input", { idx, value: L.opacity });
      }
    });
    range.addEventListener("change", () => {
      if (L.opacity !== lastOpacity) {
        try { recordLayerOpacityChange?.(idx, lastOpacity, L.opacity); } catch {}
        console.log("[layersUI] opacity change", { idx, prev: lastOpacity, next: L.opacity });
        lastOpacity = L.opacity;
      }
    });
    opacityRow.appendChild(label);
    opacityRow.appendChild(range);

    // Row click selection model (no checkbox)
    row.addEventListener("click", (ev) => {
      ...
      if (ranged) {
        ...
        const willSelect = !selectedLayerIndices.has(idx);
        const ok = setSelectionFromCheckbox(idx, willSelect);
        if (!ok) return;
      } else {
        selectSingle(idx, L);
        return;
      ...
      refreshHeader();
      console.log("[layersUI] row click", { idx, activeLayerIndex, selected: Array.from(selectedLayerIndices) });
    });
  ...
  console.log("[layersUI] rebuild done", {
    activeLayerIndex,
    selected: Array.from(selectedLayerIndices)
  });
}
...
function startTransform(mode, withShift = false) {
  try {
    if (!Array.isArray(layers) || !layers.length) { console.error("[startTransform] no layers"); return; }
    const active = Number.isInteger(activeLayerIndex) ? activeLayerIndex : 0;
    const A = layers[active]; if (!A) { console.error("[startTransform] no active layer"); return; }
    ...
    // selection set
    let indices = (selectedLayerIndices instanceof Set && selectedLayerIndices.size)
      ? Array.from(selectedLayerIndices) : [];
    if (!indices.includes(active)) indices.push(active);
    indices = indices.filter(i => Number.isInteger(i) && layers[i])
                     .filter((v,i,a)=>a.indexOf(v)===i);

    // helper: transform document point by layer transform
    function tfDocPoint(L, x, y) {
      ...
      return { x: px + rx + (L.x || 0), y: py + ry + (L.y || 0) };
    }

    // compute unified group pivot = bbox center of transformed quads
    let minX =  Infinity, minY =  Infinity, maxX = -Infinity, maxY = -Infinity;
    for (const idx of indices) {
      const L = layers[idx]; norm(L);
      const W = fixedFBOWidth, H = fixedFBOHeight;
      const pts = [ tfDocPoint(L,0,0), tfDocPoint(L,W,0), tfDocPoint(L,0,H), tfDocPoint(L,W,H) ];
      for (const p of pts) {
    ...
    if (!isFinite(minX)) { minX = 0; minY = 0; maxX = fixedFBOWidth; maxY = fixedFBOHeight; }
    const pivotDoc = { fx: (minX + maxX) * 0.5, fy: (minY + maxY) * 0.5 };

    // force same pivot on all selected
    for (const idx of indices) {
      layers[idx].px = pivotDoc.fx;
      layers[idx].py = pivotDoc.fy;
    }

    // canvas pivot for pointer deltas
    const toCanvasX = canvas.width  / Math.max(1, fixedFBOWidth);
    const toCanvasY = canvas.height / Math.max(1, fixedFBOHeight);
    const pivotCanvas = { cx: pivotDoc.fx * toCanvasX, cy: pivotDoc.fy * toCanvasY };

    transformTool.mode = mode;
    transformTool.shiftInvert = (mode === "rotate") ? !!withShift : false; // Shift only affects rotate
    transformTool.pivotCanvas = pivotCanvas;
    transformTool.start = null;
    transformTool.ref = { x: A.x, y: A.y, scaleX: A.scaleX, scaleY: A.scaleY, rotation: A.rotation, px: A.px, py: A.py };
    transformTool.multi = indices.map(i => ({ index: i, ref: { x: layers[i].x, y: layers[i].y, scaleX: layers[i].scaleX, scaleY: layers[i].scaleY, rotation: layers[i].rotation } }));

    if (typeof isDrawing !== "undefined" && isDrawing) { isDrawing = false; try { stopRender?.("draw"); } catch {} }
    ...
    console.log("[startTransform]", {
      ...
      pivotCanvas,
      activeLayerIndex,
      group: transformTool.multi.map(m => m.index),
      shiftInvert: transformTool.shiftInvert
    });

    showStatusMessage?.(
      transformTool.multi.length > 1
        ? `Transform (${mode}) — ${transformTool.multi.length} layers (group pivot)`
        : `Transform (${mode})`,
      "info"
    );
...
function onTransformPointerMove(ev) {
  try {
    ...
    if (!transformTool.start) {
      ...
      console.log("[transform] gesture start", { mode: transformTool.mode, start: transformTool.start });
    }

    const dxCanvas = p.cx - transformTool.start.cx;
    const dyCanvas = p.cy - transformTool.start.cy;

    const toDocX = Math.max(1, fixedFBOWidth)  / Math.max(1, canvas.width);
    const toDocY = Math.max(1, fixedFBOHeight) / Math.max(1, canvas.height);
    const pc = transformTool.pivotCanvas || { cx: canvas.width * 0.5, cy: canvas.height * 0.5 };
    ...
    function applyScale(Li, ref) {
      const SENS = 0.004, DEAD = 3.0, MIN_S = 0.05, MAX_S = 20;
      ...
      let delta = d1 - d0;
      if (Math.abs(delta) <= DEAD) delta = 0;
      else delta -= Math.sign(delta) * DEAD;
    ...
    const multi = (transformTool.multi && transformTool.multi.length > 0) ? transformTool.multi : null;
    ...
    console.error("[onTransformPointerMove] error", err);
  }
}
...
function endTransform(bake = true) {
  try {
    ...
    if (bake) {
      const group = (transformTool.multi && transformTool.multi.length > 0) ? transformTool.multi : null;
      if (group && group.length > 1) {
        console.log("[endTransform] baking group", group.map(m => m.index));
        const keep = activeLayerIndex;
        for (const m of group) {
          if (!layers[m.index]) continue;
          try {
            const before = snapshotLayer(m.index);
            activeLayerIndex = m.index;
            applyActiveLayerTransform?.();
            const after = snapshotLayer(m.index);
            if (before && after) History?.push?.({ type: "transform_bake", before, after });
          } catch (e) {
            console.warn("[endTransform] bake failed", m.index, e);
          }
        }
        ...
        try {
          ...
          console.warn("[endTransform] single bake failed", e);
        }
      }
    }
    ...
    console.error("[endTransform] error", err);
