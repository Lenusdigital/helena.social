console.log("Helena Paint - draw29.js")
...
let rafId = null;
let idleFrames = 0;
const activeReasons = new Set(); // e.g. "draw", "transform", "preview"
...
function stopRender(reason) {
  ...
  // don’t cancel raf immediately; the tick will naturally stop after idle
}

function tick() {
  rafId = null;

  const isActive = activeReasons.size > 0 || needsRedraw === true;
  ...
  // keep the loop alive while active, or for a couple “cool-down” frames
  if (isActive || idleFrames < 2) {
    rafId = requestAnimationFrame(tick);
  } else {
    // fully stop
    idleFrames = 0;
...
["mouseup","mouseleave","touchend","touchcancel"].forEach(ev =>
  window.addEventListener(ev, endTransform, { passive:true })
);
...
canvasWrapper.addEventListener("touchstart", (e) => {
    if (e.touches.length === 2) {
        ...
        // prevent drawing on single touch that follows a 2-finger pinch
        e.preventDefault();
    }
}, { passive: false });
...
function endTransform(bake = true) {
  const wasActive = (transformTool.mode !== "idle");

  if (bake) {
    const L = getActiveLayer();
    if (L) {
      // This flattens the current x/y/scale/rotation into the texture and
      // resets the layer’s transform to identity.
      applyActiveLayerTransform();
      // Make sure any in-flight stroke deltas don’t reuse old coords.
      resetStrokeState?.();
    }
  }
...
// Converts a client pointer (page) → canvas bitmap px by undoing CSS pan/zoom.
...
// Canvas px → document/FBO px (top-left origin).
...
// Combined helper from an event-like (MouseEvent or Touch) → both spaces.
...
canvas.addEventListener("mousedown", (ev) => {
  ...
  isDrawing = true;
  startRender("draw");                 // keep RAF loop running while drawing
  drawBrushStrokeToPaintLayer(p.cx, p.cy);
});
...
canvas.addEventListener("mousemove", (ev) => {
  ...
  // update angle from last position
  if (lastX !== null && lastY !== null) {
  ...
  // update brush overlay position (0..1)
  overlayPosition = [p.cx / canvas.width, p.cy / canvas.height];

  if (isDrawing) {
    // draw stroke
    drawBrushStrokeToPaintLayer(p.cx, p.cy);
  } else if (transformTool.mode === "idle") {
    // ← this is the “brush preview” part
    needsRedraw = true;
    startRender("preview");            // keep RAF loop alive while hovering
  }
});
...
canvas.addEventListener("mouseleave", () => {
  isDrawing = false;
  stopRender("draw");
  stopRender("preview");               // stop the hover preview when the mouse leaves
  lastFx = null; lastFy = null;
});
...
canvas.addEventListener("touchstart", (ev) => {
  ev.preventDefault();
  if (isTwoFingerGesture) return;      // ignore while pinching
...
canvas.addEventListener("touchmove", (ev) => {
  ...
  if (isDrawing) {
    ...
    // hover-style preview for touch (when not drawing)
    needsRedraw = true;
    startRender("preview");
  }
}, { passive: false });
...
function drawScene() {
  ...
  // FBO→screen scale so doc/FBO px map to screen px
  const fboToScreenX = canvas.width  / Math.max(1, fixedFBOWidth  || canvas.width);
  const fboToScreenY = canvas.height / Math.max(1, fixedFBOHeight || canvas.height);

  // ——— helpers ———
  function makeQuadVerticesScreen(x, y, w, h, rotRad) {
    const cx = x + w * 0.5;
    const cy = y + h * 0.5;

    const hw = w * 0.5, hh = h * 0.5;
    const c = Math.cos(rotRad || 0);
    const s = Math.sin(rotRad || 0);

    // corners around center → rotate → translate
    const pts = [
      { x: -hw, y: -hh }, // TL
      { x:  hw, y: -hh }, // TR
      { x: -hw, y:  hh }, // BL
  ...
  function drawTexturedQuadWithVertices(program, tex, vertices, layerOpacity) {
    ...
    // common uniforms most quads expect
    const uFlipY = gl.getUniformLocation(program, "u_flipY");
    if (uFlipY) gl.uniform1f(uFlipY, -1.0);
  ...
  // ——— 1) background image (fills canvas, no per-layer transform) ———
  ...
  // ——— 2) paint layers (with opacity + transform) ———
    // ——— 2) paint layers (with opacity + transform around pivot) ———
    gl.enable(gl.BLEND);
    gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);
  ...
  // ——— 3) brush overlay ———
  if (overlayProgram && overlayTexture && gl.isTexture(overlayTexture) && typeof drawBrushOverlay === "function") {
    gl.useProgram(overlayProgram);
