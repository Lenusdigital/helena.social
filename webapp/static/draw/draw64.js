...
/* applySnapshotToLayer(layerIndex, snapshot)
   Restores a CPU snapshot BACK into the same layer’s texture, in-place and without any draw.
   Critical details preventing intermittent first-undo Y-flip:
   - Forces UNPACK_FLIP_Y_WEBGL = 0 and UNPACK_ALIGNMENT = 1 for this upload path.
   - Uses texSubImage2D when size matches; otherwise texImage2D + reattach to stable FBO.
   - Leaves WebGL object identities (layer.texture / layer.fbo) stable.
   - Normalizes rect/transform to a neutral pose so future brush sampling aligns.
   Swap this entire function body in place of the existing applySnapshotToLayer. */
function applySnapshotToLayer(layerIndex, snapshot) {
  ...
  // Ensure texture exists and is bound to the layer; keep handles stable.
  if (!L.texture) {
  ...
  // Hard reset pixel-store flags so prior uploads (e.g., brush/overlay loads) can’t leak state.
  if (gl.UNPACK_FLIP_Y_WEBGL !== undefined) gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, 0);
  gl.pixelStorei(gl.UNPACK_ALIGNMENT, 1);

  // Upload pixels back into the same texture space the brush pipeline uses.
  const sizeChanged = ((L.texW | 0) !== W) || ((L.texH | 0) !== H);
  if (sizeChanged) {
  ...
  // Normalize geometry so subsequent strokes preview/render correctly.
  L.ox = 0; L.oy = 0;
  L.x = 0;  L.y = 0;
...
function applyLayerTransformState(index, state) {
  ...
  L.px = state.px; L.py = state.py;
  L.ox = state.ox; L.oy = state.oy;
  L.texW = state.texW; L.texH = state.texH;
...
/* History (full replacement)
   Purpose: unified, robust undo/redo for strokes (pixel snapshots) and transforms (parametric state).
   Notes:
   - Strokes store immutable CPU snapshots from the layer FBO (no GL objects inside actions).
   - Transforms store parametric state only (x/y/scale/rotation/pivot/opacity/visibility).
   - Undo/redo uses applySnapshotToLayer for pixels and applyLayerTransformState for transforms.
   - Insert/remove/move/merge/clear are handled without leaking GL resources.
*/
const History = {
  /* stacks */
  stack: [],
  ...
  /* push(action)
     Adds an action to the undo stack, evicts oldest beyond limit, clears redo. */
  push(action) {
  ...
  /* undo()
     Applies inverse of the last action, moves it to redo. Forces a visible redraw. */
  undo() {
  ...
  /* redoDo()
     Reapplies an action from redo, moves it back to undo. Forces a visible redraw. */
  redoDo() {
  ...
  /* _dispose(action)
     Releases CPU snapshots embedded in an evicted action. */
  _dispose(action) {
    if (!action) return;
    // CPU snapshots are plain Uint8Array; nothing to delete on GL side.
    // Keep for symmetry; older code paths may attach extra fields.
    if (action.before && action.before.pixels) action.before.pixels = null;
    if (action.after  && action.after.pixels)  action.after.pixels  = null;

    if (action.removedLayer?.snapshot?.pixels) action.removedLayer.snapshot.pixels = null;
    if (action.addedLayer?.snapshot?.pixels)   action.addedLayer.snapshot.pixels   = null;

    if (action.type === "merge_layers") {
      if (action.added?.data?.snapshot?.pixels) action.added.data.snapshot.pixels = null;
      if (Array.isArray(action.removed)) {
        for (const r of action.removed) {
          if (r?.data?.snapshot?.pixels) r.data.snapshot.pixels = null;
        }
      }
    }
  },
  ...
  /* _apply(action)
     Redo side: applies “after” snapshots or next parametric state. */
  _apply(action) {
    switch (action.type) {
      /* pixel actions */
      case "stroke":
      ...
      case "merge_layers": {
        ...
        syncActiveAliases?.(); rebuildLayersUI?.();
        break;
      }
  ...
  /* _applyInverse(action)
     Undo side: applies “before” snapshots or previous parametric state. */
  _applyInverse(action) {
    switch (action.type) {
      /* pixel actions */
      case "stroke":
  ...
  /* _moveLayer(from, to)
     Reorders layers array and updates active index + UI. */
  _moveLayer(from, to) {
    if (from === to) return;
  ...
  /* _removeLayerAt(index)
     Deletes a layer’s GL resources and removes it from the list. */
  _removeLayerAt(index) {
    const L = layers[index];
    if (!L) return;
    try { gl.deleteTexture(L.texture); } catch {}
  ...
  /* _insertLayer(index, data)
     Inserts a layer and restores its snapshot if provided. */
  _insertLayer(index, data) {
    const layer = {
      id: Date.now() + Math.random(),
      ...
      // geometry/transform defaults (kept neutral — snapshots restore pixels only)
    ...
    // Restore pixels if provided, else allocate empty texture
    if (data.snapshot && data.snapshot.pixels) {
      ...
      layer.texW = fixedFBOWidth; layer.texH = fixedFBOHeight;
    }
  ...
  /* _captureProject()
     Captures the entire project into CPU snapshots and metadata (no GL handles). */
  _captureProject() {
    return {
      w: fixedFBOWidth,
      h: fixedFBOHeight,
  ...
  /* _restoreProject(all)
     Rebuilds the project from CPU snapshots; replaces layers array and UI. */
  _restoreProject(all) {
    if (!all) return;
    try {
      // Dispose current
      ...
      // Recreate layers from snapshots
