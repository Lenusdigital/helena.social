console.log("Helena Paint - gemini1.js")
...
// === REPLACE ENTIRE FUNCTION: onTransformPointerMove ===
function onTransformPointerMove(ev) {
    ...
    // The start point must be set by startTransform. If it's missing, log an error and bail.
    if (!transformTool.start) {
        console.error("[transform] onTransformPointerMove called without a start point. Gesture was not initialized correctly. Bailing out.");
        return;
    }
...
// === REPLACE ENTIRE FUNCTION: startTransform ===
function startTransform(mode, withShift = false) {
    const L = getActiveLayer();
    if (!L) {
        console.warn("[transform] startTransform called with no active layer.");
        return;
    }

    // Sanitize current transform values
    ...
    // Get the current pointer position in canvas pixels, which becomes our interaction point.
    const pcx = (lastPointer.cx != null) ? lastPointer.cx : canvas.width * 0.5;
    const pcy = (lastPointer.cy != null) ? lastPointer.cy : canvas.height * 0.5;
    const pf = canvasPxToFBO(pcx, pcy);

    // For scale and rotate, we want the pivot to be under the cursor.
    // If we change the pivot, we must compensate L.x and L.y to prevent a visual jump.
    if (mode === 'scale' || mode === 'rotate') {
        ...
        const delta_px = p_new_x - p_old_x;
        const delta_py = p_new_y - p_old_y;

        // Only compensate if the pivot actually moved.
        if (Math.abs(delta_px) > 1e-6 || Math.abs(delta_py) > 1e-6) {
            const sX = L.scaleX;
            const sY = L.scaleY;
            const c = Math.cos(L.rotation);
            const s = Math.sin(L.rotation);

            // Corrected formula to calculate the visual jump caused by moving the pivot.
            const jumpX = delta_px * (sX * c - 1) - delta_py * sY * s;
            const jumpY = delta_px * sX * s + delta_py * (sY * c - 1);
            
            // Apply the inverse of the jump to the layer's translation.
            L.x -= jumpX;
            L.y -= jumpY;
        }
    }

    // Now, officially set the new pivot for the layer for scale/rotate modes.
    // For 'grab', the pivot remains unchanged.
    if (mode === 'scale' || mode === 'rotate') {
        L.px = pf.fx;
        L.py = pf.fy;
    }
    ...
    // Stash the pivot in canvas pixels for gesture calculations in onTransformPointerMove.
    ...
    // Set up the tool state for the gesture.
    transformTool.mode = mode;
    transformTool.shiftInvert = withShift;
    
    // Save the reference state *after* compensation.
    transformTool.ref = {
        x: L.x,
        y: L.y,
        scaleX: L.scaleX,
    ...
    // Set the gesture's start coordinate.
    transformTool.start = { cx: pcx, cy: pcy };

    // --- Stop any in-progress painting ---
    if (isDrawing) {
        console.log("[transform] Halting active drawing to start transform.");
        isDrawing = false;
        stopRender("draw");
    }
