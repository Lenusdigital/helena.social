console.log("Helena Paint - draw63.js")
...
// ============================================================================
// screenToCanvasPx (v60)
// Maps screen → canvas pixels, removing pan/zoom/viewRotation, then CLAMPS
// to the canvas size. This is what the transform code in v60 was built for.
// ============================================================================
...
// ============================================================================
// pointerToBoth(eLike)
// Returns pointer both in canvas px (unbounded) and document/FBO px.
// Keeps values unbounded so transforms can use true off-canvas pivots.
// ============================================================================
function pointerToBoth(eLike) {
  const c = screenToCanvasPx(eLike.clientX, eLike.clientY); // UNBOUNDED
  // Map to document/FBO space (also unbounded; clamp later only where needed)
  const fx = c.x * (fixedFBOWidth  / canvas.width);
  const fy = c.y * (fixedFBOHeight / canvas.height);
  return { cx: c.x, cy: c.y, fx, fy };
}
...
// // === Add near other DOM listeners (after the panel exists) ===
// (function wireLayerToBrushButton(){
//   const btn = document.getElementById("layerToBrushBtn");
//   if (!btn) return;
//   btn.addEventListener("click", () => {
//     const brush = makeBrushFromActiveLayer();
//     if (brush) {
//       // optional UI refresh hooks if present in your app:
//       rebuildBrushList?.();
//       setCurrentBrushById?.(brush.id);
//       showStatusMessage?.(`Brush created from layer: ${brush.name}`, "success");
//     } else {
//       showStatusMessage?.("No active layer to convert.", "warning");
//     }
//   });
// })();

// Wire "To Brush" panel button (same scope as Add/Del/Imp/Exp)
...
// ============================================================================
// onTransformPointerMove (v60)
// Uses canvas-space deltas for grab, and canvas-space distances/angles around
// transformTool.pivotCanvas for scale/rotate. This matches draw60 behavior.
// ============================================================================
function onTransformPointerMove(ev) {
  ...
  if (transformTool.mode === "grab") {
    // canvas delta → document translation
    ...
    // distances from pivot at start vs current (canvas space)
    ...
    L.scaleX = clampScale(baseX * f);
    L.scaleY = clampScale(baseY * f);
...
// ============================================================================
// startTransform (v60)
// Captures pivot from pointer, stores pivot in doc space AND a canvas-space
// copy for gesture math. This pairing is what kept pivot stable in v60.
// ============================================================================
function startTransform(mode, withShift=false) {
  ...
  // stop any in-progress painting before entering transform
  if (isDrawing) {
    isDrawing = false;
    stopRender("draw");
  }
...
// ============================================================================
// Overlay (WebGL): pivot ring draw (uses existing ringProgram and ringFS)
// Purpose:
//   - FIX center uniform to EXACT canvas-px pivot derived from (L.px,L.py).
//   - Avoid using stale transformTool.pivotCanvas; recompute per-frame.
//   - Works regardless of layer position/scale/rotation or clones.
// Drop-in: replace your existing function that binds ringProgram and sets
//          u_center/u_radius/u_thickness/u_color/u_viewport (keep name).
//          If you don’t have one, use this as `drawPivotOverlay()` and call
//          it in your overlay pass after drawing layers.
// ============================================================================
function drawPivotOverlay() {
  ...
  // recompute canvas-px pivot directly from document pivot
  const Wdoc = Math.max(1, fixedFBOWidth  | 0);
  const Hdoc = Math.max(1, fixedFBOHeight | 0);
  const cx   = (Number.isFinite(L.px) ? L.px : Wdoc * 0.5) * (canvas.width  / Wdoc);
  const cy   = (Number.isFinite(L.py) ? L.py : Hdoc * 0.5) * (canvas.height / Hdoc);

  const radiusPx    = 12.0;
  const thicknessPx = 2.0;
  ...
  // Fullscreen pass for ringFS (uses gl_FragCoord)
  gl.bindFramebuffer(gl.FRAMEBUFFER, null);
  ...
  gl.useProgram(ringProgram);
  gl.uniform2f(gl.getUniformLocation(ringProgram, "u_center"),   cx, cy);
  gl.uniform1f(gl.getUniformLocation(ringProgram, "u_radius"),   radiusPx);
  gl.uniform1f(gl.getUniformLocation(ringProgram, "u_thickness"), thicknessPx);
  gl.uniform4f(gl.getUniformLocation(ringProgram, "u_color"),    1.0, 0.95, 0.1, 0.95);
  gl.uniform2f(gl.getUniformLocation(ringProgram, "u_viewport"), canvas.width, canvas.height);

  // draw a single screen-filling triangle (no VAO assumed; create/discard VBO here)
  const verts = new Float32Array([
    -1, -1,
     3, -1,
    -1,  3
  ...
  gl.bindBuffer(gl.ARRAY_BUFFER, vbo);
  gl.bufferData(gl.ARRAY_BUFFER, verts, gl.STREAM_DRAW);
  const aPos = gl.getAttribLocation(ringProgram, "a_position");
  if (aPos >= 0) { gl.enableVertexAttribArray(aPos); gl.vertexAttribPointer(aPos, 2, gl.FLOAT, false, 8, 0); }
...
// ============================================================================
// startTransform(mode, withShift=false)
// Captures pivot exactly under the pointer in DOCUMENT/FBO px (unbounded),
// primes reference params for drift-free math, and sets HUD pivot once.
// ============================================================================
function startTransform(mode, withShift = false) {
  const L = getActiveLayer();
  if (!L) { console.warn("[transform] startTransform: no active layer"); return; }
  ...
  // last pointer in canvas px (fallback: canvas center)
  const pcx = (lastX != null) ? lastX : canvas.width  * 0.5;
  const pcy = (lastY != null) ? lastY : canvas.height * 0.5;

  // map pivot → DOCUMENT/FBO px (UNBOUNDED: do not clamp!)
  const pDoc = canvasPxToFBO(pcx, pcy); // { fx, fy }
  L.px = pDoc.fx;
  L.py = pDoc.fy;

  // HUD pivot (canvas px) derived from L.px/L.py (no drift)
  const Wdoc = Math.max(1, fixedFBOWidth  | 0);
  const Hdoc = Math.max(1, fixedFBOHeight | 0);
  transformTool.pivotCanvas = {
    cx: (L.px * canvas.width ) / Wdoc,
    cy: (L.py * canvas.height) / Hdoc
  };
  ...
  // anchors at first pointer position in BOTH spaces
  transformTool.start    = { cx: pcx,    cy: pcy };
  transformTool.startDoc = { fx: pDoc.fx, fy: pDoc.fy };

  // history “before” for non-baked param change
  try { transformTool.paramsBefore = snapshotLayerParams(activeLayerIndex); }
  catch { transformTool.paramsBefore = null; }

  // stop painting if needed, enter transform render mode
  if (isDrawing) { isDrawing = false; try { stopRender("draw"); } catch {} }
...
function endTransform(bake = true) {
  ...
  // 1) Auto-promote if user upscaled a lot → avoid repeated resample loss.
  //    (Safe no-op if scale is modest)
  try { promoteLayerForScale?.(1.25); } catch {}

  // 2) Guard: if baking would crop (layer quad extends beyond the document AND
  //    your bake path would write into a fixed doc FBO), skip baking to prevent loss.
  try {
    if (bake && activeLayerWouldClip()) {
      bake = false;
      showStatusMessage?.("Skipped bake to prevent off-canvas cropping", "info");
    }
  } catch (e) {
    console.warn("[endTransform] clip-guard error:", e);
  }

  if (bake) {
    try {
      before = snapshotLayer?.(activeLayerIndex);   // BEFORE
      applyActiveLayerTransform?.();                // bake pixels (your non-cropping version)
      after  = snapshotLayer?.(activeLayerIndex);   // AFTER
      if (before && after && History?.push) {
...
// === REPLACE: mergeSelectedLayers() ===
// Merge selected layers into a single layer sized to the UNION bbox (layers may sit outside document).
function mergeSelectedLayers() {
  if (!gl || !layers || !selectedLayerIndices || selectedLayerIndices.size < 2 || !quadProgram) return;

  const Wdoc = fixedFBOWidth | 0, Hdoc = fixedFBOHeight | 0;
  const sel = Array.from(selectedLayerIndices).sort((a,b)=>a-b);

  // Compute union bbox in document space using each layer's current transform
  let minX =  Infinity, minY =  Infinity, maxX = -Infinity, maxY = -Infinity;

  function addLayerBounds(L) {
    const Lx = Number.isFinite(L.ox)?L.ox:0, Ly = Number.isFinite(L.oy)?L.oy:0;
    const Lw = Math.max(1, Number.isFinite(L.texW)?L.texW:Wdoc);
    const Lh = Math.max(1, Number.isFinite(L.texH)?L.texH:Hdoc);
    const scx= Number.isFinite(L.scaleX)?L.scaleX:1, scy= Number.isFinite(L.scaleY)?L.scaleY:1;
    const rot= Number.isFinite(L.rotation)?L.rotation:0;
    const tx = Number.isFinite(L.x)?L.x:0, ty = Number.isFinite(L.y)?L.y:0;
    const pivX=Number.isFinite(L.px)?L.px:Wdoc*0.5, pivY=Number.isFinite(L.py)?L.py:Hdoc*0.5;
    const c=Math.cos(rot), s=Math.sin(rot);
    function tf(x,y){let dx=(x-pivX)*scx, dy=(y-pivY)*scy; const rx=dx*c-dy*s, ry=dx*s+dy*c; return {x:pivX+rx+tx,y:pivY+ry+ty};}
    const p0=tf(Lx,Ly), p1=tf(Lx+Lw,Ly), p2=tf(Lx,Ly+Lh), p3=tf(Lx+Lw,Ly+Lh);
    minX=Math.min(minX,p0.x,p1.x,p2.x,p3.x); minY=Math.min(minY,p0.y,p1.y,p2.y,p3.y);
    maxX=Math.max(maxX,p0.x,p1.x,p2.x,p3.x); maxY=Math.max(maxY,p0.y,p1.y,p2.y,p3.y);
  }

  sel.forEach(i => layers[i] && addLayerBounds(layers[i]));
  minX = Math.floor(minX); minY = Math.floor(minY);
  maxX = Math.ceil(maxX);  maxY = Math.ceil(maxY);
  const outW = Math.max(1, (maxX - minX) | 0);
  const outH = Math.max(1, (maxY - minY) | 0);

  // Setup target FBO of union bbox
  const prevFBO = gl.getParameter(gl.FRAMEBUFFER_BINDING);
  const prevProg = gl.getParameter(gl.CURRENT_PROGRAM);
  const prevViewport = gl.getParameter(gl.VIEWPORT);
  ...
  if (uFlipY) gl.uniform1f(uFlipY, 1.0); // FBO/doc space, no flip
  ...
  // Draw each selected layer into union FBO (shift to origin by -minX,-minY)
  for (const idx of sel) {
    const L = layers[idx];
    if (!L || !L.texture || !L.visible || (L.opacity ?? 1) <= 0) continue;
    ...
    const c=Math.cos(rot), s=Math.sin(rot);
    function tf(x,y){let dx=(x-pivX)*scx, dy=(y-pivY)*scy; const rx=dx*c-dy*s, ry=dx*s+dy*c; return {x:pivX+rx+tx,y:pivY+ry+ty};}
    const q0=tf(Lx,Ly), q1=tf(Lx+Lw,Ly), q2=tf(Lx,Ly+Lh), q3=tf(Lx+Lw,Ly+Lh);

    const verts = new Float32Array([
      q0.x - minX, q0.y - minY, 0, 0,
      q1.x - minX, q1.y - minY, 1, 0,
      q2.x - minX, q2.y - minY, 0, 1,
      q2.x - minX, q2.y - minY, 0, 1,
      q1.x - minX, q1.y - minY, 1, 0,
      q3.x - minX, q3.y - minY, 1, 1
    ]);
    const vbo = gl.createBuffer();
    gl.bindBuffer(gl.ARRAY_BUFFER, vbo);
    gl.bufferData(gl.ARRAY_BUFFER, verts, gl.STREAM_DRAW);
  ...
  // Remove old layers and insert merged
  const top = sel[sel.length - 1];
  for (let k = sel.length - 1; k >= 0; k--) {
    const rm = layers[sel[k]];
    if (rm) {
      try { if (rm.texture) gl.deleteTexture(rm.texture); } catch {}
      try { if (rm.fbo) gl.deleteFramebuffer(rm.fbo); } catch {}
      layers.splice(sel[k], 1);
    }
  }
  ...
  const merged = {
    ...
    texture: tex, fbo: fbo,
    visible: true, opacity: 1,
    ox: minX, oy: minY, texW: outW, texH: outH,
    x:0, y:0, scaleX:1, scaleY:1, rotation:0,
    px: minX + outW*0.5, py: minY + outH*0.5
  };
  const insert = Math.min(top - (sel.length - 1), layers.length);
  layers.splice(insert, 0, merged);
  activeLayerIndex = insert;
  selectedLayerIndices = new Set([insert]);
...
function duplicateLayer(srcIndex = activeLayerIndex) {
  ...
  // Create a destination FBO the SAME SIZE as the source layer’s texture
  const W = Number.isFinite(src.texW) ? src.texW : fixedFBOWidth;
  const H = Number.isFinite(src.texH) ? src.texH : fixedFBOHeight;
  ...
  // Build cloned layer (copy doc rect + pivot in *document space*)
  const clone = {
    id: Date.now() + Math.random(),
    name: src.name + " copy",
    fbo: dstFbo,
    ...
    x: src.x, y: src.y, scaleX: src.scaleX, scaleY: src.scaleY, rotation: src.rotation,
    px: src.px, py: src.py,
  ...
  const insertAt = Math.min(activeLayerIndex + 1, layers.length);
  layers.splice(insertAt, 0, clone);
  activeLayerIndex = insertAt;
...
// === REPLACE: applySnapshotToLayer(layerIndex, snapshot) ===
// Restores a snapshot as a FULL-DOCUMENT layer with neutral transform.
// If your restore function has a different name/signature, drop this body into it.
function applySnapshotToLayer(layerIndex, snapshot) {
  if (!snapshot || !gl) return;
  const W = snapshot.w | 0, H = snapshot.h | 0;
  const L = layers?.[layerIndex]; if (!L) return;
  ...
  // Swap into layer; NORMALIZE rect and transform (avoids future crops)
  try { if (L.texture) gl.deleteTexture(L.texture); } catch {}
  try { if (L.fbo) gl.deleteFramebuffer(L.fbo); } catch {}

  L.texture = tex; L.fbo = fbo;
  L.ox = 0; L.oy = 0; L.texW = W; L.texH = H;
...
// ============================================================================
// [History] Exact GPU snapshot of a layer (no transforms applied, no flips).
// Purpose: capture the layer’s pixels 1:1 in an FBO+texture so Undo/Redo can
// restore without quality loss, cropping, or orientation issues.
// Returns: { index, w, h, texture, fbo }  — do NOT delete these in this fn.
// ============================================================================
function snapshotLayer(layerIndex) {
  const W = fixedFBOWidth | 0, H = fixedFBOHeight | 0;
  const L = layers?.[layerIndex];
  if (!gl || !L || !L.fbo || !L.texture || !W || !H) return null;

  // Create a GPU target for the snapshot (texture + FBO), full document size.
  const tex = gl.createTexture();
  gl.bindTexture(gl.TEXTURE_2D, tex);
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, W, H, 0, gl.RGBA, gl.UNSIGNED_BYTE, null);
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
  ...
  // WebGL2 fast path: blit L.fbo → snapshot FBO.
  if (gl instanceof WebGL2RenderingContext && gl.blitFramebuffer) {
    gl.bindFramebuffer(gl.READ_FRAMEBUFFER, L.fbo);
    ...
    // WebGL1 fallback: draw L.texture into snapshot FBO using the quad program (no flip).
    const prevFBO = gl.getParameter(gl.FRAMEBUFFER_BINDING);
    const prevProg = gl.getParameter(gl.CURRENT_PROGRAM);
    const prevViewport = gl.getParameter(gl.VIEWPORT);
    ...
    // Full-rect quad vertices (positions in px, UV in [0..1])
    const verts = new Float32Array([
      0,0, 0,0,  W,0, 1,0,  0,H, 0,1,
      0,H, 0,1,  W,0, 1,0,  W,H, 1,1
    ]);
  ...
  // Do NOT delete fbo/tex here. History needs to hold onto them.
  return { index: layerIndex, w: W, h: H, texture: tex, fbo };
}
...
// ============================================================================
// [History] Serialize and apply layer transform parameters.
// Purpose: allow Undo/Redo of non-baked transformations (grab/scale/rotate).
// These helpers only store and restore numeric transform-related fields.
// ============================================================================
function snapshotLayerParams(index) {
  ...
  return {
    ...
    px: +(L.px ?? fixedFBOWidth  * 0.5),
    py: +(L.py ?? fixedFBOHeight * 0.5),
    ox: +(L.ox ?? 0), oy: +(L.oy ?? 0),
...
function applyLayerParams(params) {
  if (!params) return;
  const L = layers[params.index];
  if (!L) return;
  L.x = params.x; L.y = params.y;
  L.scaleX = params.scaleX; L.scaleY = params.scaleY;
  L.rotation = params.rotation;
  L.px = params.px; L.py = params.py;
  L.ox = params.ox; L.oy = params.oy;
  L.texW = params.texW; L.texH = params.texH;
  needsRedraw = true;
  requestDrawIfIdle?.();
}
...
// ============================================================================
// [History] Restore a previously captured snapshot back into the target layer.
// Purpose: exact pixel restore for Undo/Redo, avoiding flips/crops.
// Accepts: { index, w, h, texture, fbo } from snapshotLayer(...).
// ============================================================================
function restoreLayerSnapshot(snap) {
  const L = layers?.[snap?.index];
  if (!gl || !L || !L.fbo || !snap) return;

  const W = snap.w | 0, H = snap.h | 0;

  // WebGL2 fast path
  if (snap.fbo && gl instanceof WebGL2RenderingContext && gl.blitFramebuffer) {
  ...
  // WebGL1 fallback: draw snapshot.texture into layer FBO using quad.
  const prevFBO = gl.getParameter(gl.FRAMEBUFFER_BINDING);
  const prevProg = gl.getParameter(gl.CURRENT_PROGRAM);
  const prevViewport = gl.getParameter(gl.VIEWPORT);
  ...
  gl.activeTexture(gl.TEXTURE0);
  gl.bindTexture(gl.TEXTURE_2D, snap.texture);
  gl.drawArrays(gl.TRIANGLES, 0, 6);
...
// ============================================================================
// History: central undo/redo manager
// Purpose:
//   - Record and revert editing operations in order, including:
//       • "stroke" and "fill" (pixel changes via snapshots)
//       • "transform_bake" (pixel changes from baking transforms, with params)
//       • "transform_params" (non-baked transform parameter changes only)
//       • Layer ops: visibility, opacity, rename, move, add, remove
//       • Project ops: clear_project, merge_layers
// Notes:
//   - Requires the following helpers to exist in this file/codebase:
//       snapshotLayer(index)         → { index,w,h,texture,fbo }
//       restoreLayerSnapshot(snap)   → void
//       disposeSnapshot(snap)        → void
//       blitFBOtoFBO(srcFbo,dstFbo,w?,h?) → void
//       createLayerFBO(w,h)          → { texture,fbo }
//       snapshotLayerParams(index)   → { index, x,y, scaleX,scaleY, rotation, px,py, ox,oy, texW,texH }
//       applyLayerParams(params)     → void
//   - UI helpers are optional and used with optional chaining:
//       rebuildLayersUI(), syncActiveAliases(), showStatusMessage()
// ============================================================================
const History = {
  stack: [],
  redo: [],
  ...
  // Record a new action and clear the redo branch.
  push(action) {
  ...
  // Dispose GPU resources (snapshots) associated with a pruned action.
  _dispose(action) {
    if (!action) return;

    // Generic before/after disposal
    try {
      if (action.before) disposeSnapshot(action.before);
      if (action.after)  disposeSnapshot(action.after);
    } catch (e) { console.warn("dispose before/after", e); }

    // Layer removal/addition snapshots (if embedded)
    try {
      if (action.removedLayer?.snapshot) disposeSnapshot(action.removedLayer.snapshot);
      if (action.addedLayer?.snapshot)   disposeSnapshot(action.addedLayer.snapshot);
    } catch (e) { console.warn("dispose layer add/remove", e); }

    // Merge specific: embedded snapshots in arrays
    if (action.type === "merge_layers") {
      try {
        if (action.added?.data?.snapshot) disposeSnapshot(action.added.data.snapshot);
        if (Array.isArray(action.removed)) {
          for (const r of action.removed) {
            if (r?.data?.snapshot) disposeSnapshot(r.data.snapshot);
          }
        }
      } catch (e) { console.warn("dispose merge_layers", e); }
    }
  },

  // Apply (redo) an action.
  _apply(action) {
    switch (action.type) {
      // Pixel snapshot actions
      case "stroke":
      case "fill":
      case "transform_bake":
        if (action.after) restoreLayerSnapshot(action.after);
        if (action.afterParams) applyLayerParams?.(action.afterParams);
        break;

      // Non-baked parameter change (transform-only) — no pixel copy
      case "transform_params":
        applyLayerParams?.(action.afterParams);
        break;

      // Layer view/model changes
      ...
      // Whole project capture/restore
      case "clear_project":
        this._restoreProject(action.afterAll);
        break;

      // Merge layers → remove originals; insert merged result
      case "merge_layers": {
        // action.removed: [{index, data:{name,visible,opacity,snapshot,params}}...]
        // action.added:   {index, data:{name,visible,opacity,snapshot,params}}
        if (!Array.isArray(action.removed) || !action.added) break;
        // Remove originals in DESC order to keep indices valid
        const toRemove = action.removed.map(r => r.index).sort((a, b) => b - a);
        for (const idx of toRemove) this._removeLayerAt(idx);
        // Insert merged layer at stored index
  ...
  // Apply the inverse (undo) of an action.
  _applyInverse(action) {
    switch (action.type) {
      // Pixel snapshot actions (inverse = restore "before")
      case "stroke":
      case "fill":
      case "transform_bake":
        if (action.before) restoreLayerSnapshot(action.before);
        if (action.beforeParams) applyLayerParams?.(action.beforeParams);
        break;

      // Non-baked parameter change (inverse params)
      case "transform_params":
        applyLayerParams?.(action.beforeParams);
        break;
      ...
      // Merge layers (inverse) → remove merged result; reinsert originals
      case "merge_layers": {
  ...
  // Move a layer within the stack and refresh UI/selection.
  _moveLayer(from, to) {
    if (from === to) return;
  ...
  // Remove a layer at index; dispose GPU resources; maintain selection.
  ...
  // Insert a layer at index from stored data (name/visible/opacity/snapshot[,params]).
  // Recreates GPU targets and copies pixels from the snapshot. Preserves transform params if provided.
  _insertLayer(index, data) {
    const { name, visible, opacity, snapshot, params } = data;
    const { texture, fbo } = createLayerFBO(fixedFBOWidth, fixedFBOHeight);
    ...
    const layer = {
      ...
      // Preserve transform/pivot if provided; else defaults.
      x:      (params && Number.isFinite(params.x))      ? params.x      : 0,
      y:      (params && Number.isFinite(params.y))      ? params.y      : 0,
      scaleX: (params && Number.isFinite(params.scaleX)) ? params.scaleX : 1,
      scaleY: (params && Number.isFinite(params.scaleY)) ? params.scaleY : 1,
      rotation:(params && Number.isFinite(params.rotation)) ? params.rotation : 0,
      px:     (params && Number.isFinite(params.px))     ? params.px     : fixedFBOWidth  * 0.5,
      py:     (params && Number.isFinite(params.py))     ? params.py     : fixedFBOHeight * 0.5,
      ox:     (params && Number.isFinite(params.ox))     ? params.ox     : 0,
      oy:     (params && Number.isFinite(params.oy))     ? params.oy     : 0,
      texW:   (params && Number.isFinite(params.texW))   ? params.texW   : fixedFBOWidth,
      texH:   (params && Number.isFinite(params.texH))   ? params.texH   : fixedFBOHeight,

      history: [], redo: []
    };

    layers.splice(index, 0, layer);
    blitFBOtoFBO(snapshot.fbo, layer.fbo, fixedFBOWidth, fixedFBOHeight);
    activeLayerIndex = index;
    syncActiveAliases?.();
    rebuildLayersUI?.();
  },

  // Capture the whole project (doc size + per-layer snapshots + optional background).
  // Also capture each layer's transform params so a full restore preserves pivots.
  _captureProject() {
    const all = {
      w: fixedFBOWidth,
      h: fixedFBOHeight,
  ...
  // Restore the whole project from a prior capture (sizes, background, layers).
  // Applies snapshots AND transform params so overlays/gizmos remain correct.
  _restoreProject(all) {
    if (!all) return;
    try {
      // Dispose current layers
      layers.forEach(L => { try { gl.deleteTexture(L.texture); } catch {} try { gl.deleteFramebuffer(L.fbo); } catch {} });
      layers = [];
      ...
      // Rebuild layers and copy their snapshots; apply stored params
      all.layers.forEach((info) => {
        ...
        const L = {
          ...
          // Defaults; will be replaced by applyLayerParams below
          x: 0, y: 0, scaleX: 1, scaleY: 1, rotation: 0,
          px: all.w * 0.5, py: all.h * 0.5,
          ox: 0, oy: 0, texW: all.w, texH: all.h,
          history: [], redo: []
        };
        layers.push(L);
        blitFBOtoFBO(info.snapshot.fbo, L.fbo, all.w, all.h);
        if (info.params) applyLayerParams(info.params);
      });
...
// ============================================================================
// [Transform] End a transform interaction.
// Purpose: push history for parameter-only transforms and for baked transforms
// (with pixel snapshots). Prevents off-canvas cropping by allowing a bake
// guard if such a predicate exists in the codebase.
// ============================================================================
function endTransform(bake = true) {
  const active = transformTool && transformTool.mode && transformTool.mode !== "idle";
  if (!active) {
    try { isTwoFingerGesture = false; pinchStart = null; } catch {}
    if (transformTool) transformTool.mode = "idle";
    needsRedraw = true;
  ...
  const L = getActiveLayer?.();
  const paramsBefore = transformTool.paramsBefore || snapshotLayerParams(activeLayerIndex);
  const paramsAfter  = snapshotLayerParams(activeLayerIndex);

  // Optional bake guard: if codebase provides a clip test, skip bake to avoid cropping.
  try {
    if (bake && typeof activeLayerWouldClip === "function" && activeLayerWouldClip()) {
      bake = false;
      showStatusMessage?.("Skipped bake to prevent off-canvas cropping", "info");
    }
  } catch {}

  let didBake = false, beforeSnap = null, afterSnap = null;

  if (bake && L && typeof applyActiveLayerTransform === "function") {
    try {
      beforeSnap = snapshotLayer(activeLayerIndex);
      applyActiveLayerTransform();            // bake layer’s transform into pixels
      afterSnap  = snapshotLayer(activeLayerIndex);
      didBake = true;
    } catch (e) {
      console.warn("[endTransform] bake failed:", e);
      didBake = false;
  ...
  // Push history: parameter change (even if not baked).
  const paramsChanged =
    paramsBefore && paramsAfter &&
    JSON.stringify(paramsBefore) !== JSON.stringify(paramsAfter);

  if (paramsChanged) {
    History.push({
      type: "transform_params",
      beforeParams: paramsBefore,
      afterParams:  paramsAfter
    });
  }

  // Push history: baked pixels (with params for exact redo/undo state).
  if (didBake && beforeSnap && afterSnap) {
    History.push({
      type: "transform_bake",
      before: beforeSnap,
      after:  afterSnap,
  ...
  try { transformTool.start = null; } catch {}
  try { transformTool.ref   = null; } catch {}
  try { transformTool.paramsBefore = null; } catch {}
  try { transformTool.shiftInvert = false; } catch {}
  try { transformTool.pivotCanvas = null; } catch {}
  if (transformTool) transformTool.mode = "idle";
  try { isTwoFingerGesture = false; pinchStart = null; } catch {}

  // Optional: re-arm if the layer is transform-locked for mobile workflows.
  const stayArmed = !!(L && L.transformLocked);
  if (transformTool) transformTool.mobileCombo = stayArmed;
  if (stayArmed) queueMicrotask?.(() => startTransform?.("grab"));

  showStatusMessage?.(
    didBake ? "Transform applied" : (bake ? "Transform set" : "Transform set"),
    didBake ? "success" : "info"
  );
...
// === NEW: prevents loss when a bake would clip against the fixed doc FBO ===
function activeLayerWouldClip() {
  ...
  const tx = Number.isFinite(L.x) ? L.x : 0;
  const ty = Number.isFinite(L.y) ? L.y : 0;
  const px = Number.isFinite(L.px) ? L.px : W * 0.5;
  const py = Number.isFinite(L.py) ? L.py : H * 0.5;

  const c = Math.cos(rot), s = Math.sin(rot);
  function tf(x, y) {
    let dx = (x - px) * sx, dy = (y - py) * sy;
    const rx = dx * c - dy * s, ry = dx * s + dy * c;
    return { x: px + rx + tx, y: py + ry + ty };
  }
  ...
  // If ANY corner lies outside the document, a bake-into-doc-FBO would crop.
  return (
    p0.x < 0 || p0.y < 0 || p0.x > W || p0.y > H ||
    p1.x < 0 || p1.y < 0 || p1.x > W || p1.y > H ||
    p2.x < 0 || p2.y < 0 || p2.x > W || p2.y > H ||
    p3.x < 0 || p3.y < 0 || p3.x > W || p3.y > H
  );
}



// === NEW: promote once when heavily upscaled to preserve quality ===
function promoteLayerForScale(threshold = 1.25) {
  const L = getActiveLayer?.();
  if (!gl || !L || !L.texture || !quadProgram) return false;

  const sx = Math.abs(Number.isFinite(L.scaleX) ? L.scaleX : 1);
  const sy = Math.abs(Number.isFinite(L.scaleY) ? L.scaleY : 1);
  const maxS = Math.max(sx, sy);
  if (maxS <= threshold) return false;

  const srcW = Math.max(1, L.texW | 0);
  const srcH = Math.max(1, L.texH | 0);
  const maxTex = gl.getParameter(gl.MAX_TEXTURE_SIZE) | 0;

  const outW = Math.min(maxTex, Math.max(1, Math.round(srcW * sx)));
  const outH = Math.min(maxTex, Math.max(1, Math.round(srcH * sy)));
  ...
  // Grow texture once (NEAREST; no blur)
  const tex = gl.createTexture();
  gl.bindTexture(gl.TEXTURE_2D, tex);
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, outW, outH, 0, gl.RGBA, gl.UNSIGNED_BYTE, null);
  gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
  ...
  // Swap in and normalize scale back toward 1 (keeps visual size the same)
  const oldTex = L.texture, oldFbo = L.fbo;
  L.texture = tex; L.fbo = fbo;
  L.texW = outW; L.texH = outH;
  if (sx) L.scaleX /= sx;
  if (sy) L.scaleY /= sy;
...
// === REPLACE: applyActiveLayerTransform() ===
// Bake current transform into a NEW texture sized to the transformed bbox (no crop to document).
function applyActiveLayerTransform() {
  ...
  // Tight AABB that can extend outside the document
  ...
  // Create target (bbox-sized), render layer into it (shift to origin)
  const prevFBO = gl.getParameter(gl.FRAMEBUFFER_BINDING);
  const prevProg = gl.getParameter(gl.CURRENT_PROGRAM);
  const prevViewport = gl.getParameter(gl.VIEWPORT);
  ...
  gl.useProgram(quadProgram);
  const uFlipY = gl.getUniformLocation(quadProgram, "u_flipY");
  if (uFlipY) gl.uniform1f(uFlipY, 1.0); // no vertical flip
  const uRes = gl.getUniformLocation(quadProgram, "u_resolution");
  if (uRes) gl.uniform2f(uRes, outW, outH);
  const uOpacity = gl.getUniformLocation(quadProgram, "u_layerOpacity");
  if (uOpacity) gl.uniform1f(uOpacity, 1.0);
  ...
  // Shifted verts (bbox origin at 0,0)
  const verts = new Float32Array([
    p0.x - minX, p0.y - minY, 0, 0,
    p1.x - minX, p1.y - minY, 1, 0,
    p2.x - minX, p2.y - minY, 0, 1,
    p2.x - minX, p2.y - minY, 0, 1,
    p1.x - minX, p1.y - minY, 1, 0,
    p3.x - minX, p3.y - minY, 1, 1
  ]);
  const vbo = gl.createBuffer();
  gl.bindBuffer(gl.ARRAY_BUFFER, vbo);
  gl.bufferData(gl.ARRAY_BUFFER, verts, gl.STREAM_DRAW);
  ...
  // Swap into layer; keep off-document pixels via ox/oy and bbox size
  const oldTex = L.texture, oldFbo = L.fbo;
  L.texture = dstTex; L.fbo = dstFBO;
  L.ox = minX; L.oy = minY; L.texW = outW; L.texH = outH;
  L.x = 0; L.y = 0; L.scaleX = 1; L.scaleY = 1; L.rotation = 0;
  L.px = L.ox + L.texW * 0.5; L.py = L.oy + L.texH * 0.5;
...
async function importProjectZIP(file) {
  ...
  try {
    ...
    // Back-compat defaults for v1 exports
    ...
    // Resize document/FBOs to match the project
    if (typeof setFixedFBODimensions === "function") {
      setFixedFBODimensions(W, H);
    } else {
      // fallback if you set directly elsewhere
      window.fixedFBOWidth = W;
      window.fixedFBOHeight = H;
    }
    initPaintLayerFixed?.(); // clears/rebuilds base state for W×H document
    ...
    if (manifest.hasBackground && zip.file("background.png")) {
      ...
      // Draw into your background layer/canvas if you have one:
      if (typeof setDocumentBackgroundImageBitmap === "function") {
        await setDocumentBackgroundImageBitmap(bgImg);
      } else {
        // no-op: your renderer may composite currentImage separately
        window.currentImage = bgImg;
      }
    }

    // Helper to upload a bitmap to a new texture/FBO of exact size
    function createLayerFromBitmap(name, bmp, rect, opts) {
      const w = Math.max(1, rect.texW|0), h = Math.max(1, rect.texH|0);
      ...
      gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, gl.CLAMP_TO_EDGE);
      gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL, false);
      gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, w, h, 0, gl.RGBA, gl.UNSIGNED_BYTE, bmp);
      ...
      // Insert a layer object compatible with your renderer, including the NEW rect fields
      const layer = {
        id: Date.now() + Math.random(),
        name,
        texture: tex,
        ...
        // transform identity (content was already baked inside this texture)
        x: 0, y: 0, scaleX: 1, scaleY: 1, rotation: 0,

        // pivot stays in document space
        px: Number.isFinite(opts.px) ? opts.px : (W * 0.5),
        py: Number.isFinite(opts.py) ? opts.py : (H * 0.5),

        // NEW: layer rect in document space
    ...
    // Layers: load in manifest order (bottom→top)
    const defs = (manifest.layers || []).slice().sort((a,b) => a.index - b.index);
    for (let i = 0; i < defs.length; i++) {
      const def = defs[i];

      // locate the PNG; the exporter named them "index_Name_wxh.png"
      const prefix = String(def.index).padStart(3, "0") + "_";
      const candidates = Object.keys(zip.files).filter(k => k.startsWith("layers/"));
      const match = candidates.find(k => k.split("/").pop().startsWith(prefix)) || candidates[def.index];
      ...
      // Back-compat: v1 had no rects; assume full canvas at (0,0)
      const rect = (ver >= 2) ? {
      ...
      createLayerFromBitmap(def.name || `Layer ${i+1}`, bmp, rect, {
        ...
        px: Number.isFinite(def.px) ? def.px : (W * 0.5),
        py: Number.isFinite(def.py) ? def.py : (H * 0.5)
      });
    }

    // Topmost layer active, refresh UI
    activeLayerIndex = Math.max(0, layers.length - 1);
    syncActiveAliases?.();
    rebuildLayersUI?.();
    needsRedraw = true;
...
// ————————————————————————————————————————————————
// Layers: Import/Export single layer (Imp / Exp)
// ————————————————————————————————————————————————
...
// Compose full document into an offscreen 2D canvas (all layers + background)
const offscreenCanvas = document.createElement("canvas");
offscreenCanvas.width = docW;
offscreenCanvas.height = docH;
const offscreenCtx = offscreenCanvas.getContext("2d");

// start transparent
offscreenCtx.clearRect(0, 0, docW, docH);

// optional background
if (currentImage) {
  offscreenCtx.drawImage(currentImage, 0, 0, docW, docH);
}

// helper: read FBO into ImageData (w,h = layer texture size)
function readFBOToImageData(fbo, w, h) {
  ...
  gl.bindFramebuffer(gl.FRAMEBUFFER, null);
  return new ImageData(new Uint8ClampedArray(px), W, H);
}

// draw all visible layers from bottom → top with transforms and per-layer rects
for (let i = 0; i < layers.length; i++) {
  const L = layers[i];
  if (!L || !L.fbo || !L.visible || (L.opacity ?? 1) <= 0) continue;

  const Lx = Number.isFinite(L.ox) ? L.ox : 0;
  const Ly = Number.isFinite(L.oy) ? L.oy : 0;
  const Lw = Math.max(1, Number.isFinite(L.texW) ? L.texW : docW);
  const Lh = Math.max(1, Number.isFinite(L.texH) ? L.texH : docH);

  // pull pixels
  const imgData = readFBOToImageData(L.fbo, Lw, Lh);

  // put into temp canvas
  const src = document.createElement("canvas");
  src.width = Lw; src.height = Lh;
  src.getContext("2d").putImageData(imgData, 0, 0);

  // flip vertically (WebGL bottom-left → Canvas top-left)
  const srcFlipped = document.createElement("canvas");
  srcFlipped.width = Lw; srcFlipped.height = Lh;
  const sctx = srcFlipped.getContext("2d");
  sctx.translate(0, Lh);
  sctx.scale(1, -1);
  sctx.drawImage(src, 0, 0);

  // apply layer transform in document space
  const scx = Number.isFinite(L.scaleX) ? L.scaleX : 1;
  const scy = Number.isFinite(L.scaleY) ? L.scaleY : 1;
  const rot = Number.isFinite(L.rotation) ? L.rotation : 0;

  // choose a sensible pivot: explicit pivot if present, else center of the layer rect
  const pivX = Number.isFinite(L.px) ? L.px : (Lx + Lw * 0.5);
  const pivY = Number.isFinite(L.py) ? L.py : (Ly + Lh * 0.5);

  offscreenCtx.save();
  offscreenCtx.globalAlpha = Math.max(0, Math.min(1, L.opacity ?? 1));
  offscreenCtx.translate(pivX, pivY);
  offscreenCtx.rotate(rot);
  offscreenCtx.scale(scx, scy);

  // draw so the layer’s top-left maps to (Lx, Ly) after transform
  offscreenCtx.drawImage(srcFlipped, Lx - pivX, Ly - pivY);
  offscreenCtx.restore();
}

// --- build circular avatar from composed offscreen ---
...
// cover-fit scale
const scale = Math.max(avatarSize / docW, avatarSize / docH);
const drawWidth = docW * scale;
const drawHeight = docH * scale;
const dx = (avatarSize - drawWidth) / 2;
const dy = (avatarSize - drawHeight) / 2;

avatarCtx.drawImage(offscreenCanvas, dx, dy, drawWidth, drawHeight);
