...
// === Touch debug globals ===
let touchDebug = [];            // [{x,y} in wrapper px]
let touchDebugActive = false;   // any active touches?


// Last *screen-space* pointer (wrapper-relative) used for transform HUD
let lastTransformPointerPx = null;

function setTransformPointerFromClient(clientX, clientY) {
  const wrap = canvasWrapper.getBoundingClientRect();
  lastTransformPointerPx = { x: clientX - wrap.left, y: clientY - wrap.top };
}
...
// Solid color fragment shader (for dashed line quads)
const solidFS = `
  precision mediump float;
  varying vec2 v_texCoord;  // not used, but VS provides it
...
function initGL() {
  ...
  solidProgram   = createProgram(gl, quadVS, solidFS); 
}
...
function onTransformPointerMove(ev) {
  ...
  // Update dashed-line HUD pointer in screen px (wrapper-relative)
  if (ev && Number.isFinite(ev.clientX) && Number.isFinite(ev.clientY)) {
    const wrap = canvasWrapper.getBoundingClientRect();
    // store on window so drawScene can read it even if not pre-declared
    window.lastTransformPointerPx = { x: ev.clientX - wrap.left, y: ev.clientY - wrap.top };
  }
  ...
  if (transformTool.mode === "grab") {
    ...
    let delta = a1 - a0;
    // normalize to [-PI, PI] for stable rotation
...
function onTouchStart(ev) {
  ...
  if (ev.touches.length === 1) {
    ...
    setTransformPointerFromClient(t.clientX, t.clientY);
    // Drive desktop path so transformTool.start is set consistently
    onTransformPointerMove(t);
  } else if (ev.touches.length === 2) {
    // prepare pinch state (you already had layerPinch in outer scope)
    ev.preventDefault();
    ev.stopImmediatePropagation();
    const t1 = ev.touches[0], t2 = ev.touches[1];

    // HUD pointer = midpoint of the two touches
    setTransformPointerFromClient(
    ...
    const dist0 = Math.max(1e-6, Math.hypot(dx, dy));
...
function onTouchMove(ev) {
  ...
  if (ev.touches.length === 2 && layerPinch) {
    ...
    // HUD pointer tracks pinch midpoint
    setTransformPointerFromClient(
      (t1.clientX + t2.clientX) * 0.5,
      (t1.clientY + t2.clientY) * 0.5
    );
  ...
  // One-finger drag → reuse the desktop path (grab)
  if (ev.touches.length === 1 && transformTool.mode !== "idle") {
    ev.preventDefault();
    ev.stopImmediatePropagation();
    const t = ev.touches[0];
...
function onTouchEnd(ev) {
  ...
  // Clear layerPinch when fingers lift below 2
...
//   // 3) Brush overlay + transform ring
//   const isTransforming = !!(transformTool && transformTool.mode !== "idle");
//   const overlayActive  = isDrawing || isTransforming || brushHUD.visible; // keep your overlay behavior
//   const ringActive     = isTransforming;                                   // ring ONLY during transforms
...
//     // Red ring only while transforming (never while drawing)
//     if (ringActive) {
//       const posX = overlayPosition[0] * canvas.width;
//       const posY = overlayPosition[1] * canvas.height;
//       const brushW = brushSize * canvas.width;
//       const radius = brushW * 0.5;
//       const thickness = Math.max(1.0, brushW * 0.06); // ~6% of brush width
...
function drawScene() {
  ...
  // 3) Brush overlay + transform ring + dashed tether (only during transform)
  const isTransforming = !!(transformTool && transformTool.mode !== "idle");
  const overlayActive  = isDrawing || isTransforming || brushHUD.visible;
  const ringActive     = isTransforming;
  ...
  if (overlayActive &&
    ...
    if (ringActive) {
      ...
      // dashed tether from pointer to circle center
      const ax = lastPointer?.cx ?? posX;
      const ay = lastPointer?.cy ?? posY;

      // keep it visible but light on fill-rate
      const dashLen = 10.0;
      const gapLen  = 6.0;
      const linePx  = 1.5;
...
//   // 3) Brush overlay (ONLY while drawing or transforming)
//   //const overlayActive = isDrawing || (transformTool && transformTool.mode !== "idle");
...
//     // <<< ADD THIS: red ring matching brush size
//     const posX = overlayPosition[0] * canvas.width;
//     const posY = overlayPosition[1] * canvas.height;
//     const brushW = brushSize * canvas.width;
//     const radius = brushW * 0.5;
