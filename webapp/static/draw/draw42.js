...
// One-time global set (put near your globals if not present)
...
function rebuildLayersUI() {
  ...
  if (!window.__mobileLayerTransformSetup) {
    ...
    function onTouchStart(ev) {
      ...
      if (ev.touches.length === 2) {
        ...
        console.debug("[layersUI] touchstart combo pinch init", layerPinch);
      }
    }
  ...
  // Build rows. Keep header at the top; insert each new row just below header so newest is on top without reversing.
  (layers || []).forEach((L, idx) => {
    ...
    selCb.addEventListener("click", (e) => {
      ...
      console.debug("[layersUI] checkbox toggle", idx, Array.from(selectedLayerIndices));
    });
    sel.appendChild(selCb);

    // — Visibility (eye) + Mobile Transform
    ...
    visBtn.addEventListener("click", (e) => {
      ...
      updateEye();
      needsRedraw = true;
      console.debug("[layersUI] visibility toggle", idx, L.visible);
    });

    vis.appendChild(visBtn);

    // Mobile-only transform toggle
    ...
    transformBtn.addEventListener("click", (ev) => {
      ...
      needsRedraw = true;
      console.debug("[layersUI] transform lock", idx, !currentlyLocked);
    });
    ...
    const commitName = () => {
      ...
      if (v !== L.name) {
        L.name = v;
        updateEye();
        console.debug("[layersUI] rename", idx, v);
      }
    };
    ...
    dupBtn.addEventListener("click", (ev) => {
      ...
      clearTransformLockAll();
      duplicateLayer(idx);
      console.debug("[layersUI] duplicate", idx);
    });
    ...
    upBtn.addEventListener("click", (ev) => { 
      ...
      clearTransformLockAll();
      moveLayerUp(idx); 
      console.debug("[layersUI] move up", idx);
    });
    ...
    downBtn.addEventListener("click", (ev) => { 
      ...
      clearTransformLockAll();
      moveLayerDown(idx); 
      console.debug("[layersUI] move down", idx);
    });
    ...
    opRange.addEventListener("input", () => {
      ...
      if (!Number.isNaN(v) && v !== L.opacity) {
        ...
        needsRedraw = true;
        console.debug("[layersUI] opacity", idx, L.opacity);
      }
    });
    ...
    el.addEventListener("click", (ev) => {
      ...
      if (ranged) {
        ...
        toggleSelection(idx, true);
        activeLayerIndex = idx; // make last-touched active
      } else {
        selectSingle(idx, L);
        return; // selectSingle rebuilds UI
      }

      // Update visuals without full rebuild:
    ...
    // Insert newest directly under the header (no DOM reversing)
...
/* =========================================================
 * History: core (complete update)
 * =======================================================*/
const History = {
  /* Public state */
  stack: [],
  redo: [],
  limit: typeof UNDO_LIMIT !== "undefined" ? UNDO_LIMIT : 50,

  /* Internal flags */
  _inUndoRedo: false,
  _tx: null,               // active transaction (if any)
  _logging: true,          // set to false to silence logs

  /* ---------- Config ---------- */
  setLimit(n) {
    this.limit = Math.max(1, n|0);
    while (this.stack.length > this.limit) this._dispose(this.stack.shift());
    this._log("[History] setLimit", this.limit);
  },
  setLogging(enabled) {
  ...
  /* ---------- Push / Transactions ---------- */
  /**
   * Push a completed action onto the history stack.
   * Resets redo and enforces limit. Safe to call outside transactions.
   */
  push(action) {
    if (!action || this._inUndoRedo) return;
    try {
      this.stack.push(action);
      if (this.stack.length > this.limit) this._dispose(this.stack.shift());
      // once a new action is pushed, redo history is invalid
      if (this.redo.length) this._drainRedo();
      this._log("[History] push", action.type);
    } catch (e) {
      console.error("[History] push failed:", e);
      // Dispose any snapshots in the failed action
      this._dispose(action);
    }
  },

  /**
   * Begin a grouped history transaction.
   * Usage:
   *   History.begin("stroke");
   *   // ... perform edits ...
   *   History.commit(); // auto-snapshots 'after' if none provided
   *   // or History.cancel();
   */
  begin(type, beforeSnap /* optional precomputed snap */) {
    if (this._inUndoRedo || this._tx) return;
    try {
      const L = getActiveLayer?.();
      const before = beforeSnap || (L ? snapshotLayer(activeLayerIndex) : null);
      this._tx = { type, before, after: null, meta: {} };
      this._log("[History] begin", type);
    } catch (e) {
      console.warn("[History] begin failed:", e);
      this._tx = null;
    }
  },

  commit({ afterSnap, meta } = {}) {
    if (!this._tx || this._inUndoRedo) return;
    try {
      const L = getActiveLayer?.();
      const after = afterSnap || (L ? snapshotLayer(activeLayerIndex) : null);
      if (!this._tx.before && !after) {
      ...
      console.warn("[History] commit failed:", e);
      // clean up transaction snapshots if any
      if (this._tx?.before) this._dispose({ before: this._tx.before });
      if (afterSnap) this._dispose({ after: afterSnap });
      this._tx = null;
    }
  },
  ...
  cancel() {
    if (!this._tx) return;
    try {
      this._dispose(this._tx);
    } catch (e) {
      console.warn("[History] cancel dispose failed:", e);
    } finally {
      this._tx = null;
      this._log("[History] cancel");
    }
  },
  ...
  undo() {
    ...
    try {
      ...
      this._log("[History] undo", action.type);
    } catch (e) {
      console.error("[History] undo failed:", e);
      // action already popped; nothing to re-push
      this._dispose(action);
    } finally {
  ...
  redoDo() {
    ...
    try {
      ...
      this._log("[History] redo", action.type);
    } catch (e) {
  ...
  clear() {
    ...
    try {
      this.stack.forEach(a => this._dispose(a));
      this.redo.forEach(a => this._dispose(a));
    } catch (e) {
      console.warn("[History] clear dispose error:", e);
    } finally {
      this.stack.length = 0;
      this.redo.length = 0;
      this.cancel();
  ...
  _dispose(action) {
    ...
    try {
      ...
      console.warn("[History] _dispose error:", e);
    }
  },
  ...
  _apply(action) {
    switch (action.type) {
      ...
      case "layer_visibility":
        if (layers[action.index]) layers[action.index].visible = action.next;
        break;

      case "layer_opacity":
        if (layers[action.index]) layers[action.index].opacity = action.next;
        break;

      case "layer_rename":
        if (layers[action.index]) layers[action.index].name = action.next;
        rebuildLayersUI?.();
        break;
      ...
      default:
        this._log("[History] _apply unknown type:", action.type);
        break;
    }
  },
  ...
  _applyInverse(action) {
    switch (action.type) {
      ...
      case "layer_visibility":
        if (layers[action.index]) layers[action.index].visible = action.prev;
        break;

      case "layer_opacity":
        if (layers[action.index]) layers[action.index].opacity = action.prev;
        break;

      case "layer_rename":
        if (layers[action.index]) layers[action.index].name = action.prev;
        rebuildLayersUI?.();
        break;
      ...
      default:
        this._log("[History] _applyInverse unknown type:", action.type);
        break;
    }
  },
  ...
  _moveLayer(from, to) {
    ...
    activeLayerIndex = Math.max(0, Math.min(to, layers.length - 1));
    syncActiveAliases?.();
    rebuildLayersUI?.();
  },
  ...
  _removeLayerAt(index) {
    ...
    } catch (e) { console.warn("[History] GL cleanup on remove failed:", e); }
  ...
  _insertLayer(index, data) {
    ...
    if (snapshot?.fbo) blitFBOtoFBO?.(snapshot.fbo, layer.fbo);
  ...
  _captureProject() {
    ...
    if (typeof currentImage !== "undefined" && currentImage) {
      ...
      } catch (e) { console.warn("[History] captureProject background", e); }
    }
    return all;
  },
  ...
  _drainRedo() {
    try {
      // When a new action is pushed, redo is invalid; dispose its snapshots.
      this.redo.forEach(a => this._dispose(a));
    } catch (e) {
      console.warn("[History] _drainRedo dispose failed:", e);
    } finally {
      this.redo.length = 0;
    }
  },
  ...
  _log(...args) {
    if (this._logging) console.log(...args);
  }
};

/* =========================================================
 * History: stroke grouping (uses transactions)
 * =======================================================*/
...
function beginStrokeHistory() {
  try {
    ...
    History.begin("stroke", __strokeBeforeSnap);
...
function endStrokeHistory(type = "stroke") {
  try {
    const L = getActiveLayer?.(); 
    if (!L || !__strokeBeforeSnap) { 
      __strokeBeforeSnap = null; 
      History.cancel();
      return; 
    }
    const after = snapshotLayer?.(activeLayerIndex);
    // Commit uses the snapshots we gathered
    History.commit({ afterSnap: after, meta: { tool: type } });
    __strokeBeforeSnap = null;
  } catch (e) { 
...
document.addEventListener("keydown", (event) => {
  ...
  if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "z") { event.preventDefault(); undoStroke(); }
...
/* =========================================================
 * FINAL Flood Fill (self-contained history)
 * =======================================================*/
function performFloodFill(fx, fy) {
  const L = getActiveLayer?.();
  if (!L) return;

  // Snapshot BEFORE and begin transaction
  const before = snapshotLayer?.(activeLayerIndex);
  History.begin("fill", before);
  ...
  runFloodFillShader?.(L, targetColor, typeof tintColor !== "undefined" ? tintColor : [0,0,0,1]);

  // Snapshot AFTER + commit
  const after = snapshotLayer?.(activeLayerIndex);
  History.commit({ afterSnap: after, meta: { tool: "fill", seed: [ix, iy] } });
...
// Reuse your downscalers if present; otherwise provide fallbacks.
function downscaleImageToWindow(image) {
  const maxWidth = window.innerWidth;
  const maxHeight = window.innerHeight;
  const scale = Math.min(maxWidth / image.width, maxHeight / image.height, 1);
  if (scale < 1) {
    ...
    c.width = Math.floor(image.width * scale);
    c.height = Math.floor(image.height * scale);
...
/**
 * Rasterize an image onto a canvas of size (W,H) using 'contain' fit
 * (keeps aspect, centers, letterboxes with transparent pixels).
 */
function rasterizeToFixedCanvas(img, W, H) {
  const c = document.createElement("canvas");
...
// Simple passthrough textured quad for blitting an HTMLCanvas/Image into an FBO.
// We reuse one program/buffer for all imports.
let __importBlit = null;
function ensureImportBlit() {
  if (__importBlit) return __importBlit;
  ...
  function makeShader(type, src) {
    ...
    if (!gl.getShaderParameter(sh, gl.COMPILE_STATUS)) {
      console.error("Import shader compile error:", gl.getShaderInfoLog(sh));
    }
    return sh;
  }
  ...
  if (!gl.getProgramParameter(prog, gl.LINK_STATUS)) {
    console.error("Import program link error:", gl.getProgramInfoLog(prog));
  }

  // Fullscreen quad (-1..1), UV 0..1
  ...
  __importBlit = { prog, vbo, aPos, aUV, uTex };
  return __importBlit;
}

/** Upload a canvas into a temp texture and draw it into target FBO. */
function blitCanvasToFBO(canvasSrc, targetFBO, W, H) {
  const { prog, vbo, aPos, aUV, uTex } = ensureImportBlit();

  // Create & configure temp texture
  const tex = gl.createTexture();
  gl.bindTexture(gl.TEXTURE_2D, tex);
  ...
  // Cleanup bindings (keep program/VBO cached)
  gl.bindTexture(gl.TEXTURE_2D, null);
  gl.bindFramebuffer(gl.FRAMEBUFFER, null);
  gl.useProgram(null);
...
/**
 * Core import: reads file → image → (optional) downscale → rasterize to project size
 * → insert new layer → blit → snapshot → push history.
 */
function importImageFileAsLayer(file) {
  if (!file) return;

  const reader = new FileReader();
  reader.onerror = (err) => console.error("[Import] FileReader error:", err);

  reader.onload = (e) => {
    const img = new Image();
    img.onerror = (err) => console.error("[Import] Image load error:", err);

    img.onload = () => {
      // if project is not initialized yet, bootstrap it with the image dims
      const projectReady = typeof fixedFBOWidth !== "undefined" && fixedFBOWidth > 0 && typeof fixedFBOHeight !== "undefined" && fixedFBOHeight > 0;
      if (!projectReady) {
        // Initialize a blank project to the image size (no background)
        fixedFBOWidth  = img.width;
        fixedFBOHeight = img.height;
        initPaintLayerFixed?.();
        initFloodFillProgram?.();
        updateCanvasSize?.({ width: fixedFBOWidth, height: fixedFBOHeight });
      ...
      // Optionally downscale super huge inputs (same idea as bg loader)
      const maybeDown = downscaleImageToWindow(img);
      if (maybeDown) {
        const tmp = new Image();
        tmp.onload = () => doImport(tmp);
        tmp.src = maybeDown;
      } else {
        doImport(img);
      }

      function doImport(sourceImg) {
        // Fit the image into the project size (contain)
        const canvasSrc = rasterizeToFixedCanvas(sourceImg, fixedFBOWidth, fixedFBOHeight);

        // Create a brand-new layer above the active one
        const insertIndex = Math.min((typeof activeLayerIndex === "number" ? activeLayerIndex : layers.length - 1) + 1, layers.length);
        const layerName = file.name ? `Import: ${file.name}` : "Imported Image";
        const { texture, fbo } = createLayerFBO(fixedFBOWidth, fixedFBOHeight);
        const newLayer = {
          id: Date.now() + Math.random(),
          name: layerName,
        ...
        // Blit the rasterized canvas into the new layer’s FBO
        blitCanvasToFBO(canvasSrc, newLayer.fbo, fixedFBOWidth, fixedFBOHeight);

        // Snapshot for history (so undo removes it; redo re-inserts with content)
        const snap = snapshotLayer?.(insertIndex);
        History.push({
          type: "add_layer",
          index: insertIndex,
          addedLayer: {
        ...
        // Center & refresh view (same feel as bg loader)
        void canvas.offsetWidth; 
        void canvasWrapper.offsetWidth;
        zoomScale = 1;
        panX = (canvasWrapper.clientWidth  - canvas.width ) / 2;
        ...
        showStatusMessage?.("Imported image as new layer", "info");
      }
    };
  ...
  try {
    ...
    console.error("[Import] Exception while reading file:", ex);
  }
}

// Hook up UI
const importLayerButton = document.getElementById("importLayerButton");
const importLayerInput  = document.getElementById("importLayerInput");

importLayerButton?.addEventListener("click", () => {
  importLayerInput?.click();
});
importLayerButton?.addEventListener("touchend", () => {
  importLayerInput?.click();
});

// On iOS, let users pick from gallery or camera
if (/iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream) {
  importLayerInput?.removeAttribute("capture");
}

importLayerInput?.addEventListener("change", (ev) => {
  const file = ev.target.files && ev.target.files[0];
  importImageFileAsLayer(file);
...
//–––––––––––––––––––
// EXPORT ACTIVE LAYER → PNG
//–––––––––––––––––––
// Toggle if your exported PNG looks upside down.
let EXPORT_FLIP_Y = true; // ← try false first

function sanitizeFilename(name) {
  return (name || "Layer").replace(/[<>:"/\\|?*\x00-\x1F]/g, "_").slice(0, 120);
}

function readFBOBytes(fbo, W, H) {
...
/**
 * Convert FBO -> Canvas, with optional Y flip.
 * If EXPORT_FLIP_Y = false, pixels are used as-is (no flip).
 * If true, rows are reversed to convert bottom-left to top-left.
 */
function fboToCanvas(fbo, W, H, flipY = EXPORT_FLIP_Y) {
  const src = readFBOBytes(fbo, W, H);
  ...
  if (flipY) {
    ...
    for (let y = 0; y < H; y++) {
      const srcOff = y * row;
      const dstOff = (H - 1 - y) * row;
      data.set(src.subarray(srcOff, srcOff + row), dstOff);
    }
  } else {
    // No flip
    data = new Uint8ClampedArray(src.buffer.slice(0));
  }

  // Put pixels into a bitmap canvas
  ...
  // When we *don’t* flip rows, the imageData is bottom-left–origin.
  // putImageData ignores transforms, so to ensure a top-left PNG
  // we draw onto a second canvas with a drawImage-based flip.
  if (!flipY) {
    ...
    ctx.save();
    ctx.translate(0, H);
    ctx.scale(1, -1);          // flip Y once
    ctx.drawImage(bmpCanvas, 0, 0);
    ctx.restore();
    return finalCanvas;
  }

  // If we flipped rows already, we’re done.
  return bmpCanvas;
}

function exportActiveLayerAsPNG() {
  const L = getActiveLayer?.();
  if (!L) { showStatusMessage?.("No active layer to export", "warn"); return; }

  const W = fixedFBOWidth|0, H = fixedFBOHeight|0;
  if (!W || !H) { showStatusMessage?.("Project has no size", "warn"); return; }

  const canvas = fboToCanvas(L.fbo, W, H, EXPORT_FLIP_Y);
  const dataURL = canvas.toDataURL("image/png");

  const name = sanitizeFilename(L.name || "Layer");
  ...
  // Safari/iOS fallback
  if (/iPad|iPhone|iPod|Safari/i.test(navigator.userAgent) && !/Chrome/i.test(navigator.userAgent)) {
    try { window.open(dataURL, "_blank"); } catch {}
  }

  showStatusMessage?.(`Exported "${name}"`, "info");
}

// UI bindings
document.getElementById("exportLayerButton")?.addEventListener("click", exportActiveLayerAsPNG);
document.getElementById("exportLayerButton")?.addEventListener("touchend", exportActiveLayerAsPNG);
...
// Optional: export any layer by index
function exportLayerIndexAsPNG(index) {
  const L = layers?.[index];
  if (!L) return;
  const W = fixedFBOWidth|0, H = fixedFBOHeight|0;
  const canvas = fboToCanvas(L.fbo, W, H);
  const dataURL = canvas.toDataURL("image/png");
  const a = document.createElement("a");
  a.href = dataURL;
  a.download = `${sanitizeFilename(L.name || `Layer_${index+1}`)}.png`;
  document.body.appendChild(a);
  a.click();
  a.remove();
}
